...
{
	FLERROR_LOG( PROGRAM_NAME, _T( "CD3DApplication::Creat BEGIN" ) );
    HRESULT hr;
	...
	//gmpbigsun( 20120621 ) : 구동시 로딩완료시간동안 클라 Hide
	//풀스크린 모드 윈도모드로 구동되고 로딩완료시 FullScreen으로 바꿔준다.

	//if( g_xFlyffConfig->IsFakeFullMode() == true )
	//{
	...
	//else
	//{
	//	m_dwWindowStyle = WS_OVERLAPPED | WS_CAPTION;
	//}
	...
	m_dwWindowStyle = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | 
        ...
        m_hWnd = CreateWindow( _T("saida"), m_strWindowTitle, m_dwWindowStyle,
    ...
    {
		FLERROR_LOG( PROGRAM_NAME, "ChooseInitialD3DSettings() Failed!!" );
        SAFE_RELEASE( m_pD3D );
        return DisplayErrorMsg( hr, MSGERR_APPMUSTEXIT );
    }

	FLTRACE_LOG( PROGRAM_NAME, "ChooseInitialD3DSettings OK!!" );
    ...
    {
		FLERROR_LOG( PROGRAM_NAME, "OneTimeSceneInit() Failed!!" );
        SAFE_RELEASE( m_pD3D );
        return DisplayErrorMsg( hr, MSGERR_APPMUSTEXIT );
    }

	FLTRACE_LOG( PROGRAM_NAME, "OneTimeSceneInit OK!!" );

    // Initialize the 3D environment for the app
    if( FAILED( hr = Initialize3DEnvironment() ) )
    {
		FLERROR_LOG( PROGRAM_NAME, "Initialize3DEnvironment() Failed!!" );
        SAFE_RELEASE( m_pD3D );
        return DisplayErrorMsg( hr, MSGERR_APPMUSTEXIT );
    }

	FLTRACE_LOG( PROGRAM_NAME, "Initialize3DEnvironment OK!!" );

	// The app is ready to go
    Pause( false );

	//gmpbigsun( 20120621 ) : 로딩이 완료되면 Show
	DestroyWindow( m_hWndLogo );
	::ShowWindow( m_hWnd, SW_SHOW );
	::SetFocus( m_hWnd );

	if( g_Option.m_bFullScreenOldData )
	{	
		g_Option.m_nToggleScreen = TRUE;
		g_Neuz.ToggleFullscreen();
		g_Option.m_nToggleScreen = FALSE;
	}
	...
	FLERROR_LOG( PROGRAM_NAME, _T( "CD3DApplication::Creat END" ) );
    return S_OK;
}
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        if( pDeviceInfo->DevType == D3DDEVTYPE_REF )
            FLStrcpy( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT("REF") );
        else if( pDeviceInfo->DevType == D3DDEVTYPE_HAL )
            FLStrcpy( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT("HAL") );
        else if( pDeviceInfo->DevType == D3DDEVTYPE_SW )
            FLStrcpy( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT("SW") );

        if( behaviorFlags & D3DCREATE_HARDWARE_VERTEXPROCESSING &&
            behaviorFlags & D3DCREATE_PUREDEVICE )
        {
            if( pDeviceInfo->DevType == D3DDEVTYPE_HAL )
                FLStrcat( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT(" (pure hw vp)") );
            else
                FLStrcat( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT(" (simulated pure hw vp)") );
        }
        else if( behaviorFlags & D3DCREATE_HARDWARE_VERTEXPROCESSING )
        {
            if( pDeviceInfo->DevType == D3DDEVTYPE_HAL )
                FLStrcat( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT(" (hw vp)") );
            else
                FLStrcat( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT(" (simulated hw vp)") );
        }
        else if( behaviorFlags & D3DCREATE_MIXED_VERTEXPROCESSING )
        {
            if( pDeviceInfo->DevType == D3DDEVTYPE_HAL )
                FLStrcat( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT(" (mixed vp)") );
            else
                FLStrcat( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT(" (simulated mixed vp)") );
        }
        else if( behaviorFlags & D3DCREATE_SOFTWARE_VERTEXPROCESSING )
        {
            FLStrcat( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT(" (sw vp)") );
        }
        ...
        if( pDeviceInfo->DevType == D3DDEVTYPE_HAL )
        {
            ...
            FLStrcat( m_strDeviceStats, _countof( m_strDeviceStats ), TEXT(": ") );
            ...
            FLStrncat( m_strDeviceStats, _countof( m_strDeviceStats ), szDescription, maxAppend );
        }
		...
		//gmpbigsun(20110524) : win64 cursor 
...
//        m_pd3dDevice->ShowCursor( TRUE );
        ...
        if( FAILED(hr) )
        {
			FLERROR_LOG( PROGRAM_NAME, "InitDeviceObjects failed" );
            DeleteDeviceObjects();
        }
        else
        {
            ...
            if( FAILED(hr) )
            {
				FLERROR_LOG( PROGRAM_NAME, "RestoreDeviceObjects failed" );
                InvalidateDeviceObjects();
            }
            else
            {
				FLERROR_LOG( PROGRAM_NAME, "RestoreDeviceObjects OK" );
		...
		FLERROR_LOG( PROGRAM_NAME, "Cleanup3DEverionment try again" );
        Cleanup3DEnvironment();
    }
	...
	FLERROR_LOG( PROGRAM_NAME, "Initialize3DEnvironment all failed and fall back to ther referrence rasterizer %d", hr );
...
HRESULT CD3DApplication::Reset3DEnvironment()
{
    ...
    if( FAILED( hr = m_pd3dDevice->Reset( &m_d3dpp ) ) )
	{
		...
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "Reset리턴값 : D3DERR_DEVICELOST가 나왔다." ) );
		}
...
//#endif	// __CLIENT
	...
	if( FALSE == g_Option.m_nToggleScreen )
		return S_OK;

	//if( m_bStartFullscreen )
	//{
	//	//gmpbigsun( 20110609 ) : 최초 풀모드에서 창모드로 전환시 다시 접속할때 윈도모드로 접속하도록 강제함.
	//	g_Option.m_bStartFullScreen = false;
...
//	}
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "npgl" ) );
		::PostMessage( GetSafeHwnd(), WM_CLOSE, TRUE, 0 );
		return S_FALSE;
	}
#endif //_USE_XTRAP
    ...
    if( m_bDeviceLost )
    {
        ...
        if( FAILED( hr = m_pd3dDevice->TestCooperativeLevel() ) )
        {
            ...
            if( D3DERR_DEVICELOST == hr )
			{
				//FLERROR_LOG( PROGRAM_NAME, _T( "TestCooperativeLevel리턴값 : D3DERR_DEVICELOST" ) );
                return S_OK;
			}
...
#endif
	{
		...
		if( D3DERR_DEVICELOST == hr )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "Present리턴값 : D3DERR_DEVICELOST" ) );
			m_bDeviceLost = true;
		}
	}
	SetFST();
...
void CD3DApplication::UpdateStats()
{
    ...
    if( fTime - fLastTime > 1.0f )
    {
        ...
        {
            FLSPrintf( strFmt, _countof( strFmt ), TEXT("backbuf %s, adapter %s"), 
        ...
        if( m_d3dEnumeration.AppUsesDepthBuffer )
        {
            FLSPrintf( strDepthFmt, _countof( strDepthFmt ), TEXT(" (%s)"), 
        ...
        FLSPrintf( m_strFrameStats, cchMaxFrameStats, _T("%.02f fps (%dx%d), %s%s%s"), m_fFPS,
...
{
	FLERROR_LOG( PROGRAM_NAME, "Cleanup3DEnvironment BEGIN" );
    if( m_pd3dDevice != NULL )
    {
        if( m_bDeviceObjectsRestored )
        {
		...
		{
			TCHAR szString[ 256 ];
			FLSPrintf( szString, _countof( szString ), "Device에서 Release되지 않은 갯수 : %d", nCount );
	...
	FLERROR_LOG( PROGRAM_NAME, "Cleanup3DEnvironment END" );
}
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
    ...
    switch( hr )
    {
        case D3DAPPERR_NODIRECT3D:
            FLStrcpy( strMsg, _countof( strMsg ),
        ...
        case D3DAPPERR_NOWINDOWABLEDEVICES:
            FLStrcpy( strMsg, _countof( strMsg ),
							 _T("This sample cannot run in a desktop\n")
        ...
        case D3DAPPERR_HALNOTCOMPATIBLE:
            FLStrcpy( strMsg, _countof( strMsg ),
							 _T("This sample requires functionality that is\n")
        ...
        case __HRESULT_FROM_WIN32( ERROR_FILE_NOT_FOUND ):
            FLStrcpy( strMsg, _countof( strMsg ), _T("Could not load required media." ) );
            break;

        case D3DAPPERR_RESETFAILED:
            FLStrcpy( strMsg, _countof( strMsg ), _T("Could not reset the Direct3D device." ) );
            break;
        ...
        case E_OUTOFMEMORY:
            FLStrcpy( strMsg, _countof( strMsg ), _T("Not enough memory.") );
            break;

        case D3DERR_OUTOFVIDEOMEMORY:
            FLStrcpy( strMsg, _countof( strMsg ), _T("Not enough video memory.") );
            break;

        case D3DERR_DRIVERINTERNALERROR:
            FLStrcpy( strMsg, _countof( strMsg ), _T("A serious problem occured inside the display driver.") );
        ...
        default:
			...
			char ErrorCode[12];
			FLSPrintf(ErrorCode, _countof( ErrorCode ), "%d", (int)hr);
			FLStrcat( strMsg, _countof( strMsg ), _T(ErrorCode));
			bDefault = TRUE;
    }
    ...
    if( MSGERR_APPMUSTEXIT == dwType )
    {
        ...
        FLStrcat( strMsg, _countof( strMsg ), _T("\n\nThis sample will now exit.") );
#ifdef __CLIENT		
    ...
    else
    {
        if( MSGWARN_SWITCHEDTOREF == dwType )
            FLStrcat( strMsg, _countof( strMsg ),
