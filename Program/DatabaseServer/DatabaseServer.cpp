...
//		FLASSERT( 0 );
...
#ifndef __M_USER_BOTS_DB_BASE
#ifdef __M_USER_BOTS_CHECK_DB
	user_bot::CBotMng::getInstance()->loadBehaviorScript();
#endif
#else
	user_bot::CBotDBMng::getInstance()->loadBehaviorScript();
	SetTimer(hWnd, TID_BOT_MNG_PROCESS, SEC(5), nullptr);
#endif
	...
	if( !g_dpLoginSrvr.StartServer( g_szLoginBindIP, PN_DBSRVR_0, FLStreamProtocol::GetInstance() ) )
	{
		return FALSE;
	}
	
	if( !CDPTrans::GetInstance()->StartServer( g_szWorldBindIP, PN_DBSRVR_1, FLStreamProtocol::GetInstance() ) )
	{
		return FALSE;
	}

	if( !g_dpCoreSrvr.StartServer( g_szCoreBindIP, PN_DBSRVR_2, FLStreamProtocol::GetInstance() ) )
	{
		return FALSE;
	}

	if( FALSE == g_dpAccountClient.ConnectToServer( lpConnection, PN_ACCOUNTSRVR_1, FLStreamProtocol::GetInstance() ) )
		return FALSE;
	...
	switch (message) 
	{
		case WM_COMMAND:
			...
			switch (wmId)
			{
				...
				case IDM_RELOAD_BOTCONFIG: {
					user_bot::CBotDBMng::getInstance()->reload();
					break;
				}
#endif
				default:
		...
		case WM_TIMER:
			{
				...
				{
#ifdef __M_USER_BOTS_DB_BASE
				case TID_BOT_MNG_PROCESS: {
					user_bot::CBotDBMng::getInstance()->process();
					break;
				}
#endif
					case TID_POSTPROC:
