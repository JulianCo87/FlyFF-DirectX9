...
public:
	...
	BOOL	Initialize();
	void	Handler( LPDB_OVERLAPPED_PLUS pov, ULONG_PTR dwCompletionKey );
	void	OnTimer();
	BOOL	PostRequest( int nQuery, BYTE* lpBuf = NULL, int nBufSize = 0, ULONG_PTR dwCompletionKey = 0 )	{	return m_pController->PostRequest( nQuery, lpBuf, nBufSize, dwCompletionKey );	}
	...
	void	Couple( CCouple* pCouple )	{	FLASSERT( m_pMgr );	if( m_pMgr ) m_pMgr->Couple( pCouple );	}
private:
