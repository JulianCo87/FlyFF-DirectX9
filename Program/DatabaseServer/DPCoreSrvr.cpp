...
CDPCoreSrvr::CDPCoreSrvr()
{
	...
	ON_MSG(PACKETTYPE_ADD_WSERV, onWServerConnected);
	ON_MSG(PACKETTYPE_REMOVE_WSERV, onWServerDisconnected);
#endif
}
...
void	CDPCoreSrvr::sendAddBot(const u_long uKey, const u_long idBot, std::string strName, const std::size_t nBotType) {
	BEFORESEND(ar, PACKETTYPE_ADD_BOT);
	ar << uKey << idBot;
	ar.WriteString(strName);
	ar << nBotType;
	SEND(ar, this, DPID_ALLPLAYERS);
}
void	CDPCoreSrvr::sendRemoveBot(const u_long uKey, const u_long idBot) {
	BEFORESEND(ar, PACKETTYPE_REMOVE_BOT);
	ar << uKey << idBot;
	SEND(ar, this, DPID_ALLPLAYERS);
}
void	CDPCoreSrvr::onWServerConnected(CAr& ar, LPBYTE /*lpBuf*/, u_long /*uBufSize*/) {
	u_long uKey = 0;
	ar >> uKey;
	if (uKey == 0) {
		return;
	}
	user_bot::CBotDBMng::getInstance()->addServer(uKey);
}
void	CDPCoreSrvr::onWServerDisconnected(CAr& ar, LPBYTE /*lpBuf*/, u_long /*uBufSize*/) {
	u_long uKey = 0;
	ar >> uKey;
	if (uKey == 0) {
		return;
	}
	user_bot::CBotDBMng::getInstance()->removeServer(uKey);
}
#endif
