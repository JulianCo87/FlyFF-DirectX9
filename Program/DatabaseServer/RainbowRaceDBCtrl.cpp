...
void CRainbowRaceDBCtrl::Handler( LPDB_OVERLAPPED_PLUS pov, ULONG_PTR dwCompletionKey )
{
...
void CRainbowRaceDBCtrl::LoadRainbowRace( DPID dpId )
{
	...
	FLSPrintf( szQuery, _countof( szQuery ), "RAINBOWRACE_STR 'S1', '%02d', 0, '0000000', 0", g_appInfo.dwSys );
	...
	FLSPrintf( szQuery, _countof( szQuery ), "RAINBOWRACE_STR 'S2', '%02d', %d, '0000000', 0", g_appInfo.dwSys, m_nTimes );
	if( pQuery->Exec( szQuery ) == FALSE )
	{ FLERROR_LOG( PROGRAM_NAME, _T( "%s" ), szQuery ); return; }
	...
	if( vec_dwNowPlayerId.empty() == true ) 		// 현재 신청자가 없으면 다음 회 진행...
		m_nTimes++;
	...
	{
		FLSPrintf( szQuery, _countof( szQuery ), "RAINBOWRACE_STR 'S3', '%02d', %d, '0000000', 0", g_appInfo.dwSys, m_nTimes );
		if( pQuery->Exec( szQuery ) == FALSE )
		{ FLERROR_LOG( PROGRAM_NAME, _T( "%s" ), szQuery ); return; }
...
void CRainbowRaceDBCtrl::Application( DWORD dwPlayerId )
{
	...
	// 등록한 신청자를 DB에 저장한다.
	FLSPrintf( szQuery, _countof( szQuery ), "RAINBOWRACE_STR 'I1', '%02d', %d, '%07d', 0", g_appInfo.dwSys, m_nTimes, dwPlayerId );
	if( pQuery->Exec( szQuery ) == FALSE )
	{ FLERROR_LOG( PROGRAM_NAME, _T( "%s" ), szQuery ); return; }
}
...
void CRainbowRaceDBCtrl::FailedUser( DWORD dwPlayerId )
{
	...
	FLSPrintf( szQuery, _countof( szQuery ), "RAINBOWRACE_STR 'U3', '%02d', %d, '%07d', 0", g_appInfo.dwSys, m_nTimes, dwPlayerId );
	if( pQuery->Exec( szQuery ) == FALSE )
	{ FLERROR_LOG( PROGRAM_NAME, _T( "%s" ), szQuery ); return; }
}
...
void CRainbowRaceDBCtrl::UpdateRanking( DWORD dwPlayerId, int nRanking )
{
	...
	// 완주한 유저를 랭킹 예약한다.
	FLSPrintf( szQuery, _countof( szQuery ), "RAINBOWRACE_STR 'U1', '%02d', %d, '%07d', %d", g_appInfo.dwSys, m_nTimes, dwPlayerId, nRanking );
	if( pQuery->Exec( szQuery ) == FALSE )
	{ FLERROR_LOG( PROGRAM_NAME, _T( "%s" ), szQuery ); return; }
}
...
void CRainbowRaceDBCtrl::UpdateWinnersLosers()
{
	...
	// 랭킹예약자와 완주실패자를 저장한다.
	FLSPrintf( szQuery, _countof( szQuery ), "RAINBOWRACE_STR 'U2', '%02d', %d, '0000000', 0", g_appInfo.dwSys, m_nTimes );
	if( pQuery->Exec( szQuery ) == FALSE )
	{ FLERROR_LOG( PROGRAM_NAME, _T( "%s" ), szQuery ); return; }
}
...
CRainbowRaceDBMng::CRainbowRaceDBMng(void)
{
	if( !m_RRDbCtrl.CreateDbHandler() )
		FLERROR_LOG( PROGRAM_NAME, _T( "m_RRDbCtrl.CreateDbHandler()" ) );
}
