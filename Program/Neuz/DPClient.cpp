...
#endif
#ifdef __PROFESSION
			case SNAPSHOTTYPE_PROFESSION:			OnProfession(ar); break;
			case SNAPSHOTTYPE_PROFESSION_EXP:		OnProfessionExp(ar); break;
			case SNAPSHOTTYPE_PROFESSION_LEVEL:		OnProfessionLevel(ar); break;
#endif // __PROFESSION
			default:
				{
...
void CDPClient::OnGCPlayerPoint( CAr & ar )
{
	...
	if( g_WndMng.m_pWndGuildCombatRanking )
	{
		...
		vector<PDVer>	vecPlayer;		//sun: 11, 캐릭터 정보 통합
...
//sun: 11, 메신저창 개선
	...
	if( pGuild )
	{
		vector<PDVer>	aPDVer;		//sun: 11, 캐릭터 정보 통합
...
void CDPClient::OnCooperativeContributions_Info( CAr & ar )
{
	...
	//보상 정보
	typedef				vector< DWORD >			RewardItemVec;
	typedef				vector< DWORD >			RewardTextIDVec;
...
#ifdef __PROFESSION
void CDPClient::OnProfession(CAr& ar)
{
	int nProfessionLevel, nMaxProfessionExp, nSize;
	ar >> nProfessionLevel >> nMaxProfessionExp >> nSize;

	g_pPlayer->m_nProfessionLevel = nProfessionLevel;

	prj.m_profession.nMaxProfessionExp = nMaxProfessionExp;
	prj.m_profession.vCraft.clear();

	for (int i = 0; i < nSize; i++)
	{
		ProfessionCraft temp;
		ar >> temp.nCategorie >> temp.nLevel >> temp.nExp >> temp.dwRewardId >> temp.nRewardNum >> temp.nPercent >> temp.nPenya;

		for (int j = 0; j < 3; j++)
			ar >> temp.dwNeededItemId[j] >> temp.nNeededNum[j];

		prj.m_profession.vCraft.push_back(temp);
	}
}

void CDPClient::OnProfessionExp(CAr& ar)
{
	int nProfessionExp;
	ar >> nProfessionExp;

	g_pPlayer->m_nProfessionExp = nProfessionExp;
}

void CDPClient::SendProfessionCraft(int nSelect, OBJID objId, OBJID objId2, OBJID objId3)
{
	BEFORESENDSOLE(ar, PACKETTYPE_PROFESSION_CRAFT, DPID_UNKNOWN);
	ar << nSelect << objId << objId2 << objId3;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnProfessionLevel(CAr& ar)
{
	int nProfessionLevel, nMaxProfessionExp;
	ar >> nProfessionLevel >> nMaxProfessionExp;

	g_pPlayer->m_nProfessionLevel = nProfessionLevel;
	prj.m_profession.nMaxProfessionExp = nMaxProfessionExp;
	g_pPlayer->m_nProfessionExp = 0;
}
#endif // __PROFESSION
