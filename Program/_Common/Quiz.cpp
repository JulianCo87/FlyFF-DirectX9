...
void CQuiz::LoadScript()
{
	...
	const TCHAR szFileName[]	= _T( "QuizEvent.lua" );
...
void CQuiz::OpenQuizEvent()
{
	...
	if( IsAutoMode() )
	{
		if( static_cast< u_long >( m_nChannel ) == g_uIdofMulti )
			m_bRun = TRUE;
	}
	else
		m_bRun = TRUE;
...
void CQuiz::SetNPC()
{
	...
	if( pWorld )
	{
		...
		if( !pMover )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "pMover is NULL - %d, %s" ), m_sNPC.dwNPCId, m_sNPC.strCharKey.c_str() );
			return;
		}
		FLStrcpy( pMover->m_szCharacterKey, _countof( pMover->m_szCharacterKey ), m_sNPC.strCharKey.c_str() );
		...
		if( pWorld->ADDOBJ( pMover, FALSE, nDefaultLayer ) == FALSE )
		{
			SAFE_DELETE( pMover );
		}
	}
...
int CQuiz::EntranceQuizEvent( FLWSUser* pUser )
{
	if( !IsValidObj( pUser ) )
		return 0;
...
int CQuiz::TeleportToQuizEvent( FLWSUser* pUser, int nZone )
{
	if( !IsValidObj( pUser ) )
		return TID_GAME_QUIZ_DO_NOT_TELEPORT;
	...
	case QE_OPEN:
		{
			if( dwTick > GetNextTime() )
			{
				...
				{
					FLERROR_LOG( PROGRAM_NAME, _T( "Quiz List Load Failed!" ) );
					CloseQuizEvent();
					return;
				}
				...
				if( pWorld )
				{
					for( DWORD i = 0; i < pWorld->m_dwObjNum; ++i )
					{
						...
						{
							static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizEventMessage( prj.GetText( TID_GAME_QUIZ_WAIT_QUESTION ) );
							bAble = TRUE;
							++nUser;
						}
					}
		...
		if( dwTick > GetNextTime() )
		{
			...
			if( pWorld )
			{
				for( DWORD i = 0; i < pWorld->m_dwObjNum; ++i )
				{
					if( IsInQuizEventPlayer( pWorld, nDefaultLayer, i ) )
					{
						static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizEventMessage( prj.GetText( TID_GAME_QUIZ_QUESTION ), QE_QUESTION );
						static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizQuestion( GetType(), it->strQuestion, m_dwQuestionTime / SEC(1) );
					}
				}
			}
...
/*			// 유저 이동 불가 - object
			...
			g_xWSUserManager->AddWorldCreateSfxObj( XI_INDEX( 1917, XI_QUIZCORRECTANSWER ), vPos.x, vPos.y, vPos.z, FALSE, WI_WORLD_QUIZ );
			...
			if( pWorld )
			{
				for( DWORD i = 0; i < pWorld->m_dwObjNum; ++i )
				{
					if( IsInQuizEventPlayer( pWorld, nDefaultLayer, i ) )
					{
						static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizEventMessage( str_Message, QE_CORRECT_ANSWER );
						static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizEventMessage( strMessage );
					}
				}
			}
			...
			if( m_vecQuizList.size() > REMAIN_QUIZ )
			{
				...
				if( pWorld )
				{
					for( DWORD i = 0; i < pWorld->m_dwObjNum; ++i )
					{
						if( IsInQuizEventPlayer( pWorld, nDefaultLayer, i ) )
						{
							static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizEventMessage( prj.GetText( TID_GAME_QUIZ_WATCHINGZONE_OPEN ), QE_WATCHINGZONE_OPEN );
							static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizEventMessage( prj.GetText( TID_GAME_QUIZ_TELEPORT_QUIZZONE ) );
						}
					}
				}
		...
		if( dwTick > GetNextTime() )
		{
			...
			if( pWorld )
			{
				for( DWORD i = 0; i < pWorld->m_dwObjNum; ++i )
				{
					...
					{
						static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizEventMessage( prj.GetText( TID_GAME_QUIZ_WAIT_QUESTION ) );
						//if( static_cast<FLWSUser*>( pWorld->m_apObject[i] )->IsAuthHigher( AUTH_GAMEMASTER ) == FALSE )
	...
	if( pWorld )
	{
		for( DWORD i = 0; i < pWorld->m_dwObjNum; ++i )
		{
			if( IsInQuizEventPlayer( pWorld, nDefaultLayer, i ) )
			{
				if( m_bSelectLog )
				{
					int nSelect = GetUserSelectExample( static_cast<FLWSUser*>( pWorld->m_apObject[i] ) );
					if( nSelect )
						g_dpDBClient.SendQuizEventSelect( static_cast<FLWSUser*>( pWorld->m_apObject[i] )->m_idPlayer, g_uIdofMulti, nQuizId, nSelect, nCorrect );
				}
				static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizEventMessage( prj.GetText( TID_GAME_QUIZ_DROP_OUT ), QE_DROP_OUT );
				if( PtInCorrectZoneRect( static_cast<FLWSUser*>( pWorld->m_apObject[i] ), nCorrect ) )
				{
					++nCorrectUser;
					// 091014 - mirchang : 시스템 부하 발생 가능성이 있어 보류
					//if( nItemId > 0 )	// Item Id가 있을시 정답자에게 해당 아이템 지급
					//	g_dpDBClient.SendPostPrizeItem( static_cast<FLWSUser*>( pWorld->m_apObject[i] )->m_idPlayer, nItemId, nItemNum );
				}
			}
		}
	...
	else
	{
		...
		if( pWorld )
		{
			for( DWORD i = 0; i < pWorld->m_dwObjNum; ++i )
			{
				if( IsInQuizEventPlayer( pWorld, nDefaultLayer, i ) )
				{
					static_cast<FLWSUser*>( pWorld->m_apObject[i] )->AddQuizEventMessage( strMessage, QE_CLOSE_WAIT );
					// 스크립트에 정해진 아이템 지급
					if( PtInCorrectZoneRect( static_cast<FLWSUser*>( pWorld->m_apObject[i] ), nCorrect ) )
						g_dpDBClient.SendPostPrizeItem( static_cast<FLWSUser*>( pWorld->m_apObject[i] )->m_idPlayer, m_dwPrizeItemId, m_nPrizeItemNum );
				}
			}
...
BOOL CQuiz::PtInCorrectZoneRect( FLWSUser* pUser, int nCorrect )
{
...
int CQuiz::GetUserSelectExample( FLWSUser* pUser )
{
	D3DXVECTOR3 vPos = pUser->GetPos();
	POINT pt = { (int)vPos.x, (int)vPos.z };
...
void CQuiz::GoOut( FLWSUser* pUser )
{
//	if( !pUser->IsAuthHigher( AUTH_GAMEMASTER ) )
...
void CQuiz::CloseQuizEvent()
{
	...
	if( pWorld )
	{
		...
		{
			if( IsInQuizEventPlayer( pWorld, nDefaultLayer, i ) )
				GoOut( static_cast<FLWSUser*>( pWorld->m_apObject[i] ) );
		}
	}
}
#endif // __WORLDSERVER
...
{
	memset( &m_sQuizData, 0, sizeof( m_sQuizData ) );
	...
	else if( nType == TYPE_4C )
	{
		...
		m_sQuizData.QuizEventRect = CRect( 1728, 1824, 1792, 1888 );
	}
}
