...
{
	const TCHAR szFileName[]	= _T( "SecretRoom.lua" );
...
// 입찰 가능한 길드 인가?
BOOL CSecretRoomMng::IsPossibleGuild( FLWSUser* pUser )
{
...
// 길드 마스터 인가?
BOOL CSecretRoomMng::IsGuildMaster( FLWSUser* pUser )
{
	CGuild* pGuild = pUser->GetGuild();
...
// 다른 대륙에 입찰한 길드인가?
BOOL CSecretRoomMng::IsOtherTenderGuild( FLWSUser* pUser )
{
	map<BYTE, CSecretRoomContinent*>::iterator it = m_mapSecretRoomContinent.begin();
	for( ; it!=m_mapSecretRoomContinent.end(); it++ )
	{
...
void CSecretRoomMng::SendNowState( FLWSUser* pUser, int nState, DWORD dwRemainTime )
{
		...
		if( pSRCont )
		{
			for( DWORD i=0; i<pSRCont->m_vecSecretRoomTender.size(); i++ )
			{
				for( DWORD j=0; j<pSRCont->m_vecSecretRoomTender[i].vecLineUpMember.size(); j++ )
				{
					...
					FLWSUser* pUserTemp = (FLWSUser*)prj.GetUserByID( pSRCont->m_vecSecretRoomTender[i].vecLineUpMember[j] );
					if( IsValidObj( pUserTemp ) )
					{
						if( nState )
...
void CSecretRoomMng::GetAllInfo( FLWSUser* pUser )
{
...
void CSecretRoomMng::SetTenderOpenWnd( FLWSUser* pUser )
{
	if( m_nState != SRMNG_CLOSE )
	{
...
// 입찰 신청
void CSecretRoomMng::SetTender( FLWSUser* pUser, int nPenya )
{
	if( m_nState != SRMNG_CLOSE )
	{
		pUser->AddDefinedText( TID_GAME_SECRETROOM_NOTENDER );
...
// 입찰 취소
void CSecretRoomMng::SetTenderCancelReturn( FLWSUser* pUser )
{
	if( m_nState != SRMNG_CLOSE )
	{
...
void CSecretRoomMng::SetLineUpOpenWnd( FLWSUser* pUser )
{
	if( m_nState != SRMNG_LINEUP )
	{
...
// 참가자 구성 신청
void CSecretRoomMng::SetLineUp( FLWSUser* pUser, vector<DWORD> vecLineUpMemeber )
{
	if( m_nState != SRMNG_LINEUP )
	{
		pUser->AddDefinedText( TID_GAME_SECRETROOM_NOLINEUP );
...
// 입찰 현황 정보
void CSecretRoomMng::GetTenderView( FLWSUser* pUser )
{
	if( m_nState != SRMNG_CLOSE )
	{
...
void CSecretRoomMng::SetTeleportSecretRoom( FLWSUser* pUser )
{
...
void CSecretRoomMng::SetFailGuild( FLWSUser* pUser )
{
	if( m_nState != SRMNG_WAR )
		return;
...
D3DXVECTOR3 CSecretRoomMng::GetRevivalPos( FLWSUser* pUser )
{
	if( !IsValidObj( pUser ) )
...
BOOL CSecretRoomContinent::IsLineUpMember( FLWSUser* pUser )
{
...
void CSecretRoomContinent::SetContTender( FLWSUser* pUser, int nPenya )
{
	...
	FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "SECRETROOM" );
...
//			if( pUser->GetGold() < (nPenya - m_vecSecretRoomTender[nIndex].nPenya) )
			...
			else
			{
				...
				FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "SECRETROOM_ADD" );
				aLogItem.Gold2 = pUser->GetGold();
...
void CSecretRoomContinent::SetContTenderCancelReturn( FLWSUser* pUser )
{
	int nIndex = GetTenderGuild( pUser->m_idGuild );
	if( nIndex == NULL_ID )
	{
...
//	if( (pUser->GetGold() + nPenya) < 0 )
	...
	FLStrcpy( aLogItem.Action, _countof( aLogItem.Action ), "h" );
	FLStrcpy( aLogItem.SendName, _countof( aLogItem.SendName ), pUser->GetName() );
	FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "SECRETROOM_CANCEL" );
	aLogItem.WorldId = pUser->GetWorld()->GetID();
	//aLogItem.ItemName = "SEED";
	FLSPrintf( aLogItem.kLogItem.szItemName, _countof( aLogItem.kLogItem.szItemName ), "%d", ITEM_INDEX( 12, II_GOLD_SEED1 ) );
	aLogItem.Gold = pUser->GetGold();
...
void CSecretRoomContinent::SetContLineUp( FLWSUser* pUser, vector<DWORD> vecLineUpMember )
{
	int nIndex = GetTenderGuild( pUser->m_idGuild );
	if( nIndex == NULL_ID )
		return;
	...
	{
		FLWSUser* pUserTemp = (FLWSUser*)prj.GetUserByID( vecLineUpMember[i] );
	...
	for( int nCount=1; it!=m_vecSecretRoomTender.end(); nCount++ )
	{
		...
		if( nCount > CSecretRoomMng::GetInstance()->m_nMaxGuildNum )
		{
			...
			if( pGuild )
			{
				...
				char szTitle[256] = {0,};
				char szSub[1024] = {0,};
				FLSPrintf( szTitle, _countof( szTitle ), prj.GetText( TID_GAME_SECRETROOM_RETURNPENYA_POST_TITLE ) );
				FLSPrintf( szSub, _countof( szSub ), prj.GetText( TID_GAME_SECRETROOM_RETURNPENYA_POST_SUBSTANCE ) );
...
void CSecretRoomContinent::SetTeleportSecretRoomCont( FLWSUser* pUser )
{
	int nIndex = GetTenderGuild( pUser->m_idGuild );
	if( nIndex == NULL_ID )
		return;
...
void CSecretRoomContinent::SetTeleportNormalRoom( FLWSUser* pUser, int nIndex )
{
...
// 정해진 좌표에서 특정 범위안에 랜덤하게 위치한다.
void CSecretRoomContinent::SetTeleportBossRoom( FLWSUser* pUser, int nIndex )
{
	int nRandx = xRandom(4) - 2;
	int nRandz = xRandom(4) - 2;
				...
				case MONSTER_MIDBOSS :
					{
						...
						{
							FLWSUser* pUserTemp = (FLWSUser*)prj.GetUserByID( m_vecSecretRoomTender[i].vecLineUpMember[j] );
...
void CSecretRoomContinent::UpdateInfoToMember( FLWSUser* pUser, BYTE nType, int nIndex )
{
	if( IsValidObj( pUser ) )
...
void CSecretRoomContinent::SetContFailGuild( DWORD dwGuildId )
{
	...
	for( DWORD i=0; i<m_vecSecretRoomTender[nIndex].vecLineUpMember.size(); i++ )
	{
		FLWSUser* pUserTemp = (FLWSUser*)prj.GetUserByID( m_vecSecretRoomTender[nIndex].vecLineUpMember[i] );
		if( IsValidObj( pUserTemp ) )
		{
