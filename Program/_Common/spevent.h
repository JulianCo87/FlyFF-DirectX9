...
public:
	typedef struct _T_CALLTHEROLL_ITEM
	{
		DWORD	dwItemId;
		int		nItemNum;
		int		nBinds;
		_T_CALLTHEROLL_ITEM( DWORD dwII, int nIN, int nB )
			: dwItemId( dwII ), nItemNum( nIN ), nBinds( nB ) {}
	} T_CALLTHEROLL_ITEM, *PT_CALLTHEROLL_ITEM;


private:
	map< int, vector<T_CALLTHEROLL_ITEM> >	m_mapCallTheRollItem;

	map<int, int> m_mapEventLimitLevel;
...
//	op
	...
	bool	AddCallTheRollItem( int nCallTheRollCount, DWORD dwItemId, int nItemNum, int nBinds );

	bool	GetCallTheRollItem( int nCallTheRollCount, vector<T_CALLTHEROLL_ITEM> & vecCallTheRollItem );

	int		GetEventLimitLevel( int nEventId );
