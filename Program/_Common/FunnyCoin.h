...
public:
	...
	BOOL	DoUseFunnyCoin( FLWSUser* pUser, FLItemElem* pItemElem );		// 퍼니코인을 사용한다.
	void	OnFunnyCoinAckUse( CAr & ar );	// 퍼니코인 사용에 대한 응답을 TranServer로 부터 받았다.
		
private:
	BOOL	IsUsable( FLWSUser* pUser );		// 퍼니코인을 상용할 수 있는 Player인가?
...
private:
	int InsertFunnyCoin( DWORD dwPlayerId, DWORD dwItemId, SERIALNUMBER iSerialNumber );	// 사용한 퍼니코인을 DB에 저장한다.
};
