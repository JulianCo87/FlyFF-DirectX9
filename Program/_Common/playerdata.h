...
public:
	...
	void	ReplacePlayerString( u_long idPlayer, char* pszPlayer, char* pszOld, size_t cchOld );
	u_long	GetPlayerId( char* pszPlayer );
	int		size( void )	{	return m_mapPlayerData.size();	}

	map<string, u_long> GetContainer_PlayerStringToId() { return m_mapPlayerStringToId; }
