...
//				FLTRACE_LOG( PROGRAM_NAME, _T( "GC REDCHIP - %s, %d위, %s : %d개" ), pUsertmp->GetName(), i+1, itemElem.GetProp()->szName, itemElem.m_nItemNum );
//			}
//		}
//	}
...
//sun: 10, __S_BUG_GC
	...
	// 레벨 & 경험치 검사
//sun: 10, __S_BUG_GC	// 동점자 처리 버그 수정 chipi 080218
...
void CGuildCombat::SetGuildCombatCloseWait( BOOL bGM )
{
	...
	else
	{
		...
		GuildCombatResultRanking();		//sun: 11, 길드대전 칩보상 및 칩을 통한 상점 이용
	}	
}
	...
	while( s.tok != FINISHED )
	{
		...
		if( s.Token == _T( "AUTO_OPEN_IDC" ) )
		{
			...
			while( *s.token != '}' )
			{
				if( nAutoOpenCnt >= 7 )
				{
					FLERROR_LOG( PROGRAM_NAME, _T( "자동 시작 설정 갯수가 넘음!!" ) );
					return FALSE;
				}
