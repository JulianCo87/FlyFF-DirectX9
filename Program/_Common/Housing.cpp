...
#ifdef __DBSERVER
#include "../DatabaseServer/HousingDBCtrl.h"
#endif // __DBSERVER
...
CHousing::~CHousing(void)
{
	...
	for( DWORD i=0; i<m_vecIdVisitAllow.size(); i++ )
		CHousingMng::GetInstance()->SetRemoveVisitable( m_dwMasterId, m_vecIdVisitAllow.at( i ) );
#endif // __WORLDSERVER
...
void CHousing::Serialize( CAr & ar )
{
	if( ar.IsStoring() )
	{
		...
		for( DWORD i=0; i<m_vecHousingInfo.size(); i++ )
			m_vecHousingInfo.at( i ).Serialize( ar );
		ar << m_vecIdVisitAllow.size(); // 방문 허용 목록...
		for( DWORD i=0; i<m_vecIdVisitAllow.size(); i++ )
			ar << m_vecIdVisitAllow.at( i );
	}
	else
	{
		m_vecHousingInfo.clear();
...
#ifdef __WORLDSERVER
		FLWSUser* pUser = static_cast<FLWSUser*>( prj.GetUserByID( m_dwMasterId ) );
#endif // __WORLDSERVER
		m_vecIdVisitAllow.clear();
		ar >> nSize;
		for( int i=0; i<nSize; i++ )
...
int CHousing::GetIndexFromList( DWORD dwItemId )
{
	...
	{
		if( dwItemId == m_vecHousingInfo.at( i ).dwItemId )
			return i;
	}
...
BOOL CHousing::RemoveFurnitureList( DWORD dwItemId )
{
	...
	{
#ifdef __WORLDSERVER
		if( m_vecHousingInfo.at( nIndex ).bSetup )
...
void CHousing::SetupFurniture( HOUSINGINFO housingInfo )
{
	...
	if( nIndex != NULL_ID )
	{
		m_vecHousingInfo.at( nIndex ).bSetup = housingInfo.bSetup;
		if( housingInfo.bSetup )
		{
			m_vecHousingInfo.at( nIndex ).vPos = housingInfo.vPos;
			m_vecHousingInfo.at( nIndex ).fAngle = housingInfo.fAngle;
		}
#ifdef __WORLDSERVER
		Setting( FALSE );
		FLWSUser* pUser = static_cast<FLWSUser*>( prj.GetUserByID( m_dwMasterId ) );
		if( IsValidObj( pUser ) )
		{
			pUser->AddHousingSetupFurniture( &m_vecHousingInfo.at( nIndex ) );
			if( m_vecHousingInfo.at( nIndex ).bSetup )
...
void CHousing::SetVisitAllow( DWORD dwTargetId, BOOL bAllow )
{
	...
	else
	{
		...
		pUser = static_cast<FLWSUser*>( prj.GetUserByID( dwTargetId ) );
		if( IsValidObj( pUser ) && pUser->GetWorld()
			&& pUser->GetWorld()->GetID() == WI_WORLD_MINIROOM && static_cast< DWORD >( pUser->GetLayer() ) == m_dwMasterId )
...
#ifdef __WORLDSERVER
BOOL CHousing::IsListUpAble( FLWSUser* pUser, DWORD dwItemId )
{
	if( m_bSetting )
		return FALSE;
	...
	for( DWORD i=0; i<m_vecHousingInfo.size(); i++ )
	{
		if( m_vecHousingInfo.at( i ).dwItemId == dwItemId )
		{
...
BOOL CHousing::IsSetupAble( FLWSUser* pUser, HOUSINGINFO housingInfo )
{
	if( m_bSetting )
		return FALSE;
	...
	if( m_vecHousingInfo.at( nIndex ).bSetup == housingInfo.bSetup )
		return FALSE;
	
	PT_ITEM_SPEC pItemPropReq = g_xSpecManager->GetSpecItem( housingInfo.dwItemId );
	if( !pItemPropReq )
		return FALSE;
	...
	for( DWORD i=0; i<m_vecHousingInfo.size(); i++ )
	{
		if( housingInfo.bSetup && m_vecHousingInfo.at( i ).bSetup )
		{
			PT_ITEM_SPEC pItemProp = g_xSpecManager->GetSpecItem( m_vecHousingInfo.at( i ).dwItemId );
...
BOOL CHousing::IsAllowVisit( DWORD dwPlayerId )
{
	...
	for( DWORD i=0; i<m_vecIdVisitAllow.size(); i++ )
	{
		if( m_vecIdVisitAllow.at( i ) == dwPlayerId )
			return TRUE;
	}
...
{
	PT_ITEM_SPEC pItemProp = g_xSpecManager->GetSpecItem( m_vecHousingInfo.at( nIndex ).dwItemId );
	CWorld* pWorld = g_WorldMng.GetWorld( WI_WORLD_MINIROOM );
	...
	if( pWorld && pItemProp )
	{
		...
		if( pWorld->m_linkMap.GetLinkMap( static_cast<int>( m_dwMasterId ) ) )
		{
			...
			{
				g_xWSUserManager->AddHousingPaperingInfo( pItemProp->dwID, TRUE, m_dwMasterId );
			}
			else
			{
				...
				pCtrl->SetPos( m_vecHousingInfo.at( nIndex ).vPos );
				pCtrl->SetAngle( m_vecHousingInfo.at( nIndex ).fAngle );
				pCtrl->AddItToGlobalId();
				m_vecHousingInfo.at( nIndex ).objId = pCtrl->GetId();
		...
		SetBuff( m_vecHousingInfo.at( nIndex ).dwItemId, TRUE );
	}
	else
		return FALSE;
...
void CHousing::RemoveFurnitureControl( int nIndex )
{
	...
	if( pWorld && pItemProp )
	{
		if( pWorld->m_linkMap.GetLinkMap( static_cast<int>( m_dwMasterId ) ) )
		{
			if( pItemProp->dwItemKind2 == IK2_PAPERING )	// 벽지, 장판(컨트롤을 생성하지 않고 정보를 클라에 전달)
			{
				g_xWSUserManager->AddHousingPaperingInfo( pItemProp->dwID, FALSE, m_dwMasterId );
			}
			else
			{
				CCtrl* pCtrl = prj.GetCtrl( m_vecHousingInfo.at( nIndex ).objId );
				if( pCtrl )
				{
					pCtrl->Delete();
					m_vecHousingInfo.at( nIndex ).objId = NULL_ID;
				}
			}
		}
		
		SetBuff( m_vecHousingInfo.at( nIndex ).dwItemId, FALSE );
	}
...
void CHousing::AddAllFurnitureControl()
{
	for( DWORD i=0; i<m_vecHousingInfo.size(); i++ )
	{
		if( m_vecHousingInfo.at( i ).bSetup )
...
void CHousing::SetBuff( DWORD dwItemId, BOOL bSet )
{
	...
	if( IsValidObj( pUser ) && pItemProp )
	{
		if( bSet )
		{
			g_xApplyItemEffect->ApplyEffect( pUser, pUser, pItemProp );
		}
		else
		{
...
vector<DWORD> CHousing::GetAllPaperingInfo()
{
	...
	{
		PT_ITEM_SPEC pItemProp = g_xSpecManager->GetSpecItem( m_vecHousingInfo.at( i ).dwItemId );
		if( pItemProp && m_vecHousingInfo.at( i ).bSetup && pItemProp->dwItemKind2 == IK2_PAPERING )
			vecTemp.push_back( m_vecHousingInfo.at( i ).dwItemId );
	}
...
void CHousing::ProcessExpiredFurniture( time_t time, BOOL bGMRemove )
{
	...
	for( DWORD i=0; i<m_vecHousingInfo.size(); i++ )
	{
		if( time >= m_vecHousingInfo.at( i ).tKeepTime || bGMRemove )
			vecTemp.push_back( m_vecHousingInfo.at( i ) );
	}
...
BOOL CHousingMng::ReqSetFurnitureList( FLWSUser* pUser, DWORD dwItemId )
{
...
BOOL CHousingMng::ReqSetupFurniture( FLWSUser* pUser, HOUSINGINFO housingInfo )
{
	CHousing* pHousing = GetHousing( pUser->m_idPlayer );
...
BOOL CHousingMng::ReqSetAllowVisit( FLWSUser* pUser, DWORD dwPlayerId, BOOL bAllow )
{
	FLASSERT( pUser );
	CHousing* pHousing = GetHousing( pUser->m_idPlayer );
...
void CHousingMng::ReqGMFunrnitureListAll( FLWSUser* pUser )
{
	CHousing* pHousing = GetHousing( pUser->m_idPlayer );
	if( pHousing )
	{
...
BOOL CHousingMng::DestroyHousing( DWORD dwPlayerId )
{
	...
	FLERROR_LOG( PROGRAM_NAME, _T( "Housing not found : %d" ), dwPlayerId );
	return FALSE;
}

void CHousingMng::SetVisitRoom( FLWSUser* pUser, DWORD dwPlayerId )
{
	CWorld* pWorld = g_WorldMng.GetWorld( WI_WORLD_MINIROOM );
	if( pWorld == NULL )
	{
...
void CHousingMng::OnReqVisitableList( FLWSUser* pUser )
{
...
void CHousingMng::GoOut( FLWSUser* pUser )
{
	if( IsValidObj( pUser ) == TRUE )
	{
		if( pUser->m_idMarkingWorld == WI_WORLD_MADRIGAL && pUser->m_vMarkingPos != D3DXVECTOR3( 0, 0, 0 ) )
		{
...
CHousing* CHousingMng::CreateHousing( DWORD dwPlayerId )
{
	...
	if( kItr == m_mapHousing.end() )
	{
		...
		if( bResult == false )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "Housing Create Failed!! PlayerID:[%07d]" ), dwPlayerId );
...
CGuildDeployManager::~CGuildDeployManager(void)
{
	if( m_pTargetObj )
	{
		SAFE_DELETE( m_pTargetObj->m_pModel );
		m_pTargetObj->Delete();
	}
...
BOOL	CGuildDeployManager::LoadToDeploy(int nItemId, const HOUSING_ITEM& housingInfo)
{
	...
	if( !m_pWallObj )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "Cannot find housing object %s" ), strHouseObjName.c_str() );
		AfxMessageBox( "하우징 오브젝트가 없습니다, 중형길드하우스라면 obj_miniwall11.o3d가 없습니다" );
		return FALSE;
	}
...
//	SAFE_DELETE( m_pClonedModel );
 	...
 	prj.m_modelMng.MakeModelName( szFileName, _countof( szFileName ), OT_CTRL, nItemId );
...
BOOL	CGuildDeployManager::ChangeWallTex(TCHAR* pTexName)
{
	...
	if( !m_pOriginWall )
	{
		...
		FLASSERT( m_pOriginWall );
	}

	// 미니룸 벽에 해당하는 특정 오브젝트를 이름으로 찾아서 바꿈
	if( pWorld && m_pOriginWall )
	{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "if( pWorld && m_pOriginWall )" ) );
	}
...
std::string CGuildDeployManager::GetNameHouseObj( )
{
	...
	case WI_GUILDHOUSE_MIDDLE: str = "obj_miniwall11.o3d";	break;
	}
...
std::string CGuildDeployManager::GetNameHouseWallTex( )
{
	...
	case WI_GUILDHOUSE_MIDDLE: str = "Obj_MiniWall11.dds";  break;
	}
...
std::string CGuildDeployManager::GetNameHouseTileTex( )
{
	...
	case WI_GUILDHOUSE_MIDDLE: str = "Miniroom_floor01.dds";	break;
	}
