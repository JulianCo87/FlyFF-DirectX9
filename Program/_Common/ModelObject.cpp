...
int CModelObject::RenderAttachModelElem(LPDIRECT3DDEVICE9 pd3dDevice, const D3DXMATRIX *mWorld)
{
	...
	if( m_mUpdateBone && g_pSkiningVS == NULL )
		FLERROR_LOG( PROGRAM_NAME, _T( "CMyD3DApplication::RestoreDeviceObjects()에 CreateSkiningVS()를 호출하시오. CMyD3DApplication::InvalidateDeviceObjects()에는 DeleteSkiningVS()를 호출하시오. 혹은 *.vsh파일을 읽지 못했다." ) );
	if( m_nLoop == 0 )
		FLERROR_LOG( PROGRAM_NAME, _T( "%s : CModelObject::m_nLoop가 지정되지 않았다." ), m_pMotion->m_szName );
#endif
...
{
	if( m_pBone )
		FLERROR_LOG( PROGRAM_NAME, _T( "이미 본이 로딩되어 있다. 0x%08x" ), (int)m_pBone );
	...
	if( m_pBone == NULL )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s : 찾을 수 없음" ), szFileName );
		return FAIL;
	}
...
#ifdef _DEBUG
	if( m_mUpdateBone )
		FLERROR_LOG( PROGRAM_NAME, _T( "%s 이미 읽었는데 또 읽은것 같다." ), szFileName );
#endif
...
int		CModelObject::LoadMotion( LPCTSTR szFileName )
{
	...
	if( m_pBone == NULL )		
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s : 본을 로딩하지 않고 모션을 읽음" ), szFileName );
		return FAIL;
	}
...
int		CModelObject::LoadElement( LPCTSTR szFileName, int nParts )
{
	...
	if( nParts < 0 )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "nParts 에러 - nParts = %d, %s" ), nParts, szFileName );
		return FAIL;
	}
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s 디바이스실패 %08x" ), szFileName, (int)hr );
	}
#endif
	pObject3D = g_Object3DMng.LoadObject3D( m_pd3dDevice, szFileName );
	if( pObject3D == NULL )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s : 찾을 수 없음" ), MakePath( DIR_MODEL, szFileName ) );
		return FAIL;
	}
	...
	if( pObject3D->IsUseExternBone() )		// 외장본을 사용하는 오브젝이다.
	{
		...
		{
#ifdef __CLIENT
			FLERROR_LOG( PROGRAM_NAME, _T( "%s : bone not loaded." ), szFileName );		// 본을 로딩하지 않고 메쉬를 읽음
#endif // __CLIENT
			return FAIL;
		}
	}
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "튕김 : 읽은 파일명 %s" ), MakePath( DIR_MODEL, szFileName ) );
		return FAIL;
	}
	...
	if( m_nFrameMax > 0 )	// 이미 모션이나 오브젝을 읽어서 maxframe이 있을때 이번에 읽은 오브젝틔 maxframe하고 비교. 틀리면 에러.
	{
		if( pObject3D->GetMaxFrame() )
			...
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "%s의 MaxFrame(%d)과 이전에 읽은 MaxFrame(%d)이 다르다." ), szFileName, pObject3D->GetMaxFrame(), m_nFrameMax );
			}
	}
...
int		CModelObject::Render( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXMATRIX *mWorld )
{
	...
	if( m_mUpdateBone && g_pSkiningVS == NULL )
		FLERROR_LOG( PROGRAM_NAME, _T( "CMyD3DApplication::RestoreDeviceObjects()에 CreateSkiningVS()를 호출하시오. CMyD3DApplication::InvalidateDeviceObjects()에는 DeleteSkiningVS()를 호출하시오. 혹은 *.vsh파일을 읽지 못했다." ) );
	if( m_nLoop == 0 )
		FLERROR_LOG( PROGRAM_NAME, _T( "경고 : %s : CModelObject::m_nLoop가 지정되지 않았다." ), m_pMotion->m_szName );
#endif
...
#ifdef _DEBUG
//	RenderItemEffect_TEST( mWorld );		//해당기능이 개발완료되어 주석처리함, 테스트를 위해 코드는 살려둠
#endif
...
#endif //__SFX_OPT
			{
				nEffect = pElem->m_nEffect & 0x00FFFFFF;	// 하위 24비트, 속성의 종류
				nLevel = (pElem->m_nEffect >> 24);			// 상위 8비트, 일반속성 레벨
...
void CModelObject::MakeSWDForce( int nParts, DWORD dwItemKind3, BOOL bSlow, DWORD dwColor, float fSpeed )
{
	...
	FLERROR_ASSERT_LOG_RETURN_VOID( pElem->m_pObject3D, _T("[모델 오브젝트] 3D 오브젝트 포인터가 유효하지 않습니다.") );
	...
	if( pElem->m_nParentIdx == GetRHandIdx() )
		...
		FLERROR_ASSERT_LOG_RETURN_VOID( pmLocal, _T("[모델 오브젝트] 부모 인덱스를 찾을 수 없습니다.") );
...
//sun: 9차 제련관련 (추가 검광 기능)
	...
	float		fTemp = 0.0f;		//sun:13, 제련 확장(속성, 일반)
...
//sun: 9차 제련관련 (추가 검광 기능)
	...
	//sun: 제련 이팩트 향상 버전
			...
			case PE_ELEC_AL : dwSfx = XI_INDEX( 1843, XI_NAT_ELEC01_ADV_AL ); break;	// 이경우 발생함...향상버전에서는
			...
			case PE_ELEC : dwSfx = XI_INDEX( 1793, XI_NAT_ELEC01_ADV ); break;		// 이경우 발생함...향상버전에서는
...
{
	// define key문자열을 만들어서 value값을 찾아서 리턴
	int nSfx = 0;
...
{
	//속성값과 레벨값을 토대로 보여줘야할 이펙트 레벨을 리턴

	int nEffectLevel = 0;
	if( nAttr == PE_GEN || nAttr == PE_GEN_AL )
	{
		//일반 제련의 경우  max = 10 ( 3, 5, 7, 9, 10 )
		if( nLevel < 3 )
			nEffectLevel = 0;
		else if( nLevel < 5 )
			nEffectLevel = 1;
	...
	{
		//속성 및 바루나 일반 제련의 경우 max = 20 ( 3, 7, 11, 16, 20 )
		if( nLevel < 3 )
			nEffectLevel = 0;
		else if( nLevel < 7 )
			nEffectLevel = 1;
...
{
	//구함수와 신함수 분리
	//구함수의 특징은 nLevel, nLevel2에 해당하는 이펙트를 동시에 모두 생성하므로, 신함수 역시 동일하게 작동하게 한다.
	...
	case CS_VER2:
		...
		//콜렉터도 업글이 되면 이펙트가 붇었나봄, 허나 최신버젼엔 콜렉터 이펙트 없음.
		...
		//2nd
		//속성제련관련, nType은 현재 양손무기합성여부만 지원하므로 속성제련 sfx생성에만 인자 추가함.
		if( nLevel2 > 0 )
...
CPartsWeaponEffect*	CModelObject::CreateParticle_Internal( int nParts, int nAttr, int nLevel, DWORD ik3_kind, int nType, TCHAR* filenameSFX )
{
	...
	if( bOK )
	{
		...
		{
			//속성, 레벨, 아이템 종류에 따라 sfx id를 얻어서 생성
...
void CModelObject::RenderParticle_New( CPartsWeaponEffect* pEffect, int nParts, DWORD ik3_kind, const D3DXMATRIX* pmWorld )
{
	...
	//흠 파츠는 오른손으로 지정되어 있지만 실제 모델은 왼손에 착용한경우에 문제가 있따. 
	//실제 착용이 LHAND임에도 RWEAPON으로 해놓고 내부적으로 왼쪽손에 붙이도록 하드코딩 되어있따. -_-fuck
	if( ik3_kind == IK3_BOW )
	...
	//무기로컬 0,0,0 의 월드좌표
	D3DXVECTOR3 vPos( 0.0f, 0.0f, 0.0f );				//모든 무기의 원점( 0, 0, 0 )에 붙이기로 함
...
void	CModelObject::RenderItemElec_Adv( int nParts, const D3DXMATRIX *pmWorld, int nLevel )
{
	...
	if( nLevel >= 10 )		nLevel = 5;		//sun:13, 제련 확장(속성, 일반)
...
void	CSwordForce::Add( D3DXVECTOR3 v1, D3DXVECTOR3 v2 )
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "SWDForceAdd : 범위 초과 %d" ), m_nMaxVertex );
		return;
	}
...
void	CSwordForce::MakeSpline( void )
{
	...
	for( i = 0; i < nMaxVertex-1; i ++ )
	{
		...
		for( j = 0; j < MAX_SF_SLERP+1; j ++ )
		{
			...
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "SWDForceAdd Spline : 범위 초과 %d" ), m_nMaxSpline );
				break;
			}
			pKey = m_aList;
