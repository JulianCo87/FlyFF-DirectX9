...
//		GuildDeploy()->Init();
	...
	if( GetID() == WI_WORLD_COLOSSEUM )
	{
		...
		//콜로세움 퇴장후 죽어있는경우는 없으므로 부활창도 제거한다.
...
BOOL LoadTextureSet( TextureSet* pSet )
{
	...
	if( string::npos != pSet->_strFileName.find( '.' ) ) //".xxx"
	{
		...
		{
			CString tmp;
			tmp.Format( "LoadTextureSet: pSet->_pTexture is NULL, filename : %s", pSet->_strFileName.c_str() );
			//	100805	CSTRING GETBUFFER
			FLERROR_LOG( PROGRAM_NAME, tmp.GetString() );
			return FALSE;
		}
	}
...
{
	char fullname[ 256 ];
	FLSPrintf( fullname, _countof( fullname ), "%s%s", filename, ".24l" );
...
{	
	...
	//bgm
	m_kCurContinent._iBGM = kInfo._iBGM;
	if( kInfo._iBGM > 0 )
	{
		//트리거와의 규약이 있어야 함
		//m_dwIdMusic = kInfo._iBGM;
		//PlayBGM( m_dwIdMusic );
		//g_WndMng.m_pWndWorld->m_dwIdBgmMusic = m_dwIdMusic;	// 이걸 왜 wndWorld에서 제어하고 자빠졌나..어우
	}
...
void CWorld::MoveOutContinent( const string& worldname )
{
	...
	m_skyBox.SetWeather( m_iOldWorldWeather, g_pSeasonEffect->getEffect() );
	...
	//트리거와의 규약이 있어야 함
	//m_dwIdMusic = 0;
	//g_WndMng.m_pWndWorld->m_dwIdBgmMusic = 0;
}
...
void CWorld::HookUpdateLight( CLight* pLight )
{
	...
	m_dwAddedTime += _GetDeltaTime(); //dwDeltaTime;
...
{
	char fullname[ 256 ];
	FLSPrintf( fullname, _countof( fullname ), "%s%s", filename, ".cnt" );
	...
	do{
		...
		if( script.Token == _T( "Continent" ) )
		{
			...
			else if( script.Token == _T( "END" ) )
			{
				...
				if( kInfo._bUseEnvir )		//환경값을 이용할 경우만 저장 ( 환경값 구역끼리는 겹치면 안된다 )
