...
struct VOTE_INSERTED_INFO
{
	...
	char	szSelections[MAX_VOTE_SELECT][MAX_BYTE_VOTESELECT];		// 선택문장 
};
...
private:
	...
	GUILD_VOTE_SELECT	m_selects[MAX_VOTE_SELECT];				// 선택 
};
...
public:
	...
	DWORD	GetPxpCount(int nLevel) const;
	DWORD	GetPenya(int nLevel) const;
	WORD	GetMaxMemeber(int nLevel) const;
	void	Serialize( CAr & ar );
...
#endif // client
public:
	MEMPOOLER_DECLARE( CGuildMember );
};
...
public:
	...
	CItemContainer<FLItemElem>	m_GuildBank;	// 길드 창고
#endif
...
#ifndef __CORESERVER
	FLItemElem*	GetItem( DWORD dwItemId, SERIALNUMBER iSerialNumber );
	int	RemoveItem( DWORD dwId, int nItemNum );
#endif	// __CORESERVER

	static	int	sm_anMaxMemberSize[MAX_GUILD_LEVEL];
	static	int	sm_anMaxMemberLvSize[MAX_GM_LEVEL];
public:
	MEMPOOLER_DECLARE( CGuild );
};
...
public:
	 ...
	 */
	BOOL GetRanking(CQuery* pQuery, LPTSTR p_strQuery, size_t cchQuery)
	{
		m_Lock.Enter();
		...
		{
			FLSPrintf(p_strQuery, cchQuery, "RANKING.RANKING_DBF.dbo.RANKING_STR 'R%d','%02d'", i+1, g_appInfo.dwSys );
			if( FALSE == pQuery->Exec( p_strQuery ) )
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "(%s) 실패" ), p_strQuery );
				m_Lock.Leave();
				return FALSE;
			}
			...
			for ( ; pQuery->Fetch() == TRUE ; ++j )
			{
				...
				pQuery->GetStr( "m_szGuild"		, m_Ranking[i][j].m_szGuild, _countof( m_Ranking[i][j].m_szGuild ) );
				pQuery->GetStr( "m_szName"		, m_Ranking[i][j].m_szName, _countof( m_Ranking[i][j].m_szName ) );
	...
	BOOL RankingDBUpdate(CQuery* pQuery, LPTSTR p_strQuery, size_t cchQuery)
	{
		m_Lock.Enter();

		pQuery->Clear();

		FLSPrintf(p_strQuery, cchQuery, "MAKE_RANKING_STR '%d'", g_appInfo.dwSys );
		if( FALSE == pQuery->Exec( p_strQuery ) )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "(%s) 실패" ), p_strQuery );
			m_Lock.Leave();
