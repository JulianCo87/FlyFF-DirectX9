...
BOOL CSfxBase::Load(void)
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s 찾을 수 없음" ), strFilename );
		return FALSE;
	}
	file.Close();
	return TRUE;
...
#ifdef __CLIENT
MEMPOOLER_IMPLEMENT( CSfxModel, 256 );
#endif	// __CLIENT
CSfxModel::CSfxModel()
{
...
void CSfxModel::SetSfx( DWORD dwIndex )
{
	...
	if( lpModelElem )
	{
		m_pSfxBase = g_SfxMng.GetSfxBase( lpModelElem->getName() );
...
BOOL CSfxModel::Render( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXMATRIX* pmWorld )
{
	if(m_pSfxBase) 
	{
		...
		//gmpbigsun( 20100817 ) : 이펙트랑 포그 혼합금지.
		...
		CSfxMng::m_pd3dDevice->SetRenderState( D3DRS_LIGHTING, TRUE );
		CSfxMng::m_pd3dDevice->SetRenderState(D3DRS_ZWRITEENABLE,TRUE);
		CSfxMng::m_pd3dDevice->SetRenderState( D3DRS_ALPHABLENDENABLE, FALSE);		//켰으면 끄자
	...
	for( int i = 0; i < m_apParticles.GetSize(); i++ ) 
	{
		...
		if( pParticles ) 
		{
			...
			if( pPartParticle->m_bRepeat )
			{
				...
				{
					FLERROR_LOG( PROGRAM_NAME, _T( "nEndFrame < 0" ) );
				}
			}
...
{
	for(int i=0;i<m_apSfxObj.GetSize();i++) {
		SAFE_DELETE_CONST( (CSfxModel*)m_apSfxObj.GetAt(i) );
	}
...
LPDIRECT3DTEXTURE9 CSfxTexture::AddTex(CString str)
{
	...
	if( hr != D3D_OK ) 
	{
		FLTRACE_LOG( PROGRAM_NAME, _T( "CSfxTexture::AddTex에서 텍스춰 없음 %s" ), str );
		return NULL;
	}
...
SfxModelSet::SfxModelSet( const OBJID idMaster, const char* szSfxName, const char* szBoneName, BOOL bLoop, int nState )
{
	...
	FLStrcpy( _szFileName, _countof( _szFileName ), szSfxName );
	FLStrcpy( _szBone, _countof( _szBone ), szBoneName );
	_bLoop = bLoop;
...
BOOL SfxModelSet::Update( )
{
	...
	D3DXMATRIX* pMat = pMaster->GetMatrixWorldPtr();	//D3DXMATRIX* pMat = &pMaster->GetMatrixWorld( ) 이문장은 Release에서 문제를 발생시킨다.
...
int call_mte( lua_State* L )
{
	...
	char szDDS[ 64 ];
	FLStrcpy( szDDS, _countof( szDDS ), sz2ndDDS );
	FLStrcat( szDDS, _countof( szDDS ), ".dds" );
	BOOL bOK = pMover->SetDataMTE( pMover->GetNameO3D(), szDDS );

	if( !bOK )
		FLERROR_LOG( PROGRAM_NAME, _T( "please check the mte texture %s" ), pMover->GetNameO3D() );
...
void run_lua_sfx( int nState, OBJID caller, const char* szMoverName )
{
	...
	char szFunc[ 64 ];
	GetFileTitle( szMoverName, szFunc, _countof( szFunc ) );
	...
	if( 0 != luaL_dofile( g_SfxLua, strFullPath.GetString() ) )
	{
	//	FLERROR_LOG( PROGRAM_NAME, _T( "run_lua_sfx : %s" ), (char*)lua_tostring( g_SfxLua, -1 ) );
		return;
	}
#else
	...
	CResFile file;
	//	100805	CSTRING GETBUFFER
	BOOL bRet = file.Open( strFullPath.GetString(), "rt" );
	if( bRet == FALSE )	
		return;
	...
	{
		//	100805	CSTRING GETBUFFER
		FLERROR_LOG( PROGRAM_NAME, _T( "error!! overflowed size of lua file %s" ), strFullPath.GetString() );
		return;
	}
	...
	if( (a | b) != 0 )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "error run_lua_sfx : %s" ), (char*)lua_tostring( g_SfxLua, -1 ) );
		return;
	}
	...
	if( OBJSTAF_COMBAT == nState )
		FLStrcat( szFunc, _countof( szFunc ), "_battle" );
	else
	if( OBJSTA_NONE == nState ) 
	{
		//생성상태로 간주한다.. 즉 생성되고 아무것도 아닌상태란 걸로 해석하기 바람.
		FLStrcat( szFunc, _countof( szFunc ), "_create" );
	}
	else
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "SFXLUA is not supported the state : %d" ), nState );
		return;
	}
...
//_DEBUG
	if( lua_pcall( g_SfxLua, 1, 0, 0 ) != 0 ) 
	{
		...
		FLERROR_LOG( PROGRAM_NAME, _T( "run_lua_sfx : %s" ), szError );
	}
}
