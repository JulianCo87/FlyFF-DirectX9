...
HRESULT		CreateShadowMask( LPDIRECT3DDEVICE9 pd3dDevice, int nWidth, int nHeight )
{
	...
	if( FAILED(hr) )
		FLERROR_LOG( PROGRAM_NAME, _T( "버텍스 버퍼 생성 실패" ) );
...
CObject3D		*CObject3DMng :: LoadObject3D( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR szFileName )
{
	...
	FLStrcpy( sFile, _countof( sFile ), szFileName );
	FLStrLowercase( sFile, _countof( sFile ) );
...
//	m_nMaxObject = 0;
	...
	m_vForce3.x = m_vForce3.y = m_vForce3.z = 0;	// 9차 제련관련 (추가 검광 기능)
...
{
#ifdef	__WORLDSERVER
	UNREFERENCED_PARAMETER( pObject );		UNREFERENCED_PARAMETER( ppd3d_VB );	UNREFERENCED_PARAMETER( pool );
	return S_OK;
#else
//	pool = D3DPOOL_MANAGED;
	...
	if( ppd3d_VB == NULL )		// ppd3d_VB가 널이면 내부... this->m_pd3d_VB를 써야 한다.
	{
		...
		if( pObject->m_pd3d_VB == NULL )
		{
			...
			if( FAILED(hr) )
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "1 %s Object3D 버텍스 버퍼 생성 실패(%s) : m_nMaxVB=%d, nVertexSize=%d, dwFVF=%d, dwUsage=%d, pool=%d %08x" ),
				...
				{
					FLERROR_LOG( PROGRAM_NAME, _T( "%s CObject3D 디바이스실패 %08x" ), m_szFileName, (int)hr );
				}
				int *p = NULL;
				*p = 1;
				return E_FAIL;
	...
	} else
	{
		if( *ppd3d_VB == NULL )		// 아직 할당 되지 않았음
		{
			...
			if( FAILED(hr) )
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "2 %s Object3D 버텍스 버퍼 생성 실패(%s) : m_nMaxVB=%d, nVertexSize=%d, dwFVF=%d, dwUsage=%d, pool=%d %08x" ),
																		  m_szFileName, DXGetErrorString9(hr), pObject->m_nMaxVB, 
																		  nVertexSize, dwFVF, dwUsage, (int)pool, (int)m_pd3dDevice );
	...
	if( pObject->m_pd3d_IB == NULL )
	{
		...
		if( FAILED(hr) )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "%s Object3D 인덱스 버퍼 생성 실패(%s) : m_nMaxIB=%d" ),
...
int CObject3D::IsAnimateFile( LPCTSTR szFileName )
{
	...
	if( bRet == FALSE )	
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "IsAnimateFile : %s 읽기 실패" ), szFileName );
		return FAIL;
	}
	char c0;
	int d0;
...
int		CObject3D :: LoadObject( LPCTSTR szFileName )
{
	...
	if( bRet == FALSE )	
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s 읽기 실패" ), szFileName );
		return FAIL;
	}
	Init();
	...
	FLStrLowercase( m_szFileName, _countof( m_szFileName ) );
//	strcpy( szName, GetFileName(szFileName) );
	GetFileName( szFileName, szName, _countof( szName ) );
	...
	for( int j = 0; j < cLen; j ++ )
		buff[j] = buff[j] ^ '\xcd';//(char)0xcd;	// 암호화 해제

	if( cLen >= 64 )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s 파일명이 너무 길다 %d" ), szFileName, (int)cLen );
		return 0;
	}
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "잘못된 파일 : %s, %s, %s" ), szFileName, szName, buff );
		return 0;
	}
	...
	if( nVer < VER_MESH )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s의 버전은 %d.  최신버전은 %d" ), szFileName, nVer, VER_MESH );
		resFp.Close();
		return FAIL;
	}
...
//sun: 9차 제련관련 (추가 검광 기능)
	...
	if( pPool == NULL )
		FLERROR_LOG( PROGRAM_NAME, _T( "메모리 할당 실패:( %s ) %d" ), m_szFileName, nPoolSize );
	...
	for( int i = 0; i < nMaxGroup; i ++ )
	{
		...
		if( nDebugSize > nPoolSize )
			FLERROR_LOG( PROGRAM_NAME, _T( "메모리 오버플로우 %d, %d" ), nPoolSize, nDebugSize );
...
//	SetBB( m_vBBVList, m_vBBMin, m_vBBMax );
	...
	if( bNormalObj == TRUE )
	{
		...
		for( int i = 0; i < nMaxGroup; i ++ )
		{
			...
			for( int j = 0; j < m_Group[i].m_nMaxObject; j ++ )
...
/*
	...
	if( fp == NULL )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%를 찾을 수 없다. 혹은 파일을 Check Out했는지 확인할 것!." ), szFileName );
		return FAIL;
	}
	// common header
	mem_set( reserved, 0, 32 );
	nTemp = VER_MESH;
...
//nt				nIdx = 0;
	...
	if( bIsMaterial )
	{
		...
		//mem_set( mMaterialAry, 0, sizeof(mMaterialAry) );
//		for( i = 0; i < 16; i ++ )	mMaterialAry[i] = NULL;
		...
		for( i = 0; i < pObject->m_nMaxMaterial; i ++ )
		{
			...
			if( nLen > sizeof(szBitmap) )		
				FLERROR_LOG( PROGRAM_NAME, _T( "%s 텍스쳐 파일명이 너무길다 : 길이 = %d" ), m_szFileName, nLen );
			file->Read( szBitmap, nLen, 1 );
			FLStrLowercase( szBitmap, _countof( szBitmap ) );		// 소문자로 변환
			
			pObject->m_MaterialAry[i].m_Material = mMaterial;

			if( strlen(szBitmap)+1 > sizeof(pObject->m_MaterialAry[i].strBitMapFileName) )
				FLERROR_LOG( PROGRAM_NAME, _T( "%s의 길이가 너무 길다" ), szBitmap );

			FLStrcpy( pObject->m_MaterialAry[i].strBitMapFileName, _countof( pObject->m_MaterialAry[i].strBitMapFileName ), szBitmap );
		#if	!defined(__WORLDSERVER)
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "매터리얼 갯수가 32개를 넘었다. %s" ), m_szFileName );
		return FAIL;
	}
			...
			if( bIsMaterial )
			{
				if( mMaterialAry[ pObject->m_pMtrlBlk[i].m_nTextureID ] )
				{
				...
				#endif //__YENV_WITHOUT_BUMP
					{
						...
						FLStrcpy( strFileName, _countof( strFileName ), mMaterialAry[ pObject->m_pMtrlBlk[i].m_nTextureID ]->strBitMapFileName );		
					
						::GetFileTitle( strFileName, szTexture, _countof( strFileName ) );
						lstrcat( szTexture, "-n." );
						::GetFileExt( strFileName, szFileExt, _countof( strFileName ) );
...
{
#ifdef __WORLDSERVER
	UNREFERENCED_PARAMETER( szSrc );		UNREFERENCED_PARAMETER( szDest );
	return;
#else
	int		i, j, k;
	GMOBJECT	*pObject;
	...
	FLStrcpy( szBuff, _countof( szBuff ), szSrc );
	FLStrLowercase( szBuff, _countof( szBuff ) );
	...
	for( k = 0; k < MAX_GROUP; k++ )
	{
		for( i = 0; i < m_Group[k].m_nMaxObject; i ++ )
		{
			...
			for( j = 0; j < pObject->m_nMaxMtrlBlk; j ++ )		// 매터리얼 블럭을 돌면서
			{
				...
				FLStrcpy( szBitMapFileName[ j ], _countof( szBitMapFileName[ j ] ), pObject->m_MaterialAry[ nID ].strBitMapFileName );
			}
			for( j = 0; j < pObject->m_nMaxMtrlBlk; j ++ )		// 매터리얼 블럭을 돌면서
			{
				int nID = pObject->m_pMtrlBlk[j].m_nTextureID;
				if( strcmp( szBitMapFileName[j], szBuff ) == 0 )	// szSrc랑 같은 파일명이 있으면
				{
					...
					FLStrcpy( pObject->m_MaterialAry[ nID ].strBitMapFileName, _countof( pObject->m_MaterialAry[ nID ].strBitMapFileName ), szDest );	// 파일명 바꿔놓는다.
				}
			}
		}
	} // LOD_GROUP
...
//	GMOBJECT* pObject = m_Group[0].m_pObject;
	...
	{
		if( pObject->m_Type == GMT_SKIN )	
			FLERROR_LOG( PROGRAM_NAME, _T( "SlideVectorXZ:스킨오브젝트 발견. 에러%s" ), m_szFileName ); 
...
//			bRet = D3DXIntersectTri( v1, v2, v3, &vInvPos, &vInvDir, &fU, &fV, &fDist );
			...
			if( bRet && fDist >= 0.0f )	// 반대방향 면은 검사하지 않음.
			{
				...
				if( fDist < 1.0f )
				{
					...
					if( vN.x == 0 && vN.z == 0 )
						FLERROR_LOG( PROGRAM_NAME, _T( "충돌한 면의 노말이 완전 수직이다" ) );
...
//	for( i = 0; i < nMaxObj; i++ )
	{
		if( pObject->m_Type == GMT_SKIN )	
			FLERROR_LOG( PROGRAM_NAME, _T( "SlideVectorUnder:스킨오브젝트 발견. 에러%s" ), m_szFileName );
...
void CObject3D::FindTouchTriLine( D3DXVECTOR3 **pTriOut, const D3DXVECTOR3 &vPos, const D3DXVECTOR3 &vEnd , const D3DXMATRIX &mWorld, FLOAT *pDist, BOOL bCollObj )
{
	...
	{
		if( pObject->m_Type == GMT_SKIN )	
			FLERROR_LOG( PROGRAM_NAME, _T( "FindTouchTriLine:스킨오브젝트 발견. 에러%s" ), m_szFileName );
...
static D3DXVECTOR3	_vPool[5120];		//바실리스크 
...
//gmpbigsun(20101021): SimpleTriIntersect 대응버젼 ( 너무 느리고, 다 삐꾸다 )
// 대상의 BBOX와 this의 각 삼각형에 대해 충돌검사를 실시한다.
// this 는 충돌메시를 이용해야 하고, 각 삼각형당 3개의 라인을 생성해서 타겟의 BBOX와 충돌검사를 실시한다.
BOOL	CObject3D::TriIntersect_BBOX( D3DXMATRIX mWorld, CObj* pTargetObj, D3DXMATRIX /*mTargetWorld*/)
{
	if( !pTargetObj )
		return FALSE;
	
	D3DXVECTOR3	v1, v2, v3;
	BBOX rBox = pTargetObj->m_OBB;
	int nMaxObj = m_Group[0].m_nMaxObject;
	int	nMax = 0;
	D3DXMATRIX	mTM;
	WORD* pIB = NULL;
	NORMALVERTEX* pVB = NULL;
	...
	for( int i = 0; i < nMaxObj; ++i )
	{
		...
		for( int j = 0; j < nMax; ++j )
		{
		 //로컬좌표계에서 월드좌표로 변환..
			...
			Segment3 kSeg1( v1, v2 );
			Segment3 kSeg2( v2, v3 );
			Segment3 kSeg3( v3, v1 );

			if( IntrSegment3Box3_Test( kSeg1, rBox ) )		// line 1
				return TRUE;
			if( IntrSegment3Box3_Test( kSeg2, rBox ) )		// line 2
				return TRUE;
			if( IntrSegment3Box3_Test( kSeg3, rBox ) )		// line 3
				return TRUE;
		}
...
//			D3DXVECTOR3	*pVList = pObj->m_pVertexList;
			...
			if( nMax > sizeof(_vPool) / sizeof(D3DXVECTOR3) )
				FLERROR_LOG( PROGRAM_NAME, _T( "버텍스버퍼의 개수가 너무 크다 %d" ), nMax );
...
void	CObject3D::LoadTextureEx( int nNumEx, GMOBJECT *pObj, MATERIAL *pmMaterial[16] )
{
	...
	for( i = 0; i < pObj->m_nMaxMaterial; i ++ )
	{
		...
		FLStrcpy( szTexture, _countof( szTexture ), pObj->m_MaterialAry[i].strBitMapFileName );		// mvr_bang.dds
//		strcpy( szTexture, ::GetFileTitle( pObj->m_MaterialAry[i].strBitMapFileName ) );	// mvr_bang
		GetFileTitle( pObj->m_MaterialAry[i].strBitMapFileName, szTexture, _countof( szTexture ) );	// mvr_bang
		FLStrcat( szTexture, _countof( szTexture ), "-et" );		// -et. 를 붙임.  mvr_bang-et.
		FLSPrintf( szNum, _countof( szNum ), "%02d.", nNumEx );	
		FLStrcat( szTexture, _countof( szTexture ), szNum );			// -et01 ~ 07
//		strcat( szTexture, ::GetFileExt( pObj->m_MaterialAry[i].strBitMapFileName ) );		// mvr_bang-et.dds
		GetFileExt( pObj->m_MaterialAry[i].strBitMapFileName, szFileExt, _countof( szFileExt ) );	// mvr_bang-et.dds
		FLStrcat( szTexture, _countof( szTexture ), szFileExt );
...
void	CObject3D::SetTextureEx( GMOBJECT *pObj, int nNumEx )
{
	...
	if( nNumEx >= 8 )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "nNumEx = %d" ), nNumEx );
		return;
	}
...
//	if( bUse )
	if( 1 )
	{
		MATERIAL	*mMaterial[16] = { 0 };
		//mem_set( mMaterial, 0, sizeof(mMaterial) );
...
/*
	FLTRACE_LOG( PROGRAM_NAME, _T( "%s\t%d,%d : TYPE : %d" ), m_szFileName, pObj->m_nMaxVB, pObj->m_nVertexSize, pObj->m_VBType );
*/				
...
//		m_pd3dDevice->SetRenderState( D3DRS_ALPHABLENDENABLE, TRUE );
		...
		g_mReflect._41 += 0.004f;
		g_mReflect._42 += 0.004f;

//		g_mReflect._41 = 1.0f / xRandomF( 10.0f );
//		g_mReflect._42 = 1.0f / xRandomF( 10.0f );;

		if( g_mReflect._41 > 1.0f )
			g_mReflect._41 = 0.0f;

		if( g_mReflect._42 > 1.0f )
			g_mReflect._42 = 0.0f;
...
//	D3DXMATRIX *mUpdate = _mUpdate;
	...
	if( m_nNoEffect == 0 )
	{
		...
		if( g_pSeasonEffect->getSeason() == SEASON_SPRING )
		{
			if( _stricmp( m_szFileName, "obj_macoprtr16.o3d" ) == 0 || _stricmp( m_szFileName, "obj_macoprtr17.o3d" ) == 0 )
			{
...
//	m1 = mUpdate[i] * mWorld;		// mUpdate에 미리 mWorld를 곱한상태로 와도 될것 같다.
	...
	while( nMaxMtrl-- )
	{
		...
		if( pObj->m_bLight )
		{
			if( m_nNoEffect == 0 )
			{
				//gmpbigsun( 20101001 ) : 라이트 오브젝트랑 포그 혼합금지.
...
void	CObject3D::SetShader( const D3DXMATRIX *mWorld )
{
	...
	if( m_bSendVS )	// 뼈대개수가 MAX_VS_BONE이하라서 한번에 다 전송한다.
	{
		...
		if( nMaxBone > MAX_VS_BONE )	
			FLERROR_LOG( PROGRAM_NAME, _T( "뼈대개수가 최대치를 초과했다. %d" ), nMaxBone );
...
//	m_tmCreate = g_tmCurrent;	// 한번이라도 렌더링 되고 있다면 계속 사용하고 있는것으로 봐야 한다,.
	...
	for( i = 0; i < nMax; i ++ )
	{
		...
		if( pObj->m_nVertexSize == 0 || pObj->m_dwFVF == 0 )
			FLERROR_LOG( PROGRAM_NAME, _T( "%s : FVF가 기술되지 않았다. VertexSize=%d, FVF=%d" ), m_szFileName, pObj->m_nVertexSize, pObj->m_dwFVF );

		if( pObj->m_Type == GMT_SKIN )
		{
			if( ppd3d_VB == NULL )
				FLERROR_LOG( PROGRAM_NAME, _T( "%s 스킨오브젝트인데 ppd3d_VB가 없다." ), m_szFileName );
		...
		} else
		{
#ifdef __BS_TEST_MTE
			BOOL bMode_A = FALSE;
			if( gConsole()->_bTestMTE && NULL != strstr( this->m_szFileName, "item_" ) )
				bMode_A = TRUE;
			...
			if( bMode_A )
			{
				//test
				D3DXMATRIX tempMat;
				D3DXMatrixScaling( &tempMat, gConsole()->_fTestMTE, 1.0f, gConsole()->_fTestMTE ); //, gConsole()->_fTestMTE, gConsole()->_fTestMTE ); //gConsole()->_fTestMTE, gConsole()->_fTestMTE );

				tempMat = tempMat * (*mWorld);
				mWorld = &tempMat;
			}
#endif //__BS_TEST_MTE

			m1 = m_pGroup->_mUpdate[i] * *mWorld;
			if( pObj->m_bLight && s_bNight == FALSE && s_bOutDoor )	// 라이트 오브젝트는 낮엔 렌더링 되지 않음.
				continue;
#ifdef __BS_TEST_MTE
			//test
			if( bMode_A )
			{
				RenderNormal( pd3dDevice, pObj, &m1, XE_MTE_NOSKIN, dwBlendFactor );
			}
			else
#endif //__BS_TEST_MTE
