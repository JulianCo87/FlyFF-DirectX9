...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "LUA - %s" ), lua_tostring(pLuaState, -1) );
...
{
	FLERROR_LOG( PROGRAM_NAME, _T( "LUA - %s" ), lua_tostring(pLuaState, -1) );
...
void CLuaBase::CallLuaFunction( int nElem, int nReturn )
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, ToString( -1 ) );
		exit( 0 );
	}
}
