...
public:
	...
	void	Init( void )
	{
		...
		memset( m_aSpline, 0, sizeof(m_aSpline) );
	}
	void	Destroy( void )
	{
		Init();
...
public:
	...
	int					m_nEffectCount;					//sun: 제련 이팩트 향상 버전
...
//sun: 제련 이팩트 향상 버전
	...
	CPartsWeaponEffect*		m_pPartsEffectNew[PEF_MAX];			//0일반, 1속성

#ifdef _DEBUG
	CPartsWeaponEffect*		m_pPartsEffectNew_Debug[2];
#endif 

#endif //__CLIENT

	D3DXVECTOR3			m_vForce1, m_vForce2;
	D3DXVECTOR3			m_vForce3, m_vForce4;		// 9차 제련관련 (추가 검광 기능)
...
public:
	...
	void	SetGroup( int nNum ) 
	{
		...
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "SetGroup : lv=%d" ), nNum );
			nNum = 0;
		}
	...
	DWORD	IsAttrHit( float fNumFrm ) { 
		if( m_pMotion )
			return m_pMotion->IsAttrHit( m_fFrameOld, fNumFrm ); // nNumFrm프레임에 타격속성이 있는가?
		else 
	...
	{ 
		...
		FLERROR_LOG( PROGRAM_NAME, _T( "pMotion==NULL %d %d" ), m_fFrameOld, m_fFrameCurrent );
		return NULL;
	}	// 현재 프레임에 사운드속성이 있는가?
	...
	DWORD	IsAttrQuake( float fNumFrm )			{ if( m_pMotion ) { return m_pMotion->IsAttrQuake( m_fFrameOld, fNumFrm ); } return 0; }
	DWORD	IsAttrQuake( void )						{ if( m_pMotion) { return m_pMotion->IsAttrQuake( m_fFrameOld, m_fFrameCurrent ); } return 0; }
	void	SetAttr( float fNumFrm, DWORD dwAttr )	{ if( m_pMotion ) { m_pMotion->SetAttr( fNumFrm, dwAttr ); } }
...
//_SUN_ENCHANT_EFFECT17  *BEGIN=======================================================================================================
	...
	// 구인터페이스 통합.
	void    CreateParticleAndRender( int nParts, const D3DXMATRIX *pmWorld, int nType, int nLevel, int nLevel2, DWORD ik3_kind, BOOL bBaruna = FALSE );

protected:
	void	CreateParticle_New( int nParts, const D3DXMATRIX *pmWorld, int nAttr, int nLevel, DWORD ik3_kind, int nType = 0 );
	CPartsWeaponEffect*	CreateParticle_Internal( int nParts, int nAttr, int nLevel, DWORD ik3_kind, int nType = 0, TCHAR* filenameSFX = NULL );	
	void	RenderParticle_New( CPartsWeaponEffect* pEffect, int nParts, DWORD ik3_kind, const D3DXMATRIX* pmWorld );
	int		GetParticleIndex( int nParts, int nAttr );
	DWORD	GetSfxIDByAttr( int nAttr, int nLevel, DWORD ik3_kind, int nType = 0 );
	int		GetEffectLevelByOrgLevel( int nAttr, int nLevel );

public:
#ifdef _DEBUG
	//test new enchnat effect
	void    RenderItemEffect_TEST( const D3DXMATRIX* pmWorld );
#endif

#endif	//__CLIENT
//_SUN_ENCHANT_EFFECT17  *END=======================================================================================================

	void	CreateParticle( int nParts, const D3DXMATRIX *pmWorld, int nType, int nLevel, int nLevel2 );
	...
	void	RenderEffect( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXMATRIX *mWorld, DWORD dwItemKind3 = NULL_ID, int nLevelL = 0, int nLeveR = 0, int nType = 0 );
	...
	{	
		...
		for( int i = 0; i < MAX_ELEMENT; i ++ )
		{
			if( pElem->m_pObject3D )
			{
				...
				D3DXVECTOR3 *pTri = pElem->m_pObject3D->IntersectRayTri( vRayOrig, vRayDir, mWorld, pvIntersect, pfDist, bColl );
				if( pTri )
					return pTri;
			}
			pElem ++;
