...
//								str	= "Empty";
				...
				else if( s.Token == _T( "EventLimitLevel" ) )
				{
					...
					m_mapEventLimitLevel.insert( map<int, int>::value_type( pEvent->nId, nEventLimitLevel ) );
					s.GetToken();
#else	// __WORLDSERVER
					s.GetToken();
					s.GetToken();
...
bool CEventGeneric::AddCallTheRollItem( int nCallTheRollCount, DWORD dwItemId, int nItemNum, int nBinds )
{
	...
	map< int, vector<T_CALLTHEROLL_ITEM> >::iterator it = m_mapCallTheRollItem.find( nCallTheRollCount );
	...
	{
		vector<T_CALLTHEROLL_ITEM> vecCallTheRollItem;
		vecCallTheRollItem.push_back( tCallTheRollItem );
		m_mapCallTheRollItem.insert( map< int, vector<T_CALLTHEROLL_ITEM> >::value_type( nCallTheRollCount, vecCallTheRollItem ) );
		return true;
	}
...
bool CEventGeneric::GetCallTheRollItem( int nCallTheRollCount, vector<T_CALLTHEROLL_ITEM> & vecCallTheRollItem )
{
	map< int, vector<T_CALLTHEROLL_ITEM> >::iterator it = m_mapCallTheRollItem.find( nCallTheRollCount );
	if( it != m_mapCallTheRollItem.end() )
	{
...
{
	map<int, int>::iterator it = m_mapEventLimitLevel.find( nEventId );
