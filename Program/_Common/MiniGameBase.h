	...
	virtual void Serialize( CAr & ar )
	{
		...
		if( ar.IsStoring() )
		{
			...
			for( DWORD i=0; i<vecszData.size(); i++ )
				ar.WriteString( vecszData.at( i ).c_str() );
		}
		else
		{
...
public:
	...
	virtual BOOL Excute( FLWSUser* pUser, __MINIGAME_PACKET* pMiniGamePacket ) = FALSE;
	void	SendPacket( FLWSUser* pUser, __MINIGAME_PACKET MiniGamePacket );	// 클라이언트로 미니게임 패킷을 보낸다.
	void	SendExtPacket( FLWSUser* pUser, __MINIGAME_EXT_PACKET MiniGameExtPacket );
};
#endif // __WORLDSERVER
