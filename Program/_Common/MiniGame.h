...
#ifdef __WORLDSERVER
	int Result_Kawibawibo( FLWSUser* pUser );
	BOOL ResultItem_Kawibawibo( FLWSUser* pUser );
	__KAWIBAWIBO FindKawibawibo( int nWinCount );
	
	BOOL Result_Reassemble( FLWSUser* pUser, OBJID* objItemId, int nCount );
	BOOL ResultItem_Reassemble( FLWSUser* pUser );
	__ITEMGAME GetReassembleRandomItem();
	void OpenWnd_Reassemble( FLWSUser* pUser );
	
	void OpenWnd_Alphabet( FLWSUser* pUser );
	int Result_Alphabet( FLWSUser* pUser, OBJID* objItemId, int nCount, int nQuestionID );
	BOOL ResultItem_Alphabet( FLWSUser* pUser, int nNum );
	__ITEMGAME GetAlphabetRandomItem( int nNum );
	char GetPublicCharacter( int nQuestionNum, int nPublicAlphabet );
		
	void OpenWnd_FiveSystem( FLWSUser* pUser );
	void DestroyWnd_FiveSystem( FLWSUser* pUser );
	int Bet_FiveSystem( FLWSUser* pUser, int nBetNum, int nBetPenya );
	int Result_FiveSystem( FLWSUser* pUser );
	BOOL ResultPenya_FiveSystem( FLWSUser* pUser, int nResult );
#endif // __WORLDSERVER
