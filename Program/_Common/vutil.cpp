...
{
	FLASSERT( g_xFlyffConfig->GetCodePage() != 0 );

	LPCWSTR pwSrc = (LPCWSTR) lpszStr;

	if( g_xFlyffConfig->GetCodePage() == 874 ) // 타이 
	{
		return (BYTE)*lpszStr >= 0xa1 && (BYTE)*lpszStr <= 0xfb;
	}
	else if( g_xFlyffConfig->GetCodePage() == 949 ) // 한글 
	{
		return IsHangul( *pwSrc );
	}
	else if( g_xFlyffConfig->GetCodePage() == 932 ) // 일본 
	{
		return IsDBCSLeadByte( (BYTE)( *pwSrc ) );
	}
	else if( g_xFlyffConfig->GetCodePage() == 936 ) // 한자 : 중국
	{
		return IsDBCSLeadByte( (BYTE)( *pwSrc ) );
	}
	else if( g_xFlyffConfig->GetCodePage() == 950 ) // 한자 : 대만 
	{
...
{
	BYTE l = static_cast< BYTE >( word & 0xff );
	BYTE h = static_cast< BYTE >( word >> 8 );
...
void SetStrNull( TCHAR* lpStr, int nNullLength )
{
	...
	{
#ifdef __CLIENT
		if( g_xFlyffConfig->GetMainLanguage() == LANG_THA && g_xFlyffConfig->GetCodePage() == 874 ) // 타이 
			i++;
		else if( g_xFlyffConfig->GetMainLanguage() == LANG_VTN && g_xFlyffConfig->GetCodePage() == 1258 )
			i++;
		else
#endif//__CLIENT
	...
	)
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s not found" ), pFileName );
		return E_FAIL;
	}
	...
	if( file.Read( pSrcData, nSrcDataSize ) >= 1 )
	{
		...
		if( FAILED( hr ) )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "D3DXCreateTextureFromFileInMemoryEx %s %s" ), pFileName, DXGetErrorString9( hr ) );
		}
	}
		...
		if( pImage->nImgBit == IMGBIT_24 )
		{
			for( int y = pt.y; y < pt.y + sizeTexture.cy; y++, dwOffsetDest += dwLine )
			{
				for( int x = pt.x; x < pt.x + sizeTexture.cx; x++, dwOffsetSrc += 3, dwOffsetDest++ )
				{
					...
					{
						const DWORD bitcount = pImage->nImgBit / 8;
...
BOOL LoadImage( LPCTSTR lpszFileName, LPIMAGE lpImage ) //LPBYTE* lppData, SIZE* pSize, int* pnImgBit )
{
	...
	//gmpbigsun(20110719) : 로딩 실패시 관련값 모두 초기화
	BOOL bOK = TRUE;
	if( strstr( szFileName, ".tga" ) )
	{
		bOK = LoadTGA( lpszFileName, lpImage );
	}
	else
	if( strstr( szFileName, ".bmp" ) )
	{
		bOK = LoadBMP( lpszFileName, lpImage );
	}
	else
	{
		FLASSERT( 0 );
...
BOOL LoadBMP( LPCTSTR lpszFileName, LPIMAGE lpImage ) //LPBYTE* lppData, SIZE* pSize )
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "LoadBMP Failed = %s" ), lpszFileName );
		return FALSE;
	}
...
BOOL IsFontInstalled( LPCTSTR pszFace )
{
	...
	FLStrcpy(lf.lfFaceName, _countof( lf.lfFaceName ), pszFace);
...
BOOL IsInteger( const char* pszSource )
{
	...
	char chFirst = pszSource[ nIndex ];
	while( chFirst == ' ' )
	{
		chFirst = pszSource[ ++nIndex ];
	}
	...
	char chChecking = pszSource[ ++nIndex ];
	while( chChecking != 0 )
	{
		if( chChecking != ' ' && ( chChecking < '0' || chChecking > '9' ) )
		{
			return FALSE;
		}
		chChecking = pszSource[ ++nIndex ];
	}
