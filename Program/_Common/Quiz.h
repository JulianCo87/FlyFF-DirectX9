...
public:
	...
	void	RemoveNPC();
	void	NoticeMessage();
	int		EntranceQuizEvent( FLWSUser* pUser );
	int		TeleportToQuizEvent( FLWSUser* pUser, int nZone );
	D3DXVECTOR3	GetTeleportPos( int nZone );
	void	DropOutWrongUser( int nQuizId, int nCorrect, int nItemId = 0, int nItemNum = 0 );
	BOOL	PtInCorrectZoneRect( FLWSUser* pUser, int nCorrect );
	int		GetUserSelectExample( FLWSUser* pUser );
