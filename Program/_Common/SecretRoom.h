...
public:	
	...
	void SendNowState( FLWSUser* pUser, int nState, DWORD dwRemainTime );
	void SendNowStateAllMember( BYTE nCont = CONT_NODATA, int nState = 0, DWORD dwRemainTime = 0 );
	void GetAllInfo( FLWSUser* pUser );				// 접속시 현재 진행중인 구성원이면 비밀의 방 현재 정보를 전송
	BOOL IsPossibleGuild( FLWSUser* pUser );			// 입찰 가능한 길드인가?
	BOOL IsGuildMaster( FLWSUser* pUser );				// 길드 마스터 인가?
	BOOL IsOtherTenderGuild( FLWSUser* pUser );		// 다른 대륙에 입찰한 길드인가?
	void SetTenderOpenWnd( FLWSUser* pUser );			// 비밀의 방 신청창을 열때 필요한 정보 전송
	void SetTender( FLWSUser* pUser, int nPenya );		// 입찰
	void SetTenderCancelReturn( FLWSUser* pUser );		// 입찰 취소시 수수료를 제외한 신청금을 반환한다.
	void SetLineUpOpenWnd( FLWSUser* pUser );			// 참가자 구성 창을 열때 필요한 정보 전송
	void SetLineUp( FLWSUser* pUser, vector<DWORD> vecLineUpMember );	// 참가자 구성
	void GetTenderView( FLWSUser* pUser );				// 입찰현황 창을 열때 
	void SecretRoomOpen();							// 금주 비밀의 방을 오픈한다.
	void SetTeleportSecretRoom( FLWSUser* pUser );		// 비밀의 방으로 텔레포트...
	void SetSecretRoomKill( DWORD dwWorldId, DWORD dwMonsterId );	// 비밀의 방에서 몬스터를 kill했을때 처리
	void SetFailGuild( FLWSUser* pUser );				// 길마가 죽거나 로그 아웃시 패배 처리
	D3DXVECTOR3 GetRevivalPos( FLWSUser* pUser );		// 캐릭터 사망시, 종료시, 로그 아웃시 각 캐릭터 부활위치
...
public:
	...
	BOOL	IsLineUpMember( FLWSUser* pUser );				// 구성된 참가자 인가?
	void	SetContTender( FLWSUser* pUser, int nPenya );	// 실제 해당 대륙에 입찰
	void	SortTenderGuild();							// 입찰금 순으로 정렬
	void	SetContTenderCancelReturn( FLWSUser* pUser );	// 입찰 취소시 반환
	void	SetContLineUp( FLWSUser* pUser, vector<DWORD> vecLineUpMember );	// 참가자 구성
	void	SecretRoomContOpen();							// 실제 대륙별 비밀의 방 오픈
	void	SetSecretRoomWorld( int nContNum );				// 각 길드를 비밀의 방 16개중에 지정해 준다.
	void	SetTeleportSecretRoomCont( FLWSUser* pUser );		// 해당 비밀의 방으로 텔레포트...
	void	SetTeleportNormalRoom( FLWSUser* pUser, int nIndex );		// 일반 몬스터 위치로 텔레포트...
	void	SetTeleportBossRoom( FLWSUser* pUser, int nIndex );		// 최종보스인 경우 텔레포트 위치가 다르다.
	...
	void	UpdateInfoToMember( FLWSUser* pUser, BYTE nType, int nIndex = NULL_ID );	// kill count를 client에 알려준다.
