...
{ 
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "ActiveMover==NULL 0x%08x" ), (int)g_pPlayer );
	}
...
void CWorld::Render( LPDIRECT3DDEVICE9 pd3dDevice, CD3DFont* pFont )
{
	...
	// 스카이 박스 랜더링 
	int nType = ( IsEventArena( ) ? 1 : 0 );
	m_skyBox.RenderFall( pd3dDevice, nType );
...
//	int px,pz;
	...
	for( i = z - m_nVisibilityLand; i <= z + m_nVisibilityLand; i++)
	{
		for( j = x - m_nVisibilityLand; j <= x + m_nVisibilityLand; j++)
		{
			...
			if( LandInWorld( (int)j, (int)i ) && m_apLand[ nOffset ] )
			{
				//gmpbigsun( 20110120, #11199 ) : 물은 바닥에서부터 위에 있기때문에 바닥이 안보이더래도 물은 보여야 하므로, 
				//물의 높이를 기준으로 랜더여부설정
			...
			//	if(m_apLand[ nOffset ]->isVisibile())
				if( (int)LODLEVELRANGE3 > nHeightDiff )
				{
...
//sun: 13, 하우징 시스템
	...
	if( m_bViewAllObjects )
	{
		...
		D3DXVECTOR3* pvCamera = NULL;
		float xDist = 0.0f, yDist = 0.0f, zDist = 0.0f;
		...
		FOR_LAND( this, pLand, m_nVisibilityLand, FALSE )
		{
			FOR_OBJARRAY( pLand, pObj )
			{
				...
				//gmpbigsun( 20110713 ) : 환경이펙트 옵션을 OT_SFX만 적용
				...
				////gmpbigsun( 20100916 ) : 공간분할 방식이 아니기 때문에 y축으로 크기가 큰 오브젝일 경우 
				//랜드스케이프별 컬링방식은 약간 미흡한 면이 있다. 허나 굉장히 빠른편이긴 하다.
				//카메라의 방향에 의해 랜드스케이프가 culling되었다 하더래도 크기 선별법으로 culling 여부를 판단한다.
				BOOL bInside = FALSE;
				
#ifdef __CLIENT
				//gmpbigsun( 20111114 ) : 주인공은 항상 보이게 #20426
				if( pObj == g_pPlayer )
					bInside = TRUE;
#endif //__CLIENT
...
//								if( pObj->m_pModel->GetModelType() != MODELTYPE_ANIMATED_BILLBOARD )
							...
							FLTRACE_LOG( PROGRAM_NAME, _T( "한 화면에 출력 오브젝트가 %d를 넘었다. 위험!!!!" ), m_nObjCullSize);
					}
				}
			}
			END_OBJARRAY
			...
			for( i = 0; i < m_nObjCullSize; i++)
			{
				...
				if( !pObj->IsActiveObj() )
				{
					...
					{	
						...
						{
							CMover* pMover = (CMover*)pObj;
							if( pMover && !pMover->IsDie() && !pMover->IsMode( TRANSPARENT_MODE ) )
							{
								if( pObj->IsRangeObj( g_pPlayer->GetPos(), 20.0f ) )
								{
									CWorld::m_amvrSelect[nCount++] = pMover;
...
//				pd3dDevice->SetRenderState(D3DRS_DEPTHBIAS, 0 );
	...
	{
		//수중 효과 테스트
		static LPDIRECT3DTEXTURE9 pWaterTexture = NULL;
		if( !pWaterTexture )
		{
			// 그런 텍스쳐 원래 없음 에러 로그만 자꾸 남길래 주석 처리 - lenahyang
			//HRESULT hr = LoadTextureFromRes( m_pd3dDevice, MakePath( DIR_SFXTEX, _T( "Test_underwater.dds" ) ), &pWaterTexture );
			//if( hr != D3D_OK ) 
			//{
				pWaterTexture = prj.m_terrainMng.GetTerrain( 20 )->m_pTexture;
			//}
		}
			...
			( lpWaterHeight->byWaterTexture & (byte)(~MASK_WATERFRAME)) == WTYPE_WATER )
		{
			...
			m_pd3dDevice->SetTexture( 0, pWaterTexture ); //prj.m_terrainMng.GetTerrain( 20 )->m_pTexture );
...
void RenderShadowMap( LPDIRECT3DDEVICE9 pd3dDevice, CObj **pList, int nMax )
{
	...
	pd3dDevice->SetRenderState( D3DRS_TEXTUREFACTOR, D3DCOLOR_ARGB(255,192,192,192) );		//192
...
//		fDistLight = -30.0;
	...
	D3DXMatrixPerspectiveFovLH( &g_mShadowProj, D3DX_PI/4, 1.0f, 1.0f, 1000.0f );		// gmpbigsun: 0.5f, 128.0f -> 1.0f, 1000.0f
...
BOOL CWorld::CheckBound(D3DXVECTOR3* vPos,D3DXVECTOR3* vDest,D3DXVECTOR3* vOut, FLOAT* fLength )
{
	...
	for( int i = 0; i < int( length * 10 ); i++ ) 
	{
		...
		if(tempVec2.y<tempheight+0.6f)		//sun: 11, 피킹 문제 수정
		{
			b1 = TRUE;
...
//			return TRUE;
	...
	if( bRet )
	{
		...
		//gmpbigsun( 20100817 ) : 오브젝트와의 충돌 지점 강제로 높임, 카메라가 오브젝트를 뚫는현상을 어느정도 완화, 나중에 제대로 고치자.
		if( vIntersect.y < vPos->y )
			vIntersect.y -= .6f;			
		else vIntersect.y += .6f;			
...
CObj* CWorld::FindObject( DWORD dwObjectFilter, CObj* pExceptionObj,  BOOL bOnlyNPC )
{
	if( m_pCamera == NULL )
		return NULL;
	...
	for( int i = 0; i < m_nObjCullSize; i++ )
	{
		...
		if( pObj )
		{
			if( pObj->GetModel()->m_pModelElem->m_bPick == FALSE )
				continue;
			if( pObj->IsCull() == FALSE ) 
			{
				if( pObj != pExceptionObj && ( ObjTypeToObjFilter( pObj->GetType() ) & dwObjectFilter ) )
