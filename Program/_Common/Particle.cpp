...
CEmitter::CEmitter()
{
	...
	for( list< CUnit* >::iterator kItr = m_plistUnit.begin(); kItr != m_plistUnit.end(); ++kItr )
	{
		SAFE_DELETE( *kItr );
	}
    m_plistUnit.clear();
...
HRESULT CEmitter::SetVertex(CUnit *pUnit)
{
    ...
    if( m_Kind == KIND::RECTANGLE )
    {
        ...
        m_vecvecVertex.at( Index ).push_back( Tri[0] );
        m_vecvecVertex.at( Index ).push_back( Tri[1] );
        m_vecvecVertex.at( Index ).push_back( Tri[2] );

        m_vecvecVertex.at( Index ).push_back( Tri[0] );
        m_vecvecVertex.at( Index ).push_back( Tri[2] );
        m_vecvecVertex.at( Index ).push_back( Tri[3] );
    }
    else
    if( m_Kind == KIND::CYLINDER )
    {
        ...
        // Strip º¸°£
        if( m_vecvecVertex.at( pUnit->m_AniIndex ).size() )
        {
            ...
            m_vecvecVertex.at( pUnit->m_AniIndex ).push_back( Tri[0] );
            m_vecvecVertex.at( pUnit->m_AniIndex ).push_back( Tri[0] );
            ...
            m_vecvecVertex.at( pUnit->m_AniIndex ).push_back( Tri[1] );
        }
        ...
        for( int i=0; i<MAX_CYLINDER; i++ )
        {
            ...
            m_vecvecVertex.at( pUnit->m_AniIndex ).push_back( Tri[i] );
        }
        
        m_vecvecVertex.at( pUnit->m_AniIndex ).push_back( Tri[MAX_CYLINDER-1] );
    }
...
*/
    ...
    while( iterBegin != iterEnd )
    {
        ...
        for( int i=0; i<pEmitter->m_vecvecVertex.size(); i++ )
        {
            ...
            if( pEmitter->m_vecvecVertex.at( i ).size() )
            {
                int  dwSize = pEmitter->m_vecvecVertex.AT_BLOW( i ).size();

                if( pEmitter->m_pvecTexture.size() )
                    hr = pd3dDevice->SetTexture(0, pEmitter->m_pvecTexture.at( i ).m_pTex );
                else
                ...
                memcpy( vertex, &( ((pEmitter->m_vecvecVertex).at(i)).at(0), sizeof(PARTICLE_VERTEX)*dwSize );
                ...
                pEmitter->m_vecvecVertex.at( i ).clear();
            }
        }
