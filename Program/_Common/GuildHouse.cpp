...
void CGuildHouseMng::UpdateGuildHouseTender( DWORD dwGHType, CQuery* pQuery )
{
	...
	if( ptTenderData != NULL )
	{
		for( MAP_T_GUILDHOUSE_TENDER::iterator it = m_mapGuildHouseTender.begin(); it != m_mapGuildHouseTender.end(); ++it )
		{
			for( vector<T_GUILDHOUSE_TENDER>::iterator it2 = it->second.begin(); it2 != it->second.end(); ++it2 )
			{
...
void CGuildHouseMng::SendGuildHouseTenderInfo( DPID dpId )
{
	...
	if( ptTenderData != NULL )
	{
		...
		map<DWORD, DWORD> mapGHTenderList;
...
{
	map<DWORD, DWORD> mapHighestTenderGuild;
...

	{
		for( MAP_T_GUILDHOUSE_TENDER::iterator it = m_mapGuildHouseTender.begin(); it != m_mapGuildHouseTender.end(); ++it )
		{
			...
			{
				mapHighestTenderGuild.insert( map<DWORD, DWORD>::value_type( it->first, dwHighestGuildId ) );
			}
			dwHighestGuildId = 0;
...

	{
		for( MAP_T_GUILDHOUSE_TENDER::iterator it = m_mapGuildHouseTender.begin(); it != m_mapGuildHouseTender.end(); ++it )
		{
			...
			map<DWORD, DWORD>::iterator it3 = mapHighestTenderGuild.find( it->first );
	...
	for( map<DWORD, DWORD>::iterator it = mapHighestTenderGuild.begin(); it != mapHighestTenderGuild.end(); ++it )
	{
...
bool CGuildHouseMng::ReqTenderGuildHouseList( FLWSUser* pUser, DWORD dwGHType, DWORD dwNpcIndex )
{
	if( IsValidObj( pUser ) == TRUE )
	{
		...
		if( ptTenderData != NULL )
		{
			map<DWORD, int>::iterator it = ptTenderData->mapNpc.find( dwNpcIndex );
	...
	if( IsValidObj( pUser ) == TRUE )
	{
		...
		if( ptGHData != NULL )
		{
			...
			if( ptTenderData != NULL )
			{
				...
				vector<DWORD> vecTenderGuild = GetTenderView( dwGHIndex );
...
vector<DWORD> CGuildHouseMng::GetTenderView( DWORD dwGHIndex )
{
...
void CGuildHouseMng::LoadScript()
{
	...
	while( m_Lua.TableLoop( -2 ) )
	{
		...
		while( m_Lua.TableLoop( -2 ) )
		{
			...
			it->second.mapNpc.insert( map<DWORD, int>::value_type( dwNpcIndex, nAreaIndex ) );
...
bool	CGuildHouseMng::GetTenderAbleGuildHouse( DWORD dwGHType, map<DWORD, DWORD> & mapGHTenderList )
{
	...
	if( nTenderArea > 0 )
	{
		for( MAP_T_GUILDHOUSE_DATA::iterator it = m_mapGuildHouseData.begin(); it != m_mapGuildHouseData.end(); ++it )
		{
			...
			{
				mapGHTenderList.insert( map<DWORD, DWORD>::value_type( it->second.dwGHIndex, it->second.dwGHTitleIndex ) );
			}
		}
	}
...
bool	CGuildHouseMng::AddGuildHouseTender( const PT_GUILDHOUSE_TENDER ptGHTender )
{
	...
	{
		vector<T_GUILDHOUSE_TENDER> vecGHTender;
