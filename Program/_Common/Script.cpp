...
void CScript::LoadString( void )
{
	...
	while( tok != FINISHED )
	{
		...
		if( str.Find( "IDS", 0 ) != 0 )
		{
			FLINFO_LOG( PROGRAM_NAME, _T( "LoadString: %s" ), m_strFileName );
			GetToken();
			continue;
		}
		GetLastFull();
		...
		if( !bResult )
			FLERROR_LOG( PROGRAM_NAME, _T( "string error: %s" ), str );
...
void CScript::SntxErr( LPCTSTR lpszHeader, int /*error*/)
{
	...
	FLERROR_LOG( PROGRAM_NAME, string );
	GoMark();
}
...
int CScript::GetToken( BOOL bComma )
{
	...
	if( tokenType == IDENTIFIER ) 
	{
		...
		if( IsInterpriterMode() )
		{
			...
			{
				tokenType = STRING;
				FLStrcpy( m_mszToken, _countof( m_mszToken ), i->second.c_str() );
				Token	= m_mszToken;
			}
			else
			{
				...
				{
					tokenType = NUMBER;
					FLSPrintf( m_mszToken, _countof( m_mszToken ), "%d", nValue );
					Token = m_mszToken;
				}
				else
				{
					...
					else if( m_dwDef == 2 )
					{
						...
						tok			= LookUp( token );
						if( !( tok ) && InternalFunc( token ) == -1 && FindFunc( token ) == FALSE && *token != '#' )
						{
							CString string;
							string.Format( "%s Not Found.", token );
							SntxErr( string, 0 );
...
void CScript::DeclGlobal()
{
	...
	do { // process comma-separated list 
		GetToken(); // get name 
		FLStrcpy(globalVars[gVarIndex].varName, _countof( globalVars[gVarIndex].varName ), token);
		GetToken();
		gVarIndex++;
	} while(*token==',');
	...
	do 
	{
		...
		else if(tok==ENUM) 
		{
			...
			do 
			{
				...
				if( m_defines.insert( make_pair( Token, eNum ) ).second == false )
					FLTRACE_LOG( PROGRAM_NAME, _T( "Enum %s dup." ), Token );
				eNum++;
			} 
			while(*token!='}');
		...
		else if(tokenType==IDENTIFIER) 
		{
			...
			{ 
				...
				FLStrcpy(funcTable[funcIndex].name, _countof( funcTable[funcIndex].name ), strTemp);
				++funcIndex;
				while(*m_pProg!=')') 
					m_pProg++;	
				m_pProg++;
...
void CScript::ExecDefine()
{
	...
	{
		FLTRACE_LOG( PROGRAM_NAME, _T( "define %s dup" ), Token);
		//FLERROR_LOG( PROGRAM_NAME, _T( "Define duplication. [%s] [%s] [%d]" ), m_strFileName, str.c_str(), n);
	}
}
	...
	if(!process) // 인터프리터 
	{
		...
		for(int i=0; i<22; ++i)
			if(!strcmp(_table[i].command,s)) 
				return cmd = static_cast< char >( _table[i].tok );
	#endif // __REMOVE_SCIRPT_060712
	}
	else if(s[0]=='@')  // 컴파일
		return static_cast< char >( _table[atoi(&s[1])].tok );
	return 0;
}
...
BOOL	CScript::LoadTxtDefines( const TCHAR* pszFileName, BOOL bMultiByte, int /*nProcess*/ )
{
	if( CScanner::Load( pszFileName, bMultiByte ) == FALSE )
	{
		return FALSE;
	}
	...
	{
		GetToken();
		TCHAR szDefine[128] = { 0, };
		FLStrcpy( szDefine, _countof( szDefine ), Token );
		int nDefineNumber = GetNumber();

		if( m_defines.insert( std::make_pair( szDefine, nDefineNumber ) ).second == false )
		{
			return FALSE;
		}
	}
	...
	do 
	{
		...
		switch(tokenType) 
		{
			...
			case IDENTIFIER:
				...
				{
					str[0] = '$';	
					FLIntToString(r, str, _countof( str ), 10);
					Write(&cplProg,str);
				}
				else 
					Write(&cplProg,token);
			...
			case KEYWORD:
				switch(cmd) 
				{
					...
					case ENUM: // enum
						...
						break;
					default:
						str[0] = '@';	FLIntToString(cmd, str, _countof( str ), 10);
						Write(&cplProg,str);
						break;
				}
...
BOOL CScript::Run()
{
	...
	m_pProg--;
	FLStrcpy(token, MAX_TOKENSTR, "main");
	Call();	
...
void CScript::EvalExp0(int *value)
{
	...
	if(tokenType == IDENTIFIER) 
	{
		...
		{ // if a var, see if assignment 
			FLStrcpy( temp, _countof( temp ), token );
			temp_tok = tokenType;
			GetToken();
			char op;
			op =  *token;
			...
			{ // not an assignment 
				...
				FLStrcpy(token, MAX_TOKENSTR, temp);
				tokenType = temp_tok;
			}
		}
	}
...
int CScript::InterpBlock(int aaa)
{
	...
	do {
		...
		{
			p = m_pProg;	
			FLStrcpy( tpTkn, _countof( tpTkn ), token );
			GetToken();
			if(*token!=':') 
			{
				FLStrcpy(token, MAX_TOKENSTR, tpTkn);
				m_pProg = p;
...
void CScript::DeclLocal()
{
	...
	do { // process comma-separated list 
		...
		FLStrcpy( i.varName, _countof( i.varName ), token );
		LocalPush(i);
		GetToken();
	} while(*token==',');
	if(*token!=';') SntxErr( NULL, SEMI_EXPECTED);
...
void CScript::FindLabel()
{
	...
	do 
	{
		...
		if( tokenType == IDENTIFIER ) 
		{
			...
			if( *token==':' ) 
			{
				FLStrcpy( labelTable[ labeltos ].name, _countof( labelTable[ labeltos ].name ), temp );
	...
	else 
	{
		...
		do { // process comma-separated list of parameters 
			...
			if(*token!=')') {
				...
				FLStrcpy( p->varName, _countof( p->varName ), token );
				GetToken();
				i--;
			}
			else break;
...
BOOL CScript::GetStringByStringKey( const std::string& key, CString& rValue )
{
	map<std::string, std::string>::iterator iter = CScript::m_mapString.find( key );
	if( iter != CScript::m_mapString.end() )
	{
		std::string strTemp = iter->second;
		rValue = CString( strTemp.c_str() );
		return TRUE; 
	}
