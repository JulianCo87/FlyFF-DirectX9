...
struct BONE
{
	BONE() : m_pParent( NULL ), m_nParentIdx( -1 )
	{
		m_szName[ 0 ]	= '\0';		
		::memset( &m_mTM, 0, sizeof( m_mTM ) );
		::memset( &m_mInverseTM, 0, sizeof( m_mInverseTM ) );
		::memset( &m_mLocalTM, 0, sizeof( m_mLocalTM ) );
	}
...
struct TM_ANIMATION
{
	TM_ANIMATION() : m_vPos( D3DXVECTOR3( 0, 0, 0 ) )		{	::memset( &m_qRot, 0, sizeof( m_qRot ) ); 		}
//	D3DXMATRIX			m_mAniTM;
...
struct	BONE_FRAME
{
	BONE_FRAME() : m_pFrame( NULL )  { ::memset( &m_mLocalTM, 0, sizeof( m_mLocalTM ) ); }
...
struct	MOTION_ATTR
{
	MOTION_ATTR() : m_dwAttr( 0 ), m_nSndID( 0 ), m_fFrame( 0.0 ) {}
	DWORD	m_dwAttr;
