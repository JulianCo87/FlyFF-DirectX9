...
int		CBones :: LoadBone( LPCTSTR szFileName )
{
	...
	if( nVer < VER_BONE )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s의 버전은 %d.  최신버전은 %d" ), szFileName, nVer, VER_BONE );
		resFp.Close();
		return FAIL;
	}
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s : file name is too long" ), szFileName );
		resFp.Close();
		return FAIL;
	}

	// 파일명 카피
	FLStrcpy( m_szName, _countof( m_szName ), szFileName );
...
//sun: 12, // __LEAK_0827
	...
	{
		SAFE_DELETE( pMotion );
		FLERROR_LOG( PROGRAM_NAME, _T("LoadMotion error %s"), szFileName );
		return NULL;
	}
...
//	int		i, j;
	...
	if( nVer != VER_MOTION )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s의 버전은 %d.  최신버전은 %d" ), szFileName, nVer, VER_MOTION );
		resFp.Close();
		return FAIL;
	}
	...
	if( nNumFrame <= 0 )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s read MaxFrame = %d" ), szFileName, nNumFrame );
		resFp.Close();
		return FAIL;
	}
...
void	CMotion :: ReadTM( CResFile *file, int nNumBone, int nNumFrame )
{
	...
	for( i = 0; i < nNumBone; i ++ )
	{
		file->Read( &nLen, 4, 1 );
		if( nLen > 32 )		
			FLERROR_LOG( PROGRAM_NAME, _T( "%s bonename is too long" ), m_szName );
...
//			}
	...
	if( nCnt != nNumSize )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s : frame size error" ), m_szName );
	}	
}
