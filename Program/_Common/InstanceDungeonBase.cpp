...
INDUN_INFO * CInstanceDungeonBase::CreateDungeon( const DWORD dwDungeonId, const DWORD dwWorldId, const ULONG uMultiKey, const DWORD dwGuildID, const int eDungeonLevel )
{
	...
	else
	{
		VEC_IDINFO vecTemp;
		INDUN_INFO * pInfo	= new INDUN_INFO( dwWorldId, uMultiKey, dwDungeonId, dwGuildID, eDungeonLevel );
		pair<MAP_IDBASE::iterator, bool> ret = m_mapID.insert( MAP_IDBASE::value_type( dwDungeonId, vecTemp ) );
		if( ret.second == false )
		{
			SAFE_DELETE( pInfo );
			return NULL;
...
{
	vector< pair<INDUN_INFO*, DWORD> > vecpairTemp;
...
void CInstanceDungeonBase::LoadScript( const char* szFilename )
{
	...
	while( Lua.TableLoop( -2 ) )
	{
		...
		while( Lua.TableLoop( -2 ) )
		{
			...
			realData.mapTeleportPos.insert( map<int, D3DXVECTOR3>::value_type( nStage, vPos ) );
			Lua.Pop( 1 );
		}
...
D3DXVECTOR3 CInstanceDungeonBase::GetTeleportPosByStage( DWORD dwWorldId, DWORD /*dwDungeonId*/, const DWORD dwStage )
{
	...
	map<int, D3DXVECTOR3>::iterator it2 = it->second.mapTeleportPos.find( dwStage/*GetState( dwWorldId, dwDungeonId )*/ );
...
#ifdef __WORLDSERVER
void CInstanceDungeonHelper::LoadScript() {
