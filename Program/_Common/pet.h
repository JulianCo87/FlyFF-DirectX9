...
const BYTE MAX_PET_AVAIL_LEVEL	= 9;
	...
	DWORD	GetItemId( void )
		{
			if( m_nLevel == PL_EGG )
			{
				return	( ITEM_INDEX( 21029, II_PET_EGG ) );
			}

			return	( CPetProperty::GetInstance()->GetAvailParam( m_nKind )->m_dwItemId );
		}
	DWORD	GetKind ( DWORD	dwItemId )		// 아이템 인덱스로 펫종류를 얻어옵니다.
	{
		for ( int i = 0; i < MAX_PET_KIND; i++ )
		{
			if ( CPetProperty::GetInstance()->GetAvailParam(i)->m_dwItemId == dwItemId )
				return i;
		}
		return -1;
	}
...
typedef	struct	_TransformStuffComponent
{
	...
	_TransformStuffComponent( DWORD dwItemObjID, int nNum )
	{
		this->dwItemObjID	= dwItemObjID;
		this->nNum			= nNum;
	}
...
public:
	...
	void	AddComponent( DWORD dwItemObjID, int nNum );		// 필요 아이템 요소 추가
	...
	TransformStuffComponent*	GetComponent( int i )	{	return &m_vComponents.at( i );	}
private:
...
public:
	...
	virtual	BOOL	IsValidStuff( FLWSUser* pUser, CTransformStuff & stuff );
	// 사용자의 아이템을 변환한다
	void	Transform( FLWSUser* pUser, CTransformStuff& stuff );
	// 사용자로부터 변환 재료를 제거한다
	void	RemoveItem( FLWSUser* pUser, CTransformStuff& stuff );
	// 사용자에게 변환 결과 아이템을 만들어준다
	bool	CreateItem( FLWSUser* pUser, CTransformStuff& stuff );
};
...
{
	FLItemElem*	pItem;	// 변환 결과 아이템
	int		nProb;	// 확률
	_TransformItemElement( FLItemElem* pItem, int nProb )
	{
		this->pItem	= pItem;
		this->nProb	= nProb;
	}
...
public:
	...
	FLItemElem*	GetItem( int nTransform );
	BOOL	LoadScript( const char* szFile );
	FLItemElem*	CreateItemGeneric( CScript& s );
	FLItemElem*	CreateItemPet( CScript& s );
private:
