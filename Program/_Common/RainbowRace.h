...
#endif // __CLIENT
#ifdef __WORLDSERVER
	void	SetNowGameComplete( FLWSUser* pUser );	// 이번 게임을 완료상태로 만들고 다음 게임을 셋팅한다.
...
//	BOOL	LoadScript();
	...
	void			SetApplicationUser( FLWSUser* pUser );		// 플레이어가 신청한 경우..
	...
	void			OnMiniGamePacket( FLWSUser* pUser, __MINIGAME_PACKET* pMiniGamePacket );	// 미니게임 패킷을 받은뒤 처리
	void			SetRanking( FLWSUser* pUser );	// 레이스를 종료한 캐릭터를 순위에 등록
