...
public:
	...
	void	SetPCBangPlayer( FLWSUser* pUser, DWORD dwPCBangClass );	// PC방 유저를 목록에 추가한다.
	...
	float	GetExpFactor( FLWSUser* pUser );				// 증가될 경험치를 얻어온다.
	float	GetPartyExpFactor( FLWSUser* apUser[], int nMemberSize );
	float	GetPieceItemDropInfo( DWORD dwHour );
	float	GetPieceItemDropFactor( FLWSUser* pUser );		// 증가될 아이템 드롭률을 가져온다.
