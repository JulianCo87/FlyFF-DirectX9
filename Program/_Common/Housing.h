...
#ifdef __WORLDSERVER
	BOOL IsListUpAble( FLWSUser* pUser, DWORD dwItemId );	// 가구 목록 추가가 가능한가?
	BOOL IsSetupAble( FLWSUser* pUser, HOUSINGINFO housingInfo );		// 가구 설치가 가능한가?
...
public:
	...
	BOOL ReqSetFurnitureList( FLWSUser* pUser, DWORD dwItemId );	// Trans에 가구 목록 추가 요청
	BOOL ReqSetupFurniture( FLWSUser* pUser, HOUSINGINFO housingInfo );	// Trans에 가구 설치 및 제거 요청
	BOOL ReqSetAllowVisit( FLWSUser* pUser, DWORD dwPlayerId, BOOL bAllow ); // Trans에 허용 목록 추가 요청
	void ReqGMFunrnitureListAll( FLWSUser* pUser );	// GM 가구 목록 전체 삭제..
...
public:
	...
	void OnReqVisitableList( FLWSUser* pUser );	// 클라이언트가 방문 가능 목록을 요청했다.
	void SetVisitRoom( FLWSUser* pUser, DWORD dwPlayerId ); // 방문...
	void GoOut( FLWSUser* pUser );		// 방에서 나감.
#endif // __WORLDSERVER
