...
m_tCreate( tCreate )
{
	FLStrcpy( m_szPledge, _countof( m_szPledge ), szPledge );
}
...
void CCandidate::Serialize( CAr & ar )
{
	...
	else
	{
		ar >> m_idPlayer >> m_iDeposit >> m_nVote;
		ar >> m_tCreate;
		ar.ReadString( m_szPledge, _countof( m_szPledge ) );
	}
}
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "IElection.BeginVote" ) );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "IElection::EndVote( %d )" ), idPlayer );
	SortVote();
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "IElection::AddDeposit( %07d, %I64d, %d )" ), idPlayer, iDeposit, tCreate );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "IElection::SetPledge( %07d, %s )" ), idPlayer, szPledge );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "IElection::IncVote( %07d, %07d )" ), idPlayer, idElector );
...
u_long IElection::GetResult( void )
{
	...
	if( m_vCandidates.empty() )
		return NULL_ID;
	return m_vCandidates.at( 0 )->GetIdPlayer();
}
...
__int64 ILordEvent::GetCost( int iEEvent, int iIEvent )
{
	...
	if( iEEvent >= (int)( m_vEFactor.size() ) || iIEvent >= (int)( m_vIFactor.size() ) )
		return 0;
	return m_vEFactor.at( iEEvent ).second + m_vIFactor.at( iIEvent ).second;
}
...
float ILordEvent::GetEFactor( int iEEvent )
{
	if( iEEvent >= (int)( m_vEFactor.size() ) )
		return 1.0F;
	// second로 되있이서 수정했습니다~ 
	return m_vEFactor.at( iEEvent ).first;
}
...
float ILordEvent::GetIFactor( int iIEvent )
{
	if( iIEvent >= (int)( m_vIFactor.size() ) )
		return 1.0F;
	return m_vIFactor.at( iIEvent ).first;
}
...
void ILordEvent::AddComponent( u_long idPlayer, int iEEvent, int iIEvent )
{
	...
	{
#ifdef __DBSERVER
		FLERROR_LOG( PROGRAM_NAME, _T( "iEEvent == 0 && iIEvent == 0" ) );
#endif	// __DBSERVER
		return;
	}
	if( iEEvent >= (int)( m_vEFactor.size() ) || iIEvent >= (int)( m_vIFactor.size() ) )
	{
#ifdef __DBSERVER
		FLERROR_LOG( PROGRAM_NAME, _T( "iEEvent >= m_vEFactor.size() || iIEvent >= m_vIFactor.size()" ) );
#endif	// __DBSERVER
		return;
	}
	CLEComponent* pComponent
		= new CLEComponent( eTick, idPlayer, m_vEFactor.at( iEEvent ).first, m_vIFactor.at( iIEvent ).first );
...
{
	FLASSERT( i >= 0 && i < GetComponentSize() );
