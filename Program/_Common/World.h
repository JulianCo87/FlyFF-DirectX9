...
public:
	...
	static int				s_nLandWidthMax;
	static int				s_nLandHeightMax;
...
//	CMclCritSec		m_csModifyLink;
	int				m_cbModifyLink;
	CObj*			m_apModifyLink[MAX_MODIFYLINK];		// 16k

//	CMclCritSec		m_csReplaceObj;
	int				m_cbReplaceObj;
...
public:
	...
	BOOL	IsArena( void )	{ return ( m_dwWorldID == WI_WORLD_ARENA );	}
	BOOL	IsEventArena( ) { return ( m_dwWorldID == WI_WORLD_FWC ); }
...
public:
	...
	CObj*			FindObject( DWORD dwObjectFilter, CObj* pExceptionObj,  BOOL bOnlyNPC );
...
//	if( 0 <= _dwLinkType && _dwLinkType < MAX_LINKTYPE )																				\
//	{																																	\
//		CLinkMap * pLinkMap 	= _pWorld->m_linkMap.GetLinkMap( _nLayer );																\
//		if( pLinkMap != NULL )																											\
//		{																																\
//			FLMapGrid::FLSyncIterator _it( pLinkMap->m_kMapGrid, _vPos );															\
//			FLMapGrid::FLTile * pTile = _it.GetNextTile();																					\
//			while( pTile != NULL )																											\
//			{																																\
//				for( map<OBJID, OBJID>::const_iterator __it = pTile->arrCtrlSightMap[ _dwLinkType ].begin(); __it != pTile->arrCtrlSightMap[ _dwLinkType ].end(); ++__it )	\
//				{																																							\
//					_pObj		= prj.GetCtrl( __it->first );																											
//
//#define END_LINKMAP									\
//				}									\
...
#define FOR_LINKMAP( _pWorld, _vPos, _pObj, _nRange, _dwLinkType, _nLayer ) {											\
	if( 0 <= _dwLinkType && _dwLinkType < MAX_LINKTYPE && _nRange >= 0 )																\
	{																													\
		CLinkMap * pLinkMap 	= _pWorld->m_linkMap.GetLinkMap( _nLayer );												\
		if( pLinkMap != NULL )																							\
		{																												\
			FLMapGrid::FLSyncIterator _it( pLinkMap->m_kMapGrid, _vPos, ( size_t )( _nRange ) );												\
			FLMapGrid::FLTile * pTile = _it.GetNextTile();																\
			while( pTile != NULL )																						\
			{																											\
			for( /*map< CCtrl *, CCtrl * >::const_iterator*/FLRing<CObj>::iterator __it = pTile->listCtrlSight[ _dwLinkType ].Begin()		\
					; __it != pTile->listCtrlSight[ _dwLinkType ].End()												\
					; ++__it )																							\
				{																										\
					_pObj		= __it._MyType();
					//_pObj		= __it->second;
...
#define FOR_LINKMAP_COLLISION( _pWorld, _vPos, _pObj, _nRange, _dwLinkType, _nLayer ) {											\
			if( 0 <= _dwLinkType && _dwLinkType < MAX_LINKTYPE )																\
			{																													\
				CLinkMap * pLinkMap 	= _pWorld->m_linkMap.GetLinkMap( _nLayer );												\
				if( pLinkMap != NULL )																							\
				{																												\
					...
					while( pTile != NULL )																						\
					{																											\
						for( std::vector< CObj * >::const_iterator __it = pTile->vecObjCollision.begin()						\
						; __it != pTile->vecObjCollision.end()																	\
						; ++__it )																								\
						{																										\
							_pObj		= *__it;

#define END_LINKMAP_COLLISION								\
						}									\
						pTile = _it.GetNextTile();			\
					}										\
				}											\
	...
	#define FOR_LINKMAP_COLLISION	FOR_LINKMAP				
	#define END_LINKMAP_COLLISION	END_LINKMAP
