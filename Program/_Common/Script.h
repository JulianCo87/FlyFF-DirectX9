...
public:
	...
	BOOL			LoadTxtDefines( const TCHAR* pszFileName, BOOL bMultiByte = TRUE, int nProcess = INTERPRITER );
	void			Free();
	void			PreScan();
	...
	BOOL			GetStringByStringKey( const std::string& key, CString& rValue );		//by gmpbigsun(20110831)
