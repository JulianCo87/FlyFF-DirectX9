...
void CEventLua::KeepConnectEventProcess()
{
	...
	for( int i = 0; i < nVecSize; ++i )
	{
		...
		for( vector<T_KEEP_CONNECT_ITEM>::iterator it = m_vecKeepConnectItem.begin(); it != m_vecKeepConnectItem.end(); ++it )
		{
			T_KEEP_CONNECT_ITEM tKeepConnectItem = { 0 };
			tKeepConnectItem = *it;
			FLItemElem itemElem;
...
float CEventLua::GetTroupeExpUpFactor( DWORD dwTroupeMemberCount )
{
	...
	for( vector<T_TROUPE_EXPUP_DATA>::const_iterator it = m_vecTroupeExpUpData.begin(); it != m_vecTroupeExpUpData.end(); ++it )
	{
