...
{
	int nchar = 0;
	if( ITEM_INDEX( 6370, II_SYS_SYS_EVE_A_CARD ) <= ItemId && ItemId <= ITEM_INDEX( 6395, II_SYS_SYS_EVE_Z_CARD ) )
		nchar = ItemId - ITEM_INDEX( 6370, II_SYS_SYS_EVE_A_CARD );

	return static_cast< char >('A' + nchar);
}

#ifdef __WORLDSERVER
int CMiniGame::Result_Kawibawibo( FLWSUser* pUser )
{
	FLItemElem* pItemElem = NULL;
	if( pUser->m_nKawibawiboState != KAWIBAWIBO_DRAW && pUser->m_nKawibawiboState != KAWIBAWIBO_WIN )
	{
		pItemElem	= (FLItemElem*)pUser->m_Inventory.GetAtByItemId( ITEM_INDEX( 26430, II_SYS_SYS_EVE_KAWIBAWIBO ) );
	...
	if( Kawibawibo.nItemCount != 0 )	// 아이템 갯수는 0일수 없음
	{
		if( pUser->m_nKawibawiboState != KAWIBAWIBO_DRAW && pUser->m_nKawibawiboState != KAWIBAWIBO_WIN )
		{
			...
			FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "KAWIBAWIBO_USE_ITEM" );
			aLogItem.WorldId = pUser->GetWorld()->GetID();
			aLogItem.Gold = aLogItem.Gold2 = pUser->GetGold();

			g_DPSrvr.OnLogItem( aLogItem, pItemElem, 1 );

			pUser->RemoveItem( pItemElem->m_dwObjId, 1 );		// 가위바위보 쿠폰 아이템 삭제
		}
		...
		if( pUser->m_nKawibawiboWin >= nKawiBawiBo_MaxWin )
		{
			...
			FLStrcpy( aLogItem.Action, _countof( aLogItem.Action ), "+" );
			FLStrcpy( aLogItem.SendName, _countof( aLogItem.SendName ), pUser->GetName() );
			FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "KAWIBAWIBO_LOST" );
		...
		if( (DWORD)( nRandom ) < Kawibawibo.dwDraw )	// Draw
		{
			...
			FLStrcpy( aLogItem.Action, _countof( aLogItem.Action ), "+" );
			FLStrcpy( aLogItem.SendName, _countof( aLogItem.SendName ), pUser->GetName() );
			FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ),  "KAWIBAWIBO_DRAW" );
		...
		FLStrcpy( aLogItem.Action, _countof( aLogItem.Action ), "+" );
		FLStrcpy( aLogItem.SendName, _countof( aLogItem.SendName ), pUser->GetName() );
		FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "KAWIBAWIBO_WIN" );
...
BOOL CMiniGame::ResultItem_Kawibawibo( FLWSUser* pUser )
{
	...
	if( Kawibawibo.nItemCount != 0 )	// 아이템 갯수는 0일수 없음
	{
		...
		if( pUser->CreateItem( &itemElem ) )
		{
			...
			FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "KAWIBAWIBO_PRIZE" );
		...
		else
		{
			...
			FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "KAWIBAWIBO_PRIZE_POST" );
...
void CMiniGame::OpenWnd_Reassemble( FLWSUser* pUser )
{
	pUser->AddReassembleOpenWnd( m_vecReassembleCard );
}

BOOL CMiniGame::Result_Reassemble( FLWSUser* pUser, OBJID* objItemId, int nCount )
{
	FLItemElem* pItemElem[9];
	// 일치하는지 검사 (인벤토리에서 검사)
	for( DWORD i=0; i<m_vecReassembleCard.size(); ++i )
	{
		...
		if( pItemElem[i]->GetProp()->dwID != m_vecReassembleCard.at( i ) )
			return FALSE;
	}
	...
	for( int i=0; i<nCount ; ++i )
	{
		...
		FLStrcpy( aLogItem.Action, _countof( aLogItem.Action ), "+" );
		FLStrcpy( aLogItem.SendName, _countof( aLogItem.SendName ), pUser->GetName() );
		FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "REASSEMBLE_USE" );
		aLogItem.WorldId = pUser->GetWorld()->GetID();
...
BOOL CMiniGame::ResultItem_Reassemble( FLWSUser* pUser )
{
...
//		aLogItem.RecvName = "REASSEMBLE_PRIZE";
		FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "REASSEMBLE_PRIZE" );
		pUser->AddReassembleResult( ItemGame.dwItemId, ItemGame.nItemCount );
	}
	else
	{
//		aLogItem.RecvName = "REASSEMBLE_PRIZE_POST";
		FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "REASSEMBLE_PRIZE_POST" );
...
CMiniGame::__ITEMGAME CMiniGame::GetReassembleRandomItem()
{
	...
	{
		temp = m_vecReassembleItem.at( i );
		nSum += temp.dwPer;
		if( nRandom < nSum )
			return temp;
	}

	return m_vecReassembleItem.at( 0 );
}

int CMiniGame::Result_Alphabet( FLWSUser* pUser, OBJID* objItemId, int nCount, int nQuestionID )
{
	// 페냐 검사..
//	if( pUser->GetGold() < m_nPayPenya )
	if( pUser->CheckUserGold( m_nPayPenya, false ) == false )
	{
	...
	for( int i=0; i<nCount; ++i )
	{
		pItemElem	= (FLItemElem*)pUser->m_Inventory.GetAtId( objItemId[i] );
		...
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "ItemNum - %d, %d" ), pItemElem->m_nItemNum, it->second );
			return ALPHABET_FAILED;
		}
	...
	char szTemp[32] = {0,};
	FLSPrintf( szTemp, _countof( szTemp ), "ALPHABET_USE_%s", strWord );
	...
	// 단어가 정답인지 확인
	__ALPHABET temp = m_vecAlphabet.at( nQuestionID );
...
{
	return m_vecAlphabet.at( nQuestionNum ).strWord[nPublicAlphabet-1];
}
...
CMiniGame::__ITEMGAME CMiniGame::GetAlphabetRandomItem( int nNum )
{
	...
	{
		alphabet = m_vecAlphabet.at( nNum );
		__ITEMGAME temp;
		for( int i=0; alphabet.vecItem.size(); ++i )
		{
			temp = alphabet.vecItem.at( i );
			nSum += temp.dwPer;
			if( nRandom < nSum )
				return temp;
		}
	}		
	return alphabet.vecItem.at( 0 );
}

void CMiniGame::OpenWnd_Alphabet( FLWSUser* pUser )
{
	int nRandom = xRandom( m_vecAlphabet.size() );
	int nPublicAlphabet = m_vecAlphabet.at( nRandom ).nPublic;
	pUser->AddAlphabetOpenWnd( nRandom, nPublicAlphabet-1, GetPublicCharacter( nRandom, nPublicAlphabet ), 
								m_vecAlphabet.at( nRandom ).strQuestion );
}

BOOL CMiniGame::ResultItem_Alphabet( FLWSUser* pUser, int nNum )
{
	// 아이템 지급 확률에 따라 아이템을 얻어온다.
	...
	if( pUser->CreateItem( &itemElem ) )
	{
		...
		FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "ALPABET_PRIZE" );
	...
	else
	{
		...
		FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "ALPABET_PRIZE_POST" );
...
void CMiniGame::OpenWnd_FiveSystem( FLWSUser* pUser )
{
...
void CMiniGame::DestroyWnd_FiveSystem( FLWSUser* pUser )
{
	int nPenya = 0;
	LogItemInfo aLogItem;
...
// 	if( nTotal < 0 )		// overflow?
	if( pUser->CheckUserGold( nPenya, true ) == false )
	{
		...
		FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "FIVESYSTEM_CANCEL_POST" );
		FLItemElem ItemElem;
	...
	else
	{
		...
		FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "FIVESYSTEM_CANCEL" );
	...
	aLogItem.kLogItem.nQuantity = nPenya;
	g_DPSrvr.OnLogItem( aLogItem );	
}
...
//	aLogItem.RecvName = "FIVESYSTEM_USE";
	FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "FIVESYSTEM_USE" );
	BOOL bAddGold = TRUE;
...
// 		if( nTotal < 0 )		// overflow?
		if( pUser->CheckUserGold( pUser->m_nBetFiveSystem[nBetNum] - nBetPenya, true ) == false )
		{
			FLItemElem ItemElem;
			...
			FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "FIVESYSTEM_USE_POST" );
			bAddGold = FALSE;
		}
	}
	...
	//aLogItem.ItemName = "SEED";
	FLSPrintf( aLogItem.kLogItem.szItemName, _countof( aLogItem.kLogItem.szItemName ), "%d", ITEM_INDEX( 12, II_GOLD_SEED1 ) );
	aLogItem.kLogItem.nQuantity = pUser->m_nBetFiveSystem[nBetNum] - nBetPenya;
	g_DPSrvr.OnLogItem( aLogItem );
...
int CMiniGame::Result_FiveSystem( FLWSUser* pUser )
{
	// 최고 입찰금 및 갯수
	int nMaxBet = 0;
	int nMaxCount = 0;
...
BOOL CMiniGame::ResultPenya_FiveSystem( FLWSUser* pUser, int nResult )
{
	...
	FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "FIVESYSTEM_PRIZE" );

// 	int nTotal = pUser->GetGold() + SetPenya;
// 	if( nTotal < 0 )		// overflow?
	if( pUser->CheckUserGold( SetPenya, true ) == false )
	{
		FLItemElem ItemElem;
		...
		FLStrcpy( aLogItem.RecvName, _countof( aLogItem.RecvName ), "FIVESYSTEM_PRIZE_POST" );
	}
	else
	...
	//aLogItem.ItemName = "SEED";
	FLSPrintf( aLogItem.kLogItem.szItemName, _countof( aLogItem.kLogItem.szItemName ), "%d", ITEM_INDEX( 12, II_GOLD_SEED1 ) );
	aLogItem.kLogItem.nQuantity = SetPenya;
	g_DPSrvr.OnLogItem( aLogItem );
...
int CMiniGame::GetKawibawiboYou( int nMy, int nResult )
{
	...
	{
		if( nMy == m_vecKawibawiboResult.at( i ).nMy && nResult == m_vecKawibawiboResult.at( i ).nResult )
			return m_vecKawibawiboResult.at( i ).nYou;
	}
	...
	while( s.tok != FINISHED )
	{
		...
		else if( s.Token == _T( "WORD" ) )
		{
			...
			if( it != mapQuestion.end() )
				...
				FLERROR_LOG( PROGRAM_NAME, _T( "존재하지 않는 문제 ID -> %s" ), s.Token );
	...
	while( s.tok != FINISHED )
	{
		if( s.Token == _T( "CARD" ) )
		{
			...
			while( *s.token != '}' )
			{
				...
				if( dwCard == -1 )
				{
					FLERROR_LOG( PROGRAM_NAME, _T( "\"CARD\" 아이템이 없는것을 설정했음" ) );
					return FALSE;
				}
				// 9개 이상 안들어 가도록..
		...
		else if( s.Token == _T( "ITEM" ) )
		{
			...
			while( *s.token != '}' )
			{
				...
				if( ItemGame.dwItemId == -1 )
				{
					FLERROR_LOG( PROGRAM_NAME, _T( "\"ITEM\" 아이템이 없는것을 설정했음" ) );
					return FALSE;
				}
				ItemGame.nItemCount = s.GetNumber();
				ItemGame.dwPer = s.GetNumber();
