...
void CCampus::Serialize( CAr & ar )
{
	...
	else
	{
		...
		for( int i = 0; i < (int)( nSize ); ++i )
		{
			...
			bool bResult = m_mapCM.insert( MAP_CM::value_type( pMember->GetPlayerId(), pMember ) ).second;
			if( bResult == false )
			{
				SAFE_DELETE( pMember );
			}
...
int CCampus::GetBuffLevel( u_long idPlayer )
{
	...
	if( IsMaster( idPlayer ) )
	{
		for( MAP_CM::iterator it = m_mapCM.begin(); it != m_mapCM.end(); ++it )
		{
			FLWSUser* pPupil = g_xWSUserManager->GetUserByPlayerID( ( it->second )->GetPlayerId() );
	...
	{
		FLWSUser* pMaster = g_xWSUserManager->GetUserByPlayerID( GetMaster() );
...
void CCampusMng::Serialize( CAr & ar )
{
	...
	else
	{
		...
		for( i = 0; i < (int)( nSize ); ++i )
		{
			...
			bool bResult = m_mapCampus.insert( MAP_CAMPUS::value_type( pCampus->GetCampusId(), pCampus ) ).second;
			if( bResult == false )
			{
				SAFE_DELETE( pCampus );
			}
...
u_long CCampusMng::AddCampus( CCampus* pCampus )
{
	...
	bool bResult = m_mapCampus.insert( MAP_CAMPUS::value_type( m_idCampus, pCampus ) ).second;
	if( bResult == false )
	{
		return 0;
	}
