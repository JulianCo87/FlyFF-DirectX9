...
CAr&  operator<<(CAr& ar, VOTE_INSERTED_INFO& info)
{
	...
	for( int i=0; i<MAX_VOTE_SELECT; ++i )
	{
...
{
	ar >> info.idGuild >> info.idVote;
	ar.ReadString( info.szTitle, _countof( info.szTitle ) );
	ar.ReadString( info.szQuestion, _countof( info.szQuestion ) );
	for( int i=0; i<MAX_VOTE_SELECT; ++i )
	{
		ar.ReadString( info.szSelections[i], _countof( info.szSelections[i] ) );
	}			
	return ar;
}
...
{
	FLASSERT( cbSelect < MAX_VOTE_SELECT );
	if( cbSelect < MAX_VOTE_SELECT )
	{
...
void CGuildVote::Serialize( CAr & ar )
{
	...
	else
	{
		...
		ar.ReadString( m_szQuestion, _countof( m_szQuestion ) );

		for( int i=0; i<MAX_VOTE_SELECT; ++i )
		{
			ar.ReadString( m_selects[i].szString, _countof( m_selects[i].szString ) );
			ar >> m_selects[i].cbCount;
		}	
...
BOOL CGuildTable::ReadBlock( CScript & script )
{
	...
	for( ; *script.token != '}' ; ++i )
	{
		...
		m_table[ i ].nMaxMember = static_cast< WORD >( script.GetNumber() );

		dwVal = script.GetNumber();
	}

	FLASSERT( i <= MAX_GUILD_LEVEL );
	m_nCount = i;
	return TRUE;
}

// 레벨업에 필요한 PXP공헌횟수  
DWORD CGuildTable::GetPxpCount(int nLevel) const
{
	FLASSERT( 1 <= nLevel && nLevel <= m_nCount );
	if( 1 <= nLevel && nLevel <= m_nCount )
	{
...
// 레벨업에 필요한 페냐 
DWORD CGuildTable::GetPenya(int nLevel) const 
{
	FLASSERT( 1 <= nLevel && nLevel <= m_nCount );
	if( 1 <= nLevel && nLevel <= m_nCount )
	{
...
// 최대인원수 
WORD CGuildTable::GetMaxMemeber( int nLevel) const
{
	FLASSERT( 1 <= nLevel && nLevel <= m_nCount );
	if( 1 <= nLevel && nLevel <= m_nCount )
	{
...
MEMPOOLER_IMPLEMENT( CGuildMember, 512 );
...
void CGuildMember::Serialize( CAr & ar )
{
	...
	else
	{
		...
		ar >> m_nSurrender;
		ar >> m_nClass;
		ar.ReadString( m_szAlias, _countof( m_szAlias ) );
	}
}
...
//________________________________________________________________________________
MEMPOOLER_IMPLEMENT( CGuild, 128 );
...
CGuild::CGuild()
{
	...
	m_GuildBank.SetItemContainer( ITYPE_ITEM, CONTAINER_TYPE_GUILD_BANK, MAX_GUILDBANK );
#endif
...
void CGuild::Serialize( CAr & ar, BOOL bDesc )
{
	...
	else
	{
...
#endif	// __CLIENT
		ar >> m_idGuild >> m_idMaster >> m_nLevel;
		ar.ReadString( m_szGuild, _countof( m_szGuild ) );
		ar >> m_dwLogo;
		ar >> m_nGoldGuild;
		ar >> m_nWin >> m_nLose >> m_nSurrender;
		if( !bDesc )
		{
			...
			ar.ReadString( m_szNotice, _countof( m_szNotice ) );	// 공지사항 
			ar >> m_dwContributionPxp;		// 공헌된 PXP
			ar >> m_nLevel;					// 레벨 
			ar >> m_idEnemyGuild;
...
void CGuild::SetContribution( CONTRIBUTION_CHANGED_INFO& info )
{
	...
	{
		FLTRACE_LOG( PROGRAM_NAME, _T( "Guild[%d] idPlayer[%d] not found" ), m_idGuild, info.idPlayer );
		return;
	}
...
void CGuild::DecrementMemberContribution( u_long idPlayer, DWORD dwPenya, DWORD dwPxpCount )
{
	...
	if( pMember == NULL )	// 멤버 탈퇴?
	{
		FLTRACE_LOG( PROGRAM_NAME, _T( "Guild[%d] idPlayer[%d] not found" ), m_idGuild, idPlayer );
		return;
	}
...
{
	FLASSERT( szNotice );
	FLStrncpy(m_szNotice, _countof( m_szNotice ), szNotice, MAX_BYTE_NOTICE);
}
...
//	m_nWinPoint	= source.m_nWinPoint;
	FLStrcpy( m_szGuild, _countof( m_szGuild ), source.m_szGuild );
...
BOOL CGuildMng::SetName( CGuild* pGuild, const char* szName )
{
	...
	FLStrcpy( pGuild->m_szGuild, _countof( pGuild->m_szGuild ), szName );	// set
	if( lstrlen( pGuild->m_szGuild ) > 0 )
...
//	locked
	...
	if( GetGuild( m_id ) )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "[ Exist Guild ID(%d) ]" ), m_id );
		return 0;
	}
	...
	if( bResult == false )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "[ Insert Failed. Guild ID(%d) ]" ), m_id );
		return 0;
	}
	if( lstrlen( pGuild->m_szGuild ) > 0 )
	{
...
void CGuild::SetQuest( int nQuestId, int nState )
{
	...
	if( !pQuest )
	{
		...
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "OVER MAX_GUILD_QUEST" ) );
			return;
		}
...
//	locked
	FLWSUser* pUser;
	CGuildMember* pMember;
	FLWSUser* pMaster	= NULL;
	if( bMasterAround )
	{
		pMaster	= (FLWSUser*)prj.GetUserByID( m_idMaster );
	...
	for( map<u_long, CGuildMember*>::iterator i = m_mapPMember.begin(); i != m_mapPMember.end(); ++i )
	{
		...
		if( IsValidObj( pUser ) )
		{
			...
			if( GetQuest( QUEST_INDEX( 1, QUEST_WARMON_LV1 ) ) != NULL ) // 클락워크 퀘스트면 비행 해제
				pUser->UnequipRide();
...
#ifndef __CORESERVER
FLItemElem* CGuild::GetItem( DWORD dwItemId, SERIALNUMBER iSerialNumber )
{
	for( DWORD i = 0; i < m_GuildBank.GetMax() ; i++ )
	{
		FLItemElem* pItemElem	= m_GuildBank.GetAt( i );
...
int CGuild::RemoveItem( DWORD dwId, int nItemNum )
{
	if( nItemNum < 0 )
	{
		return 0;
	...
	int nRemoved	= 0;
	FLItemElem* pItemElem	= m_GuildBank.GetAtId( dwId );
	if( pItemElem )
	{
