...
void CDPClient::OnQueryDestroyPlayer( CAr & ar, DPID dpid, LPVOID /*lpBuffer*/, u_long /*uBufSize*/ )
{
	...
	FLERROR_LOG( PROGRAM_NAME, _T( "idPlayer:%d GetPlayer return NULL." ), idPlayer );
...
// void CDPClient::SendBuyingInfo( PBUYING_INFO2 pbi2, CPlayer* pPlayer )
// {
// 	BEFORESENDSOLE( ar, PACKETTYPE_BUYING_INFO, pPlayer->GetSerial() );
...
void CDPClientArray::Free( void )
{
	...
	for( pRemove = pClient; pRemove != NULL; pRemove = pClient )
	{
...
BOOL	CDPClientArray::Connect( CServerDesc *pServer )
{
	...
	if( i == m_apServer.end() )
	{
		...
		if( pClient->ConnectToServer( pServer->m_szAddr, PN_WORLDSRVR + (u_short)pServer->GetKey(), FLStreamProtocol::GetInstance() ) == TRUE )
		{
