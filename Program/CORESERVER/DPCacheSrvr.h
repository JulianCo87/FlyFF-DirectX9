...
//	void	SendTeleportPlayer( u_long idOperator, CPlayer* pPlayer );
	...
	void	SendCaption( const CHAR* lpString, DWORD dwWorldId, BOOL bSmall, DWORD dwPartyId );
	...
	void	SendPacket( DWORD dwDPIDUser, DWORD dwDPIDCache, const FLPacket* pPacket );

	USES_PFNENTRIES( CDPCacheSrvr, ( CAr & ar, DPID, DPID, u_long ) );
//	Handlers
...
//	void	OnBuyingInfo( CAr & ar, DPID dpidCache, DPID dpidUser, u_long uBufSize );


private:
	DPID	m_dwClientSocketId;
};
