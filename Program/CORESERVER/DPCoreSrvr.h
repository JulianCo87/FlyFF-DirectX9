...
public:
	...
	map<u_long, DPID>	m_mapWorldKeyToDPID;	// WorldServer Key( ex. 101, 201.. equal g_uKey ) to DPID
	map<u_long, DPID>	m_mapWorldIDToDPID;		// WorldServer MultiID( ex. 1, 2.. equal m_uIdofMulti ) to DPID
...
public:
	...
	DPID getServerDPID(const u_long uKey) {
		auto it = m_mapWorldKeyToDPID.find(uKey);
		if (it == m_mapWorldKeyToDPID.end()) {
			return 0;
		}
		return it->second;
	}
...
public:
	...
	void	SendAddBot(const BYTE byAdd, const u_long idBot, std::string& strBotName);
	void	OnAddBot(CAr& ar, DPID dpid, DPID, DPID, u_long);
	void	OnRemoveBot(CAr& ar, DPID dpid, DPID, DPID, u_long);
#else
	void	sendAddBot(const u_long idBot, std::string& strBotName, const u_long uSrvKey, const std::size_t nBotType);
	void	sendRemoveBot(const u_long idBot, const u_long uSrvKey);
#endif
#endif
...
{
	map<u_long, DPID>::iterator i	= m_mapWorldKeyToDPID.find( uWorldSrvr );
...
{
	map<u_long, DPID>::iterator i	= m_mapWorldIDToDPID.find( uIdofMulti );
