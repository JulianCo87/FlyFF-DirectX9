...
#ifdef __M_USER_BOTS
	ON_MSG(PACKETTYPE_ADD_BOT, &CDPCoreSrvr::OnAddBot);
	ON_MSG(PACKETTYPE_REMOVE_BOT, &CDPCoreSrvr::OnRemoveBot);
#endif
#endif
	m_nGCState = 1;
}
...
void CDPCoreSrvr::OnAddConnection( CAr & ar, DPID dpid, DPID, DPID, u_long )
{
	...
	if( i != m_apSleepServer.end() )
	{
		...
		FLASSERT( bResult );
		bResult	= m_mapWorldKeyToDPID.insert( map<u_long, DPID>::value_type( uWorldSrvr, dpid ) ).second;
		FLASSERT( bResult );
		const u_long uIdofMulti	= uWorldSrvr / 100;
		bResult	= m_mapWorldIDToDPID.insert( map<u_long, DPID>::value_type( uIdofMulti, dpid ) ).second;
		FLASSERT( bResult );
...
// 		g_GuildMng.m_AddRemoveLock.Leave();
		...
		g_dpDatabaseClient.sendServerConnected(uWorldSrvr);
#endif
...
//		SendUnify( (u_short)uWorldSrvr );
		...
		CMclAutoLock lockPlayer(g_PlayerMng.m_AddRemoveLock);
		g_PlayerMng.removeAllServerPlayer(uWorldSrvr);
#endif

#ifdef __M_USER_BOTS_DB_BASE
		g_dpDatabaseClient.sendServerDisconnected(uWorldSrvr);
#endif
	}
}
...
void CDPCoreSrvr::OnJoin( CAr & ar, DPID dpid, DPID, DPID, u_long )
{
	...
	if (idPlayer >= INT_MAX) {
		// numbers in the front of the account-name are not allowed.
	//	if (g_PlayerMng.AddPlayer(idPlayer, szPlayer, "1337server") == FALSE) 
		{
		//	Error("CORE: %s: couldnt add bot: %d, %s", __FUNCTION__, idPlayer, szPlayer);
			return;
		}
	}
#endif
...
#ifdef __M_USER_BOTS
#ifndef __M_USER_BOTS_DB_BASE
void CDPCoreSrvr::OnRemoveBot(CAr& ar, DPID dpid, DPID, DPID, u_long) {
	// remove a bot from core server 
	u_long idBot = 0;
	std::string strName;
	ar >> idBot >> strName;
	...
	auto pPlayer = g_PlayerMng.GetPlayer(idBot);
	if (pPlayer == nullptr) {
		pPlayer = pPlayer = g_PlayerMng.getPlayer(strName);
		if (pPlayer == nullptr) {
			return;
		}
	}
	g_PlayerMng.RemovePlayer(pPlayer, FALSE);
}
void CDPCoreSrvr::OnAddBot(CAr& ar, DPID dpid, DPID, DPID, u_long) {
	u_long idBot = 0;
	std::string strName;
	BYTE byAdd = 0;
	ar >> idBot >> strName;
	...
	pPlayer = g_PlayerMng.getPlayer(strName);
	if (pPlayer != nullptr) {
		byAdd = 2;
	}
	SendAddBot(byAdd, idBot, strName);
}
void	CDPCoreSrvr::SendAddBot(const BYTE byAdd, const u_long idBot, std::string& strBotName) {
	// send add bot
	BEFORESENDDUAL(ar, PACKETTYPE_ADD_BOT, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << byAdd << idBot;
	ar.WriteString(strBotName);
	SEND(ar, this, DPID_ALLPLAYERS);
}
#else // __M_USER_BOTS_DB_BASE
void	CDPCoreSrvr::sendAddBot(const u_long idBot, std::string& strBotName,
	const u_long uSrvKey, const std::size_t nBotType) {


	CMclAutoLock	Lock(g_PlayerMng.m_AddRemoveLock);
	auto pPlayer = g_PlayerMng.GetPlayer(idBot);
	if (pPlayer != nullptr || g_PlayerMng.getPlayer(strBotName) != nullptr) {
		FLERROR_LOG(PROGRAM_NAME, "Bot duplicated: name %s, id %u", strBotName.c_str(), idBot);
		return;
	}

	DPID dpidServer = this->getServerDPID(uSrvKey);
	if (dpidServer == 0) {
		return;
	}
	// send add bot
	...
	ar.WriteString(strBotName);
	ar << nBotType;
	SEND(ar, this, dpidServer);

	g_PlayerMng.AddPlayer(idBot, strBotName.c_str(), BOT_ACCOUNT_NAME, uSrvKey);
}
void	CDPCoreSrvr::sendRemoveBot(const u_long idBot, const u_long uSrvKey) {
	DPID dpidServer = this->getServerDPID(uSrvKey);
	if (dpidServer == 0) {
		return;
	}
	BEFORESENDDUAL(ar, PACKETTYPE_REMOVE_BOT, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << idBot;
	SEND(ar, this, dpidServer);
	CMclAutoLock	Lock(g_PlayerMng.m_AddRemoveLock);
#ifdef __M_USER_BOTS_RELOAD_CONFIG
	if (idBot == 0) {
		g_PlayerMng.removeAllServerPlayer(uSrvKey, true);
	}
	else {
		auto pPlayer = g_PlayerMng.GetPlayer(idBot);
		if (pPlayer != nullptr) {
