...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   if (!hWnd)
   {
	...
	if( !g_dpCoreSrvr.StartServer( g_szWorldBindIP, PN_CORESRVR + 0, FLStreamProtocol::GetInstance() ) )
		return FALSE;

	if( !g_DPCacheSrvr.StartServer( g_szCacheBindIP, PN_CORESRVR + 1, FLStreamProtocol::GetInstance() ) )
		return FALSE;

	if( !g_dpLoginSrvr.StartServer( g_szLoginBindIP, PN_CORESRVR + 2, FLStreamProtocol::GetInstance() ) )
		return FALSE;

	if( !g_dpDatabaseClient.ConnectToServer( szDataBaseAddr, PN_DBSRVR_2, FLStreamProtocol::GetInstance(), FALSE ) )
	{
