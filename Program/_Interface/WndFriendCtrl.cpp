...
void CWndFriendCtrl::SetSearch( int nSelectServer, int nSex, LPCTSTR strId )
{
	...
	m_nSex = nSex;
	FLStrcpy( m_szId, _countof( m_szId ), strId );
}
...
{ 
	CPlayerDataCenter*	pPlayerDataCenter	= CPlayerDataCenter::GetInstance();		//sun: 11, 캐릭터 정보 통합
	...
	case 6: // 쪽지 보내기
		{
			...
			if( pFriend )
			{
				...
				FLStrcpy( g_WndMng.m_pWndMessageNote->m_szName, _countof( g_WndMng.m_pWndMessageNote->m_szName ), pPlayerDataCenter->GetPlayerString( idPlayer ) );
