...
//		m_wndMenuPlace.CheckMenuItem( 2, FALSE );
		...
		case 0: // 숨기기 
			...
			break;
		// gmpbigsun( 20101202 ) : 가이드 보기 메뉴 삭제
		//case 1: // 보기
		//	if( m_wndGuideText->m_VecGuideText.size() )
		//		m_wndGuideText->m_bVisible = TRUE;
...
void CWndGuideSystem::OnInitialUpdate()
{
	...
	if( g_pPlayer )
	{
		...
		FLERROR_LOG( PROGRAM_NAME, _T( "g_pPlayer == NULL" ) );
	...
	m_wndMenuPlace.AppendMenu( 0, 1 , prj.GetText(TID_GAME_VIEW_TUTORIAL) );
	m_wndMenuPlace.CheckMenuItem( 0, FALSE );
	m_wndMenuPlace.CheckMenuItem( 1, FALSE );
...
{ 
	...
	FLERROR_ASSERT_LOG_RETURN_VOID( pWndText, _T("[윈도우 가이드 텍스트 메니져] 윈도우 텍스트 포인터가 유효하지 않습니다.") );
	m_Rect[0] = pWndText->GetWndRect();

	CWndButton* pWndButtonBack = (CWndButton*)GetDlgItem( WIDC_BACK );
	FLERROR_ASSERT_LOG_RETURN_VOID( pWndButtonBack, _T("[윈도우 가이드 텍스트 메니져] 윈도우 버튼 포인터가 유효하지 않습니다.") );
	...
	FLERROR_ASSERT_LOG_RETURN_VOID( pWndButtonNext, _T("[윈도우 가이드 텍스트 메니져] 윈도우 버튼 포인터가 유효하지 않습니다.") );
	...
	FLERROR_ASSERT_LOG_RETURN_VOID( pWndButtonClose, _T("[윈도우 가이드 텍스트 메니져] 윈도우 버튼 포인터가 유효하지 않습니다.") );
	...
	m_pTextureBG = m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"GuideBG.tga" ), 0, TRUE );
} 
