...
//				
//			str += strEnter;
			strTemp.Format( prj.GetText( TID_TOOLTIP_USE ), pItemProp->szCommand );	// 용도 :
			str += strTemp;
...
void CWndCommItemCtrl::DrawSkill( C2DRender* p2DRender, CPoint* pPoint, int x, int &nScroll )
{
	...
	for( BuffItr it1 = g_pPlayer->m_buffs.m_listBuffs.begin(); it1 != g_pPlayer->m_buffs.m_listBuffs.end(); ++it1 )
	{
		...
		{
			//gmpbigsun( 20111028 ) : 버프내용 그대로 다 보여주는걸로 수정됨. #20187
			bool	bRenderBuff = false;
			//bRenderBuff = IsRenderBuff( (WORD)pMainProp->dwID );	// 중복검사 제거
...
//sun: 13, 하우징 시스템
		...
		{
			//gmpbigsun( 20111109, #20187 ) : 각 아이템별로 시간 설정
			//기존엔 중복되는 버프는 한개만 출력하고있었고, 그래서 종복되는녀석들만 따로 선별해서 소팅하는 코드가 있었으나 제거함.
			//중복 여부에 관계없이 모든 버프 출력.
			DWORD dwTimeThis = 0;
			...
			CTimeSpan ct( (long)(dwTimeThis) );		// 남은시간을 초단위로 변환해서 넘겨줌
			...
			strTemp.Format( prj.GetText( TID_MMI_BUFF_TIME ), ct.GetDays(), ct.GetHours(), ct.GetMinutes(), ct.GetSeconds() );	// 지속시간 : 
			str += strTemp;
			string	= str;	
		}
		else
...
//					if( pItemElem->m_dwKeepTime && !pItemElem->IsFlag( FLItemElem::expired ) )
			...
			else
			{
 				...
 				if( pMainProp->dwSkillTime >= 0x3b9ac9ff )		//gmpbigsun : (999999999) 시간제한 없음 
 				{
...
void	CWndCommItemCtrl::InsertRenderBuffList( WORD wRendBuffID )
{
	...
	if( bRenderBuff == true )
	{
		//gmpbigsun( 20111109 ) : 중복해서 들어올수있음.
