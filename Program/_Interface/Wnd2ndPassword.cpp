...
#ifdef _DEBUG	//gmpbigsun( 20100812 ) : 디버그환경에서는 그냥 입력받자.
	pWndEdit->DelWndStyle( EBS_READONLY );
	pWndEdit->SetFocus(); 
#endif 
}
//-----------------------------------------------------------------------------
	...
	case WIDC_BUTTON_OK:
		{
...
#ifdef _DEBUG	//gmpbigsun: 2차비번 키보드 입력받게
			...
			while( 1 )
			{
				...
				if( str2ndPasswordNumber.GetLength() != 4 )
					break;
				...
				for( int i = 0; i < 4; ++i )
				{
					temp2 = str2ndPasswordNumber.GetAt(i);
					BYTE temp = atoi( temp2 );
					for( int j = 0; j < 10; ++j )
					{
						if( m_byPassTable[j] == temp )
						{
							FLIntToString( j, szTemp, _countof( szTemp ), 10 );	//arg3: 진법
							strResult.Insert( strResult.GetLength(), szTemp );
							break;
						}
					}
				...
				str2ndPasswordNumber = strResult;
...
// PASSWORD_RESET_2ND 옵션 들어간 후 여기로 변경
...
// PASSWORD_RESET_2ND
			...
			if ( strcmp( g_Neuz.m_strNewPasswordNumber, _T( "" ) ) && g_Neuz.m_nResetPassword[m_nSelectCharacter] == 0 )
			{
				if ( strcmp( g_Neuz.m_strNewPasswordNumber, str2ndPasswordNumber ) == 0 )
				{
					g_Neuz.m_nResetPassword[m_nSelectCharacter] = 1;
					...
					for( int i = 0; i < 4; ++i )
					{
						temp2 = str2ndPasswordNumber.GetAt(i);
						BYTE temp = atoi( temp2 );
						FLIntToString( m_byPassTable[temp], szTemp, _countof( szTemp ), 10 );	//arg3: 진법
						strNewPassword.Insert( strNewPassword.GetLength(), szTemp );
					}
					str2ndPasswordNumber = strNewPassword;
					
					//g_DPlay.SendChangeBankPass( "0000", str2ndPasswordNumber, NULL_ID, 0 );
					g_dpLoginClient.SendChangeBankPass( g_Neuz.m_apPlayer[m_nSelectCharacter]->GetName(), str2ndPasswordNumber, g_Neuz.m_apPlayer[m_nSelectCharacter]->m_idPlayer );
					Destroy();
					break;
				}
				else
				{
					...
					g_WndMng.OpenMessageBox( prj.GetText( TID_GAME_NEW_PASSWORD_INPUT_ERROR ) );	// 새로운 2차 비밀번호가 동일하지 않습니다. 다시 입력하여 주십시오.
				}
				g_Neuz.m_strNewPasswordNumber = "";
			}


			if ( g_Neuz.m_nResetPassword[m_nSelectCharacter] )
			{
	...
	/*
					...
					*/ // 위로 이동
					...
					// test start
					/*m_nSelectCharacter = 2;
					pWndSelectChar->m_nSelectCharacter = 2;
					g_Neuz.m_apPlayer[2] = new CMover;
					memcpy ( g_Neuz.m_apPlayer[2], g_Neuz.m_apPlayer[1], sizeof(CMover) );
					g_Neuz.m_apPlayer[2]->m_idPlayer = 58;
					wsprintf(g_Neuz.m_apPlayer[2]->m_szName,"test3333");*/
					//test end
			...
			{
				if ( strcmp( g_Neuz.m_strNewPasswordNumber, _T( "" ) ) == 0 )
					g_Neuz.m_strNewPasswordNumber = str2ndPasswordNumber;
				pWndEdit->SetString ("");
				SetTitle(prj.GetText(TID_GAME_NEW_PASSWORD_REINPUT));

				CWndText* pWndText = (CWndText*)GetDlgItem( WIDC_TEXT_COMMENT );
	
				pWndText->m_string.SetString( prj.GetText(TID_GAME_NEW_PASSWORD_REINPUT_MSG)  );
...
#ifdef PASSWORD_RESET_2ND
	if ( g_Neuz.m_nResetPassword[m_nSelectCharacter] == 0)
	{
		SetTitle(prj.GetText(TID_GAME_NEW_PASSWORD_INPUT));
		g_Neuz.m_strNewPasswordNumber = "";
		CWndText* pWndText = (CWndText*)GetDlgItem( WIDC_TEXT_COMMENT );
		pWndText->m_string.SetString( prj.GetText(TID_GAME_NEW_PASSWORD_INPUT_MSG)  );
		pWndText->ResetString();
	}
#endif // PASSWORD_RESET_2ND
...
void CWnd2ndPassword::ResetNumberpad( u_long idNumberPad )
{
	...
	for( int i = 0; i < TABLE_NUMBER_X; ++i )
	{
		...
		pWndButton->SetTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), strKeyNumber[ i ] ), TRUE );
	}
#ifdef _DEBUG
	memcpy( &m_byPassTable, &byNumberTable[idNumberPad], sizeof(BYTE) * TABLE_NUMBER_X );
#else
#ifdef PASSWORD_RESET_2ND
	memcpy( &m_byPassTable, &byNumberTable[idNumberPad], sizeof(BYTE) * TABLE_NUMBER_X );
#endif // PASSWORD_RESET_2ND
