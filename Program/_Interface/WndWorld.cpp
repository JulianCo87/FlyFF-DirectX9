										...
										CPoint ptGName, CPoint ptHypoon, CPoint ptState, float fLogoScaleX, float fLogoScaleY, CRect rectBg)
{
	...
	// fixed 20120109
	int nMax = 0;
	map<int, int>::iterator iter = CSecretRoomMng::GetInstance()->m_mapMonsterNum.find(stGuildInfo.nWarState);
...
//sun: 11, <레벨??> 삭제, 채팅창 수정, 페냐거래 취소버튼 삭제
					...
					else
					{
						...
						if( g_xFlyffConfig->GetMainLanguage() == LANG_THA && pMover->GetIndex() == OBJECT_INDEX( 199, MI_MRPUMPKIN5 ) )	// 태국의 경우	// 괴수 미스터펌킨은
							bViewLevel = FALSE;
	...
	for( i=0; i<prj.m_aPartySkill.GetSize(); i++ )
	{
		...
		if( pSkillProp )
		{
			if( pSkillProp->dwSkillTime != -1 )
			{
				...
				m_pBuffTexture[1].insert( make_pair(pSkillProp->dwID, buffskill) );
			}
		}
	}
...
//sun: 9차 전승관련 Clienet
	...
	if( g_WndMng.GetWndBase(APP_SMELT_JEWEL_NEW) )
		return;
#endif
	if( GetBuffIconRect( ITEM_INDEX( 10469, II_SYS_SYS_SCR_RETURN ), point ) )	// 귀환의 두루마리 아이콘을 클릭하면 더블클릭까지 검사 
		return;
	if( GetBuffIconRect( ITEM_INDEX( 26202, II_SYS_SYS_SCR_PARTYSUMMON ), point ) )	// 귀환의 두루마리 아이콘을 클릭하면 더블클릭까지 검사 
		return;
...
//sun: 11, 타겟 표시 방법 변경
	...
	if( GetBuffIconRect( ITEM_INDEX( 26202, II_SYS_SYS_SCR_PARTYSUMMON ), point ) ) // "단장의 부름" 버프 더블클릭
	{
...
//sun: 9, 9-10차 펫
	if( GetBuffIconRect( ITEM_INDEX( 21035, II_SYS_SYS_SCR_PET_FEED_POCKET ), point ) )	// II_SYS_SYS_SCR_PET_FEED_POCKET 버프 더블 클릭
...
{
	//RemoveButton( );
	for( vector<BUTTON_INFO>::iterator iter = m_vecButton.begin(); iter != m_vecButton.end(); ++iter )
	{
...
//sun: 13, 레인보우 레이스
	...
	if( g_WndMng.GetWndBase(APP_SMELT_JEWEL) )	//sun: 9차 전승관련 Clienet
		return FALSE;
