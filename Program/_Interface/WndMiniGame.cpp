			...
			case WIDC_BTN_START:
				{
					if(m_bCheckCoupon && g_pPlayer->IsPlayer() && 
						g_pPlayer->m_Inventory.GetAtByItemId( ITEM_INDEX( 26430, II_SYS_SYS_EVE_KAWIBAWIBO ) ) == NULL)
					{
...
{ 
	if(m_dwItemID != -1 && m_dwNextItemID != -1)
	{
		...
		pItemProp = g_xSpecManager->GetSpecItem( m_dwItemID );
		if(pItemProp != NULL)
		{
		...
		pItemProp = g_xSpecManager->GetSpecItem( m_dwNextItemID );
		if(pItemProp != NULL)
		{
			...
			{
				TCHAR szTemp[ 32 ];
				FLSPrintf( szTemp, _countof( szTemp ), prj.GetText(TID_GAME_KWAIBAWIBO_PRESENT_NUM), m_nNextItemCount );
				CSize size = m_p2DRender->m_pFont->GetTextExtent( szTemp );
				m_p2DRender->TextOut( point.x + 36 - size.cx, point.y + 48 - size.cy, szTemp, 0xff0000ff );
			}
		}
...
{ 
	...
	DWORD dwVersion = _GetContentState( CT_NEWUI_19 );

	if( CS_VER1 == dwVersion )	{
		m_wndButton1.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(), "ButtOk.tga" ) );
		m_wndButton2.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(), "ButtCancel.tga" ) );
	}else
	if( CS_VER2 == dwVersion ) {
...
{
	m_nPublic = nPublic;
	m_firstwordID = nChar - 'A' + ITEM_INDEX( 6370, II_SYS_SYS_EVE_A_CARD );
	m_bGetFirst = TRUE;				
}
...
void CWndFindWordGame::OnDraw( C2DRender* p2DRender ) 
{
	...
	if(m_bGetFirst)
	{
		pItemProp = g_xSpecManager->GetSpecItem( m_firstwordID );
		if(pItemProp != NULL)
		{
			pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pItemProp->szIcon), 0xffff00ff );
			if(pTexture != NULL)
...
BOOL CWndFindWordGame::OnDropIcon( LPSHORTCUT pShortcut, CPoint point )
{
	...
	if(pItemElem != NULL && (pItemElem->GetProp()->dwID >= ITEM_INDEX( 6370, II_SYS_SYS_EVE_A_CARD ) && pItemElem->GetProp()->dwID <= ITEM_INDEX( 6395, II_SYS_SYS_EVE_Z_CARD )))
	{
...
void CWndFindWordGame::SetWord(FLItemElem* pItemElem)
{
	int nNum = -1;
	for(int i=0; i<5; i++)
	{
...
{ 
	...
	{
		m_wndButton.Create( _T(""), 0, rect1, this, IDOK );
		m_wndButton.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"ButtOk.tga" ) );
	...
	if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
	{
		...
		m_wndButton.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"ButtNormal00.tga" ) );
		m_wndButton.FitTextureSize();
	}
...
{ 
	...
	for(int i=0; i<9; i++)
	{
		if(m_pItemElem[i] != NULL)
		{
			pItemProp = g_xSpecManager->GetSpecItem( m_itemID[i] );
			if(pItemProp != NULL)
			{
...
void CWndPuzzleGame::SetPicture(FLItemElem* pItemElem)
{
	//if( g_xFlyffConfig->GetMainLanguage() == LANG_KOR )
	{
		int nNum = 0;
