...
#ifdef __M_USER_BOTS
#include "UserBot.h"
BOOL TextCmd_ExecBotCmd(CScanner& scanner) {
#ifdef __WORLDSERVER
	auto pUser = reinterpret_cast<FLWSUser*>(scanner.dwValue);
	if (IsValidObj(pUser) == FALSE) {
		return FALSE;
	}

	scanner.GetToken();
	u_long idBot = CPlayerDataCenter::GetInstance()->GetPlayerId(scanner.token);
	if (idBot == 0) {
		pUser->AddTextEx("Bot %s doesnt exist.", scanner.token);
		return FALSE;
	}

	auto pBot = user_bot::CBotMng::getInstance()->getBot(idBot);
	if (pBot == nullptr) {
		pUser->AddTextEx("Bot %s doesnt exist.", scanner.token);
		return FALSE;
	}
	...
	scanner.GetToken();
	std::string strBehaviour = scanner.token;
	if (strBehaviour.size() < 3) {
		pUser->AddTextEx("Command \"%s\" isnt valid. 1", strBehaviour.c_str());
		return FALSE;
	}
#ifndef __M_USER_BOTS_MULTIPLE_LOGIN_DAILY
	auto pBehaviour = pBot->getBotType()->findBehaviour(strBehaviour);
	if (pBehaviour == nullptr) {
		// just exec once
		pBot->getBotType()->addBehaviour(strBehaviour, 0);
		pBehaviour = pBot->getBotType()->findBehaviour(strBehaviour);
		if (pBehaviour == nullptr) {
			pUser->AddTextEx("Command \"%s\" isnt valid. 2", strBehaviour.c_str());
			return FALSE;
		}
	}
#else
	std::size_t nIndex = 0;
	auto pvecBehaviour = pBot->getBotType()->getAllBehaviours(&nIndex);
	auto pBehaviour = &(*pvecBehaviour)[nIndex + 1];
	for (auto& it : (*pvecBehaviour)) {
		// just exec once
		pvecBehaviour->push_back({ strBehaviour,0 });
		pBehaviour = pBot->getBotType()->findBehaviour(nIndex, strBehaviour);
		if (pBehaviour == nullptr) {
			pUser->AddTextEx("Command \"%s\" isnt valid. 2", strBehaviour.c_str());
			return FALSE;
		}
...
#endif
	// move to
	if (pBehaviour->str == "move") {
		D3DXVECTOR3 vPos;
		vPos.x = scanner.GetNumber(); // x
		vPos.y = scanner.GetNumber();  // y
		vPos.z = scanner.GetNumber(); // z

		if (vPos.x == 0 && vPos.y == 0 && vPos.z == 0) {
			//	pUser->AddText("Not valid position for this world.");
			vPos = pUser->GetPos();
		}
		...
		pBot->setDstPosition(vPos);
		return FALSE;
	}

	if (user_bot::CBotMng::getInstance()->postMsg(*pBehaviour, pBot) == false) {
		pUser->AddTextEx("Bot \"%s\" couldnt execute command: %s", pBot->GetName(), strBehaviour.c_str());
		return FALSE;
	}
#endif
	return TRUE;
...
#ifdef __PROFESSION
BOOL TextCmd_Profession( CScanner& scanner )
{
#ifdef __WORLDSERVER
	FLWSUser* pUser = (FLWSUser*)scanner.dwValue;
	if( IsValidObj( pUser ) )
	{
		pUser->m_nProfessionLevel = prj.m_profession.nMaxProfessionLevel;
...
BOOL TextCmd_DisplayShutdownList( CScanner & s )
{
	...
	{
		g_DPCoreClient.SendSetSnoop( pUser->m_idPlayer, pUser->m_idPlayer, FALSE );
		for( map< u_long /*playerID*/, CTime >::const_iterator it = FLShutdownRule_Mng::GetInstance().m_mapDoubtUser.begin(); it != FLShutdownRule_Mng::GetInstance().m_mapDoubtUser.end(); ++it )
		{
...
#ifdef __M_USER_BOTS
	ON_TEXTCMDFUNC(TextCmd_ExecBotCmd, "botcmd", "bc", "", "", TCM_SERVER, AUTH_ADMINISTRATOR, "")
#endif
#ifdef __PROFESSION
	ON_TEXTCMDFUNC( TextCmd_Profession, "Profession", "profession", "", "", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
#endif // __PROFESSION	
