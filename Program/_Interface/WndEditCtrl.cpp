...
CIMEMgr::CIMEMgr()
{
	...
	if( GetSystemDirectoryA( szPath, MAX_PATH+1 ) ) {
		...
		FLStrcat( szPath, _countof( szPath ), "\\imm32.dll" );
...
void CIMEMgr::Composition(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	if(hIMC) {
		if(lParam&GCS_COMPSTR) {
			...
			wchar_t* temp = (wchar_t*)alloca(compSize);

			ImmGetCompositionStringW(hIMC, GCS_COMPSTR, temp, compSize);

			m_composition.assign(temp, temp+compSize/sizeof(wchar_t));
		}
		if(lParam&GCS_RESULTSTR) {

			int resultSize = ImmGetCompositionStringW(hIMC, GCS_RESULTSTR, NULL, 0);

			wchar_t* temp = (wchar_t*)alloca(resultSize);

			ImmGetCompositionStringW(hIMC, GCS_RESULTSTR, temp, resultSize);

			m_result.assign(temp, temp+resultSize/sizeof(wchar_t));
		}
		if(lParam&GCS_COMPATTR) {

			int attrSize = ImmGetCompositionStringW(hIMC, GCS_COMPATTR, NULL, 0);

			BYTE* temp = (BYTE*)alloca(attrSize);

			ImmGetCompositionStringW(hIMC, GCS_COMPATTR, temp, attrSize);

			m_attr.assign(temp, temp+attrSize);
		}
...
{
	return ConvertString(m_codePage, m_result.c_str(), m_result.size(), text, len);
}
/*----------------------------------------------------------------------------*/ 
int		CIMEMgr::GetCompositon(char* text, int len)
{
	...
	if( n > 0 )
		return ConvertString(m_codePage, m_composition.c_str(), n, text, len);
	else
		return 0;
}
/*----------------------------------------------------------------------------*/ 
...
{
	int attrSize = WideCharToMultiByteEx(m_codePage, 0, m_composition.c_str(), m_composition.size(), NULL, 0, NULL, NULL);
...
{
	return	ConvertString(m_codePage, m_composition.c_str(), min(m_cursorPos, (int)( m_composition.size() )), NULL, 0);
}
/*----------------------------------------------------------------------------*/ 
...
bool CIMEMgr::GetReadingWindowOrientation()
{
    ...
    if( !bHorizontalReading && ( m_dwId[0] & 0x0000FFFF ) == LANG_CHT )
    {
        ...
        FLStrcpy( szRegPath, _countof( szRegPath ), "software\\microsoft\\windows\\currentversion\\" );
        FLStrcat( szRegPath, _countof( szRegPath ), ( dwVer >= MAKEIMEVERSION( 5, 1 ) ) ? "MSTCIPH" : "TINTLGNT" );
...
{ 
	...
	FLTRACE_LOG( PROGRAM_NAME, _T( "Candlist ----------------------" ) );
	FLTRACE_LOG( PROGRAM_NAME, _T( "Count %d" ), candidateList.dwCount );
	FLTRACE_LOG( PROGRAM_NAME, _T( "Selection %d" ), candidateList.dwSelection );
	FLTRACE_LOG( PROGRAM_NAME, _T( "PageStart %d" ), candidateList.dwPageStart );
	FLTRACE_LOG( PROGRAM_NAME, _T( "PageSize %d" ), candidateList.dwPageSize );
...
{ 
	...
	{
		char buf[255];
		FLSPrintf(buf, _countof( buf ), "%d:", y + 1);
		p2DRender->TextOut( 2, 2 + y * nMaxHeight, buf, m_dwColor );//, lstrlen( buf ) );
    }
...
void CWndHCandList::UpdateCandList(CPoint windowPos)
{
	...
	for( int i = candidateList.dwPageStart; i < (int)( dwPageEnd ); i++ ) 
	{
		int len = 0;
		char buf[255];
		len += FLSPrintf(buf, _countof( buf ), "%d:", i-candidateList.dwPageStart+1);
...
void CWndHCandList::OnDraw( C2DRender* p2DRender )
{
	...
	for(int i=0; i<(int)( m_candidate.size() ); ++i)
	{
		...
		p2DRender->TextOut(2+m_maxWidth*i, 2, text, m_dwColor );
	}
...
void CReadingList::UpdateReading(CPoint windowPos)
{
	...
	g_imeMgr.GetReading(temp, len);
	m_reading.assign( temp, len );
	...
	{
		SIZE size;
		m_pFont->GetTextExtent(m_reading.c_str(), &size, g_imeMgr.m_codePage);
...
void CReadingList::OnDraw( C2DRender* p2DRender )
{
	...
	const char* begin = m_reading.c_str();
	const char* end = begin + m_reading.size();
	...
	if(g_imeMgr.IsVerticalReading())
	{
		...
		while( *begin && begin < end )
		{
			...
			p2DRender->TextOut(x + (16-size.cx)/2, y + (16-size.cy)/2, text, m_dwColor );
	...
	{
		p2DRender->TextOut( 2, 1, m_reading.c_str(), m_dwColor );
	}
...
void CWndEdit::OnSetFocus(CWndBase* pOldWnd)
{
	...
	const PFLSHORTCUTKEY_CONTROL pShortcutKey = g_xKeyManager->GetShortcutKey_Control();

	g_bKeyTable[ pShortcutKey->KeyUp 	] = FALSE;
	g_bKeyTable[ pShortcutKey->KeyDown	] = FALSE;	
	g_bKeyTable[ pShortcutKey->KeyLeft	] = FALSE;
	g_bKeyTable[ pShortcutKey->KeyRight	] = FALSE;
	g_bKeyTable[ pShortcutKey->KeyJump	] = FALSE;
...
{
	//gmpbigsun: 타일방식에서 부모의 텍스쳐가 지정이 안되어있을때 
...
{
	DWORD dwFillColor( WNDFONTCOLOR_BLACK );
	DWORD dwTextColor( WNDFONTCOLOR_WHITE );
	DWORD dwCompositionTextColor( WNDFONTCOLOR_WHITE );

	if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
	{
		dwFillColor				= m_dwColor;
		dwTextColor				= m_dwColor;
		dwCompositionTextColor	= WNDFONTCOLOR_BLACK;
	}
	...
	if( m_bCaretVisible )
	{
		...
		if( m_timerCaret.GetLeftTime() < 500 )
		{
			if( PRIMARYLANGID( g_imeMgr.m_hkl ) == LANG_KOREAN && g_imeMgr.GetCompositon(NULL, 0) ) 
			{
				...
				p2DRender->RenderFillRect( CRect( pt1.x, pt1.y, pt1.x + size.cx, pt1.y + size.cy ), dwFillColor );
				p2DRender->TextOut( m_ptCaret.x, m_ptCaret.y, comp, dwCompositionTextColor );
			}
			else
			{
				p2DRender->RenderFillRect( CRect( pt1.x, pt1.y, pt1.x + 2, pt1.y + nFontBottom ), dwFillColor );
			}
		}
...
BOOL CWndEdit::IsYouMessage(UINT msg,WPARAM wparam, LPARAM lparam)
{
	...
	if(IsWndStyle(EBS_NUMBER) == TRUE)
	{
				...
				case VK_RETURN:
					{
						if( IsWndStyle( EBS_WANTRETURN ) )
						{
							...
							if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
								m_string.Insert( m_dwOffset, '\n', m_dwColor, 0, g_imeMgr.m_codePage );
		...
		case WM_KEYUP:
			{
				//FLTRACE_LOG( PROGRAM_NAME, _T( "WM_KEYUP %x" ), wparam );
				if( wparam == 0x0d )
				{
					int a = 0;
					a++;
...
*/
	...
	case WM_IME_ENDCOMPOSITION:          // 글자 조합의 끝. 조합된 문자열 처리를 끝낸다          break; 
		{
			...
			FLTRACE_LOG( PROGRAM_NAME, _T( "WM_IME_ENDCOMPOSITION" ) );
		}
		return TRUE;    
	...
	case WM_KEYDOWN:
		{
//			int	i;
			//FLTRACE_LOG( PROGRAM_NAME, _T( "WM_KEYDOWN %x" ), wparam );
			switch( wparam )
			{
				case 3: // Ctrl-C : Copy
					break;
...
{
#ifdef __IMPROVE_MAP_SYSTEM
	if( static_cast< DWORD >( strlen( m_stringBack ) ) >= m_dwMaxStringNumber )
	{
		return TRUE;
	}
#endif // __IMPROVE_MAP_SYSTEM
...
// 			return;
// 		FLTRACE_LOG( PROGRAM_NAME, _T( "(UINT nChar = %x)" ),nChar );
	...
	if( nChar == 22 && m_bEnableClipboard ) // Ctrl-V : Paste
	{
		...
		if( dwSize )
		{
			...
			if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
				m_string.Insert( m_dwOffset, buf, m_dwColor, 0, g_imeMgr.m_codePage );
	...
	if( nChar != 9 ) // !tab
	{
		...
		if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
			m_string.Insert( m_dwOffset, nChar, m_dwColor, 0, g_imeMgr.m_codePage );
...
BOOL CWndEdit::GetResultString()
{
	...
	for( int i = 0 ; i < (int)( len ); i++)
	{
		...
		if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
			m_string.Insert( i + m_dwOffset, str[ i ], m_dwColor, 0, g_imeMgr.m_codePage );
	}
	m_dwOffset+=len;
...
BOOL CWndEdit::GetCompString(LONG flag)
{
	...
	DWORD dwInputColor = (PRIMARYLANGID(g_imeMgr.m_hkl)==LANG_KOREAN) ? m_dwColor : 0xff0000ff;
	...
	for(int i=0; i<len; i++)
	{
		...
		if( CS_VER1 == _GetContentState( CT_NEWUI_19 ) )
			m_string.Insert( m_dwOffset + i, str[ i ], EDIT_COLOR, dwStyle, g_imeMgr.m_codePage );
		if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
			m_string.Insert( m_dwOffset + i, str[ i ], m_dwColor, dwStyle, g_imeMgr.m_codePage );
	}

	m_ptInputPos = OffsetToPoint( m_dwOffset + inputPos, m_szCaret, _countof( m_szCaret ) );
	
	m_pParentWnd->OnChildNotify( EN_CHANGE, m_nIdWnd, (LRESULT*)this ); 
	CPoint ptCaret = OffsetToPoint( m_dwOffset + curPos, m_szCaret, _countof( m_szCaret ) );
...
{
	WORD codePage = g_xFlyffConfig->GetCodePage();
...
BOOL CWndEdit::GetEditNum( OUT int& val )
{
	if( IsInteger( m_string.GetString( ) ) )
	{
		val = atoi( m_string.GetString() );
		return TRUE;
	}
...
BOOL CWndEdit::GetEditNum64( OUT __int64& val )
{
	if( IsInteger( m_string.GetString( ) ) )
	{
		val = _atoi64( m_string.GetString() );
		return TRUE;
	}
...
void		CWndEdit::SetTextSelectBlock(DWORD dwBegin,DWORD dwEnd)
{
	//텍스트 선택 블럭
	BlockSetStyle(ESSTY_BLOCK);
	m_dwBlockBegin = dwBegin;
	m_dwBlockEnd = dwEnd;
}
