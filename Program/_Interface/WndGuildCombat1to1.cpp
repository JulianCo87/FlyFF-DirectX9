...
//sun: 11, 캐릭터 정보 통합
		PlayerData* pPlayerData		= CPlayerDataCenter::GetInstance()->GetPlayerData( pMember->m_idPlayer );
		temp.Format( "No.%d  %s%.2d	%.16s %.10s", i+1, prj.GetText(TID_SKILL_BUFFICONLEVEL01),pPlayerData->data.nLevel, pPlayerData->szPlayer, prj.m_aJob[ pPlayerData->data.nJob ].szName );
...
//sun: 11, 캐릭터 정보 통합
		PlayerData* pPlayerData		= CPlayerDataCenter::GetInstance()->GetPlayerData( pMember->m_idPlayer );
		temp1.Format( "No.%d  %s%.2d	%.16s %.10s", nCurSel+1, prj.GetText(TID_SKILL_BUFFICONLEVEL01),pPlayerData->data.nLevel, pPlayerData->szPlayer, prj.m_aJob[ pPlayerData->data.nJob ].szName );
...
//sun: 11, 캐릭터 정보 통합
		...
		temp2.Format( "No.%d  %s%.2d	%.16s %.10s", nCurSel, prj.GetText(TID_SKILL_BUFFICONLEVEL01),pPlayerData->data.nLevel, pPlayerData->szPlayer, prj.m_aJob[ pPlayerData->data.nJob ].szName );
...
//sun: 11, 캐릭터 정보 통합
		pPlayerData		= CPlayerDataCenter::GetInstance()->GetPlayerData( pMember->m_idPlayer );
		temp2.Format( "No.%d  %s%.2d	%.16s %.10s", nCurSel+2, prj.GetText(TID_SKILL_BUFFICONLEVEL01),pPlayerData->data.nLevel, pPlayerData->szPlayer, prj.m_aJob[ pPlayerData->data.nJob ].szName );
...
void CWndGuildCombat1to1Offer::PaintFrame( C2DRender* p2DRender )
{
	...
	if( IsWndStyle( WBS_CAPTION ) )	
	{
		...
		if(m_nCombatType == 0)
			FLSPrintf( szNameLevel, _countof( szNameLevel ), "%s", prj.GetText(TID_GAME_GUILDCOMBAT_OFFER) );
		else if(m_nCombatType == 1)
			FLSPrintf( szNameLevel, _countof( szNameLevel ), "%s", prj.GetText(TID_GAME_GUILDCOMBAT_1TO1_OFFER) );
