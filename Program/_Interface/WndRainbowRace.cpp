...
{ 
	...
	if(m_pText != NULL && checkflag)
	{
		...
		m_pText->m_string.AddString(tempstr, D3DCOLOR_XRGB( 0xb4, 0xb4, 0xb4 ), ESSTY_BOLD);
...
void CWndRainbowRaceRanking::OnDraw( C2DRender* p2DRender ) 
{
	...
	for(int i=0; i<5; i++)
	{
		...
		m_p2DRender->TextOut( 30, 55 + nTextGap, strTemp, m_dwColor );
		...
		if(m_dwPlayerId[i] != 0 && pPlayerData && strlen(pPlayerData->szPlayer) > 0)
		{
			...
			// Draw Text Name.		
			m_p2DRender->TextOut( 80, 55 + nTextGap, pPlayerData->szPlayer, m_dwColor );
			...
			if(pWndWorld)
			{
				if( prj.m_aJob[ nJob ].dwJobType == JTYPE_PRO )			
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( 218, 55 + nTextGap ),  ( 19 + nJob - 6 ), &pVertices, WNDCOLOR_WHITE );
				else if( prj.m_aJob[ nJob ].dwJobType == JTYPE_MASTER )
				{
					...
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( 196, 55 + nTextGap ),  nMasterIndex, &pVertices, WNDCOLOR_WHITE );
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( 218, 55 + nTextGap ),  ( 19 + nJob - 16 ), &pVertices, WNDCOLOR_WHITE );
				}
				else if( prj.m_aJob[ nJob ].dwJobType == JTYPE_HERO )
				{
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( 196, 55 + nTextGap ),  33, &pVertices, WNDCOLOR_WHITE );
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( 218, 55 + nTextGap ),  ( 19 + nJob - 24 ), &pVertices, WNDCOLOR_WHITE );
				}
				else
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( 218, 55 + nTextGap ),  14 + nJob, &pVertices, WNDCOLOR_WHITE );
...
void CWndRainbowRaceMiniGame::OnDraw(C2DRender* p2DRender)
{
	for(int i=0; i<7; i++)
	{
		...
		{
			CTexture* pTexture;
			pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(), m_strPathCheck[i]), 0xffff00ff );
			if(pTexture != NULL)
...
{ 
	...
	m_bLoadTexMap = m_BtnTexture.LoadScript( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "texMapRainbowButton.inc" ) ) );
} 
...
{ 
	...
	if(!m_texTargetNum.LoadScript( D3DDEVICE, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"RainbowDiceNumber.inc" ) ))
		Destroy();
...
{ 
	...
	if(!m_texStopWatchNum.LoadScript( D3DDEVICE, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"RainbowStopWatchNumber.inc" ) ))
		Destroy();
	...
	if(nID == WIDC_START && message == WNM_CLICKED)
	{
		if( CS_VER1 == _GetContentState( CT_NEWUI_19 ) )
		{
			if(!m_bStart)
			{
				...
				if(pWndButton)
					pWndButton->SetTexture( m_pParentWnd->m_pApp->m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "Buttstop.bmp" ) ), TRUE );
			...
			else
			{
				CWndButton* pWndButton = (CWndButton*)GetDlgItem(WIDC_START);
				if(pWndButton)
					pWndButton->SetTexture( m_pParentWnd->m_pApp->m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "ButtStart.bmp" ) ), TRUE );
...
{ 
	...
	m_texCardBack = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"RainbowRaceCardBack.tga"), 0xffff00ff );
	...
	if(!m_texPenaltyNum.LoadScript( D3DDEVICE, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "RainbowCardPenaltyNumber.inc" ) ))
		Destroy();
...
void CWndRRMiniGameCard::SetQuestion(CString strQuestion)
{
	for(int i=0; i<strQuestion.GetLength(); i++)
	{
		...
		if(m_stCard[i].m_nCardNum >= 0 && m_stCard[i].m_nCardNum < 9)
			m_stCard[i].m_pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,  g_xFlyffConfig->GetMainLanguage(),m_strCard[m_stCard[i].m_nCardNum]), 0xffff00ff );
...
{ 
	...
	m_pTexBg1 = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "WndRainbowRaceLadderBg1.bmp"), 0xffff00ff );
	m_pTexBg2 = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "WndRainbowRaceLadderBg2.bmp"), 0xffff00ff );
	m_pTexHorizon = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "RainbowRaceLadderHorizon.bmp"), 0xffff00ff );
	m_pTexChoice = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"WndRainbowRaceLadderChoice.bmp"), 0xffff00ff );
	m_pTexPoint = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "WndRainbowRaceLadderPoint.bmp"), 0xffff00ff );
	m_pTexGoal = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"WndRainbowRaceLadderGoal.bmp"), 0xffff00ff );
	m_pTexFail = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "WndRainbowRaceLadderFail.bmp"), 0xffff00ff );
...
//		}
//		strtemp += "\n";
//		FLTRACE_LOG( PROGRAM_NAME, strtemp );
