...
void CWndSecretRoomSelection::AddCombatPlayer( u_long uiPlayer ) 
{
	...
	str.Format( "No.%d  %s%.2d	%.16s %.10s", pWndList->GetCount()+1, prj.GetText(TID_SKILL_BUFFICONLEVEL01),pPlayerData->data.nLevel, pPlayerData->szPlayer, prj.m_aJob[ pPlayerData->data.nJob ].szName );
...
void CWndSecretRoomSelection::RemoveCombatPlayer( int nIndex ) 
{
	...
	for(int i=nIndex; i<nListCount; i++)
	{
		...
		PlayerData* pPlayerData		= CPlayerDataCenter::GetInstance()->GetPlayerData( pMember->m_idPlayer );
		//temp.Format( "No.%d  %s%.2d	%.16s %.10s", i+1, pPlayerData->data.nLevel,prj.GetText(TID_SKILL_BUFFICONLEVEL01), pPlayerData->szPlayer, prj.m_aJob[ pPlayerData->data.nJob ].szName );

		int n1 = pPlayerData->data.nLevel;
		temp.Format( "No.%d Lv%d%.16s %.10s", i+1, n1,  pPlayerData->szPlayer, prj.m_aJob[ pPlayerData->data.nJob ].szName );
...
{
	//길드 마스터 정보가 없어서 요청한경우 5초마다 재확인해서 갱신한다.
	if( 0 != m_idHaveToFindMaster )
	{
		m_dwFindTime += _GetDeltaTime();
		if( m_dwFindTime > 5000 )
		{
			CString strFormat = CPlayerDataCenter::GetInstance()->GetPlayerString( m_idHaveToFindMaster );
			if( FALSE == strFormat.IsEmpty() )
			{
				CWndStatic* pStatic = (CWndStatic*)GetDlgItem( WIDC_STATIC9 );
				if( pStatic )
...
{ 
	...
	if( taxInfo )
		m_dwGuildId = taxInfo->dwCurrPlayerID;
	else
		m_dwGuildId = NULL_PLAYER_ID;
	...
	if(pGuild)
	{
		...
		//gmpbigsun(20120117), #22221 : 아직 마스터정보가 없다면 (5초간격으로)실시간으로 계속 옵져빙한다. 
		if( strFormat == CString( "" ) )
		{
			m_idHaveToFindMaster = pGuild->m_idMaster;
			strFormat = CString( "Now loading..." );
		}
...
{ 
	...
	if(pWndText)
	{
		pWndText->SetString(_T( prj.GetText(TID_GAME_SECRETROOM_CANCEL) ), m_dwColor );
		pWndText->ResetString();
	}
...
{ 
	...
	for(int i=0; i<nDrawCount; i++)
	{
		...
		if(lpWndCtrl == NULL)
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "SecretRoomQuick Draw i=%d" ), i );
			continue;
		}
...
{
	u_long memberList[MAX_SECRETROOM_MEMBER] = { 0 };
	//mem_set( memberList, 0, sizeof(memberList) );
