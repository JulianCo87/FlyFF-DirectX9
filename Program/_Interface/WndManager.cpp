...
#include "../Neuz/CoupleHelper.h"		// 13차 커플 시스템
...
{
	//gmpbigsun(20110722) : WndManager의 멤버변수는 이렇게 초기화해야 OnDestroyChildWnd 및 Free에서 자동으로 처리됩니다.
...
void CWndMgr::InitSetItemTextColor( )
{
	...
	dwItemColor[FIRST_TC].dwGeneralOption		= D3DCOLOR_XRGB(   0,   0,   0 );			// 일반 옵션
...
//sun: 12, 무기 피어싱
	...
	dwItemColor[FIRST_TC].dwBarunaRunePiercing	= D3DCOLOR_XRGB( 0, 0, 255 );				// 바루나 룬 피어싱
...
//sun: 12, 무기 피어싱
	...
	{
		dwItemColor[FIRST_TC].dwName0				= D3DCOLOR_XRGB( 120, 217, 255 );		// 노멀
		dwItemColor[FIRST_TC].dwName1				= D3DCOLOR_XRGB( 172, 220, 101 );		// 유니크
		dwItemColor[FIRST_TC].dwName3				= D3DCOLOR_XRGB( 255,  70, 170 );		// 얼터멋
		dwItemColor[FIRST_TC].dwName4				= D3DCOLOR_XRGB( 255,  64,  39 );		// 바루나
		dwItemColor[FIRST_TC].dwName5				= D3DCOLOR_XRGB( 255,  64,  39 );		// 바루나 방어구
		dwItemColor[FIRST_TC].dwAwakening			= D3DCOLOR_XRGB( 97,  97,  255 );		// 아이템 각성
		dwItemColor[FIRST_TC].dwBlessing			= D3DCOLOR_XRGB( 97,  97,  255 );		// 여신의 축복
		dwItemColor[FIRST_TC].dwGeneral				= D3DCOLOR_XRGB( 255, 255, 255 );		// 일반
		dwItemColor[FIRST_TC].dwGeneralOption		= D3DCOLOR_XRGB( 255, 234, 161 );		// 일반 옵션
		dwItemColor[FIRST_TC].dwPlusOption			= D3DCOLOR_XRGB( 255, 255, 255 );		// 플러스 옵션
		dwItemColor[FIRST_TC].dwPiercing			= D3DCOLOR_XRGB( 211, 134, 255 );		// 피어싱
		dwItemColor[FIRST_TC].dwSetItem1			= D3DCOLOR_XRGB(   1, 171,  25 );		// 세트 목록(착용시)
		...
		dwItemColor[FIRST_TC].dwNotUse				= D3DCOLOR_XRGB( 255,  0,    0 );		// 사용못함
		dwItemColor[FIRST_TC].dwBarunaRunePiercing	= D3DCOLOR_XRGB( 255, 64,	 0 );		// 바루나 룬 피어싱
	}
}
...
void CWndMgr::Free()
{
	...
	if( pWndTooltip )
		pWndTooltip->SetVisible( FALSE );		//접속종료 -> Login단계일경우 감춤
#endif

	DeleteAllCustomChild();
	m_pWndWorld = NULL;			//Appet 에서 삭제되므로 delete해서는 안된다.
}
...
void CWndMgr::OpenQuestQuickInfo()
{
	...
	FLERROR_ASSERT_LOG_RETURN_VOID( m_pWndQuestQuickInfo, _T("[윈도우 메니져] 퀘스트 빠른 정보 윈도우 생성 실패") );
...
{
	pShortcut->m_nExtraFrame = 60;		//..프레임동안 효과;;
...
//sun: 12, 튜토리얼 개선
			...
			else if( pShortcut->m_dwId == APP_VENDOR_REVISION )		//sun: 8차 엔젤 소환 Neuz, World, Trans
			{
			...
			{
				FLERROR_LOG( PROGRAM_NAME, _T("[윈도우 메니져] 애플릿 생성을 실패하였습니다. ID - [ %u ]"), pShortcut->m_dwId );
			}
		}
	}
...
//---------------------------------------------------------------------------------------------
// ESC를 눌러도 닫히지 않을 UI 목록을 이곳에 등록한다. ( 파괴하지 않을 UI 인가? )
// param	: 윈도우 아이디
// return	: 예 / 아니요
//---------------------------------------------------------------------------------------------
	...
	if( g_pPlayer )
	{
		if( g_bKeyTable[ pShortcutKey_System->KeyEscape ] && m_pLogOutWaitting == NULL )
		{
			...
			if( g_pPlayer->IsDie() )
			{
				...
				{
					//gmpbigsun(20110725) : 흠 부활창을 언제나 띄워선 안되는 상황이 발생했다. 
					//상황판단 함수가 추가됐따.
...
void CWndMgr::SetPlayer( CMover* pMover )
{
	...
	if( pMover != NULL )
	{
		...
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "ActiveMover 없음" ) );
		}
		if( CMover::GetActiveMover()->m_pActMover == NULL )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "ActionMover 없음" ) );
		}
...
//sun: 12, 튜토리얼 개선
	...
	//gmpbigsun( 20110321 ), #12958 : '$'가 발견된 경우 다음 문자가 i이거나 ni가 아니라면 의도적인 어뷰징으로 판단하고 걸러낸다.
	while( nWarning != -1 )
	{
	...
	/// 아이템 링크는 안걸러지게끔 한다.
	if( -1 != nStartLink || -1 != nEndLink ) //찾았다.
	{
		...
		if( -1 != nStart2 || -1 != nEnd2 ) //아이템 링크말고도 색상 관련 글자를 썼으면 사용금지.
		{
	...
	if( -1 != nStart || -1 != nEnd ) //찾았다.
	{
		CWndChat* pWndChat = ( CWndChat* )g_WndMng.GetWndBase( APP_COMMUNICATION_CHAT );
		if( !pWndChat )
			return;

		// 약속문자 시작 혹은 끝을 찻았으나, 둘다 없을경우 나쁜넘으로 간주
		if( -1 == nStart || -1 == nEnd )
		{
			string.Empty();
			pWndChat->m_kTempHLData_Send.reset();
		...
		if( nStart > 0 )
		{
			...
			while( nloop >= 0 )
			{
				...
				if( ch != ' ' && IsAlphaNum( ch ) == FALSE )		//바로앞문자가 특수문자면 나쁜놈으로 간주
				{
					string.Empty();
					pWndChat->m_kTempHLData_Send.reset();
					return;
...
void CWndMgr::PutToolTip_Skill( DWORD dwSkill, DWORD dwLevel, CPoint point, CRect* pRect, BOOL bActive )
{
	...
	if( pSkillProp == NULL )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "스킬(%d)의 프로퍼티가 업다." ), dwSkill );
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "add스킬(%d)의 프로퍼티가 업다." ), dwSkill );
		return;	// property not found
	}
...
//sun: 9차 전승관련 Clienet
	...
	DST_HEAL,	TID_TOOLTIP_DST_HEAL,	//sun: 9, 9-10차 펫
...
//sun: 9차 전승관련 Clienet
	...
	if( CS_VER1 == _GetContentState( CT_NEWUI_19 ) )
	{
		...
		if( pItemProp != NULL )
		{
			...
			{
				if( pItemElem->IsFlag( FLItemElem::binds ) == TRUE )				// 귀속 설정이 되있거나..		
				{
				...
				else if( pItemElem->IsFlag( FLItemElem::reset_bind ) == TRUE )// 귀속 해제를 한 경우..
				{
					strTemp.Format( "" );//
				}
			}	
...
{
	// 아이템 교환 창에서 온 아이템 일 경우 일반제련 된 아이템을 출력 위해
	// 정보를 얻어와서 아이템 뒤에 필요 제련 수치를 출력해 준다. ( EX. 우든소드 +9 ~ +11 )
	// 하드코딩이라 죄송합니다.
...
//sun, 11 케릭터 봉인 거래 기능 world,database,neuz( < 11 )
...
//---------------------------------------------------------------------------------------------
// 버블 타임 ( 추가 휴식 경험치 ) 툴팁 출력
// param	: MMI 정의 인덱스
// return	: 예 / 아니요
//---------------------------------------------------------------------------------------------
void CWndMgr::PutBubbleTime( CEditString* pEdit )
{
	FLERROR_ASSERT_LOG_RETURN_VOID( pEdit, _T("[윈도우 메니져] 편집 문자열 포인터가 유효하지 않습니다.") );
	FLERROR_ASSERT_LOG_RETURN_VOID( GuildHouse, _T("[윈도우 메니져] 길드 하우스 포인터가 유효하지 않습니다.") );
...
//gmpbigsun( 20101109 ) : 바루나 무기는 "각성가능" 문구 제거, #11159
//BEGIN ---------------------------------------------------------------------------------------------------
...
#endif
		if(pSpec->dwRandomOptionMode == 1 )// 각성 가능할때만,,Text찍어줘야되는데,,,ㅋ
		{
...
void CWndMgr::PutPetFeedPocket( FLItemElem* pItemElem, CEditString* pEdit )
{
	...
	if(pItemElem->m_dwKeepTime > 0) //유료아이템이 사용된 상태인가?
	{
		...
		if(g_pPlayer->HasBuff( BUFF_ITEM, ITEM_INDEX( 21035, II_SYS_SYS_SCR_PET_FEED_POCKET ) )) //활성화
		{
...
{
	FLERROR_ASSERT_LOG_RETURN_VOID( pItemElem, _T("[윈도우 메니져] 아이템 속성 포인터가 유효하지 않습니다.") );
...
//sun: 12, 세금
	...
	else if( flag == APP_SHOP_CART_ ) // 가격은 나는 안나오게 하고 상점 인터페이스에서는 나오게함 42은 상점의 Invantory 개수
	{
		// _JIN_SHOP_CART
		// 상점
		if( pItemElem->GetCost() != 0xffffffff ) 
	...
	else if( flag == APP_INVENTORY )	// 인벤토리 / 뱅크 툴팁?
	{
		...
		case CS_VER2:
			{
				CFLWndShop_Cart* pWndShop_Cart	= (CFLWndShop_Cart*)GetWndBase( APP_SHOP_CART_);
				if( pWndShop_Cart )	// 상점을 열엇을때 파는가격도 나와야함
				{
					if( pItemElem->GetCost() != 0xffffffff )
					{	//	파는가격
						char szbuffer[1024];
...
void CWndMgr::PutKeepTime( FLItemElem* pItemElem, CEditString* pEdit )
{
	...
	if( pItemSpec->IsVis( ) ) {
		...
		/************************************************************************/
		else if( t <= 0  )		// 장착된 비스만 유지시간을 받게 되고, 인벤에 위치한 비스는 Prop에서 가져다 써야 함.
		{
			...
			//ItemElem의 포인터에 대해 직접 값을 대입하면 안된다( 스택검사에서 실패뜸 )
			dwKeepTimeForTip = pItemSpec->dwAbilityMin;
			t = (time_t)( dwKeepTimeForTip * 60.0f );		//분단위 로 바뀜 
		}
	}
	...
	if( dwKeepTimeForTip && !pItemElem->IsFlag( FLItemElem::expired ) )
	{
		...
		{
			CTimeSpan time( t );
			if( g_xSpecManager->IsChargeZoneTicket( pItemElem->m_dwItemId ) != false )	// 왜 티켓만?
			{
				str.Format( prj.GetText( TID_TOOLTIP_DATE ), static_cast<int>(time.GetDays()), time.GetHours(), time.GetMinutes(), time.GetSeconds() );
			}
			else
			{
				if( pItemSpec->IsVis( ) )		// gmpbigsun : 일, 시간, 분, 초 의 텍스트를 읽어서 알맞게 조합해준다.
				{
...
void CWndMgr::PutLevel( CMover* pMover, FLItemElem* pItemElem, CEditString* pEdit )
{
	...
	CString strTemp;
	//flyingjin :  장착류와 일반아이템의 필요레벨 표시법 변경 
	// 장착류 :  최소레벨만 표시
	// 그외 : 최소 ~ 최대 레벨 표시 
	...
	if(!(nMinLevel == 1 && nMaxLevel == MAX_CHARACTER_LEVEL))
	{
		if(dwParts == NULL_ID)
		{                     
			if( nMinLevel > 1 || nMaxLevel < MAX_CHARACTER_LEVEL )//최고레벨보다 작을때멘 최소 ~ 최대 레벨표시 
			{
...
void CWndMgr::PutTHCombinedOption( FLItemElem* pItemElem, CEditString* pEdit )
{
	...
	strTemp.Format( "\n %s", szTitle );
	pEdit->AddString( strTemp.GetString() , 0xffff0000 );		//"[양손 무기 합성 옵션]"
...
void CWndMgr::MakeToolTipText( FLItemBase* pItemBase, CEditString& strEdit, int flag )
{
	...
	PT_ITEM_SPEC pItemProp( pItemBase->GetProp() );
	FLERROR_ASSERT_LOG_RETURN_VOID( pItemProp, _T("[윈도우 메니져] 아이템 속성을 찾을 수 없습니다. ID - [%u]"), pItemBase->m_dwItemId );
	...
	PutBind( pItemElem, &strEdit );			// 귀속 여부 출력 
...
//sun, 11, 케릭터 봉인 거래 기능 world,database,neuz
	...
	PutChangedLook( pItemElem, &strEdit );				//외형변경
...
//sun: 9차 전승관련 Clienet
	...
	if( pItemProp->dwID == ITEM_INDEX( 21035, II_SYS_SYS_SCR_PET_FEED_POCKET ) ) //먹이 주머니 툴팁
		PutPetFeedPocket( pItemElem, &strEdit );

	PutPiercingOpt( pItemElem, &strEdit );				//sun: 12, 무기 피어싱
...
//sun: 12, 무기 피어싱
	...
	PutAwakeningBlessing( pItemElem, &strEdit );		//sun: 11, 각성, 축복
	// 위치 변경 16.01.27
...
void CWndMgr::PutNeededVis( FLItemElem* pItemElem, CEditString* pEdit )
{
	...
	FLERROR_ASSERT_LOG_RETURN_VOID( pItemPropVis, _T("[윈도우 메니져] 비스 아이템의 속성을 얻어 올 수 없습니다.") );
	...
	if( NULL == pPetItem ) // 버프펫이 소환되어 있지 않은 상태
	...
	for( UINT nCount = 0; nCount < MAX_VIS_CONDITION_COUNT; nCount++ )
	{
		...
		FLERROR_ASSERT_LOG_DO( pVisGroupList, continue, _T("[윈도우 메니져] 비스 아이템의 그룹 리스트를 얻어 올 수 없습니다.") );
...
BOOL CWndMgr::CheckConfirm( FLItemBase* pItem )
{
	...
	if( IK3_MAPOPEN_DUNGEON == pItemProp->dwItemKind3 )
	{
		...
		pWnd->m_objid = g_pPlayer->GetId();
		pWnd->m_dwItemIndex = pItemProp->dwID;
		// %s 아이템을 사용할 경우 해당 지도가 팝업되며 아이템은 소멸 됩니다
...
//sun: 8차, 'T'키 사용, 상대방 레벨 안보이기, 화면 모든 창 On/Off Neuz
...
//sun: 12, 비밀의 방
		...
		//gmpbigsun( 20110526 ) : 이제는 태스크바도 없앨수있다.
...
void CWndMgr::CloseBoundWindow(void) // 아이템이 걸려 있거나, 아이템을 조작할 가능성이 있는 모든 창을 강제로 닫는다.
{
...
#ifdef COSTUME_UPGRADE_ENHANCEMENT_GEM
	SAFE_DELETE( m_pWndNewSmeltJewel );			// 보석 합성 창(아이콘)
#endif
...
#ifdef CARD_UPGRADE_SYSTEM
	SAFE_DELETE( m_pWndCardUpgrade );					// 카드 업그레이드
#endif	// CARD_UPGRADE_SYSTEM
#ifdef COSTUME_UPGRADE_MIX
	SAFE_DELETE( m_pWndSmeltItemMix );						// 아이템합성
#endif
	...
	// lenahyang
	// 월드 이동 시 단축 슬롯을 마우스에 들고 있을 경우 크레쉬 발생
	// 단축 슬롯 초기화
...
{
	//이미 선언되어있는 멤버변수를 생성시간에 초기화및 등록을 담당 
	//주의!! CWndWorld처럼 등록은 되어있으돼, 다른곳에서 생성 삭제를 하는경우 등록이 되어서는 아니된다.
	//예외로 다른곳에서 삭제를 할때 멤버변수 초기화를 책임진다는 가정하에 등록가능

	vecWndIter iter = std::find( m_vecChildWnds.begin(), m_vecChildWnds.end(), &pWndChild );
	if( iter != m_vecChildWnds.end() )
	{
		//이미 등록된 ChildWnd, 값만 바꾼다
		CWndBase** ppWnd = *iter;
		ppWnd = &pWndChild;
	}
	else
...
int _GetHeadQuest( const int nQuest )
{
	...
	while( pProp )	// 트리구조이기때문에 root를 찾아야 분류를 알수있다.
	{
...
{
	//퀘스트 이모티콘 texture set의 offset값이다.
...
CWndCommonInputCount* CWndMgr::GetWndCommonInputCount( CWndBase* pParentWnd )
{
	...
	FLERROR_ASSERT_LOG_RETURN( m_pWndCommonInputCount, NULL , _T("[윈도우 메니져] 공용 개수 입력 창 생성을 실패하였습니다.") );
