	...
	case WIDC_OK:
		{
			...
			if( 3 <= string.GetLength() && string.GetLength() < MAX_NAME )
			{
				if( 0 != strcmp( g_pPlayer->GetName(), string ) )
				{
					...
					FLItemElem* pItemElem = (FLItemElem*)g_pPlayer->GetItemId( wId );
					if( pItemElem )
					{
					...
					else 
					{
						...
						{
							char lpString[260]	= { 0, };
							FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_ERROR_SUMMONFRIEND_NOITEM ), pItemProp->szName, string );
...
void CWndSummonPartyMsg::SetData( OBJID objid, DWORD dwData, const char* szWorldName )
{
	...
	FLStrcpy( g_Neuz.m_szSummonPartyWorldName, _countof( g_Neuz.m_szSummonPartyWorldName ), szWorldName );
	CWndText* pWndText = (CWndText*)GetDlgItem( WIDC_TEXT1 );
	...
	case WIDC_OK:
		{
			...
			else 
			{
				...
				{
					char lpString[260]	= { 0, };
					FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_ERROR_SUMMONPARTY_NOITEM ), pItemProp->szName );
