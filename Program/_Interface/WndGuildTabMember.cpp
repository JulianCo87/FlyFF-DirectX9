...
//sun: 11, 메신저창 개선
	...
	PFONTCOLOR_WNDGUILDTABMEMBER pFontColorWndGuildTabMember = g_WndFontColorManager->GetWndGuildTabMember();
	...
	for( i = m_nCurrentList; i < m_nCurrentList + MAX_MEMBER_LIST_DRAW; i ++ )
	{
		...
		if( i == m_nSelect )
		{
			dwColor = pFontColorWndGuildTabMember->m_stSelect.GetFontColor();
		}
		else
		{
			if( !pMember->bIsOnLine )
				dwColor = pFontColorWndGuildTabMember->m_stOffLine.GetFontColor();
		}
...
//sun: 11, 메신저창 개선
		...
		case 0:
//sun: 11, 메신저창 개선
			pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( nposx, sy - 4 ), 44, &pVertices, WNDCOLOR_WHITE );
			break;
		case 1:
			{
				for( j = 0 ; j < nClass ; ++j )
				{
//sun: 11, 메신저창 개선
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( nposx + j * 10, sy - 4 ), 45, &pVertices, WNDCOLOR_WHITE );
		...
		case 2:
			{
				...
				{
//sun: 11, 메신저창 개선
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( nposx + j * 10, sy - 4 ), 46, &pVertices, WNDCOLOR_WHITE );
				}
			}
			break;
		case 3:
			{
				...
				{
//sun: 11, 메신저창 개선
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( nposx + j * 10, sy - 4 ), 47, &pVertices, WNDCOLOR_WHITE );
				}
			}
			break;
		case 4:
			{
				...
				{
//sun: 11, 메신저창 개선
					pWndWorld->m_texPlayerDataIcon.MakeVertex( p2DRender, CPoint( nposx + j * 10, sy - 4 ), 48, &pVertices, WNDCOLOR_WHITE );
				}
			}
			break;
		default:
...
//sun: 10차 전승시스템	Neuz, Worl233d, Trans
//sun: 11, 메신저창 개선
		CString strLevel;
		if(prj.m_aJob[ pMember->nJob ].dwJobType == JTYPE_MASTER)
		{
			strLevel.Format("%d-M", pMember->nLevel);
			p2DRender->TextOut( sx + 120, sy, strLevel, dwColor );		//gmpbigsun(20100816) : #9614 간격조정
		}
		else if(prj.m_aJob[ pMember->nJob ].dwJobType == JTYPE_HERO)
		{
			//strLevel = "HERO";
			strLevel.Format("%d-H", pMember->nLevel);					//gmpbigsun(20100920) : #10092 히어로 레벨표시
			p2DRender->TextOut( sx + 120, sy, strLevel, dwColor );		//gmpbigsun(20100816) : #9614 간격조정
		}
		else if(prj.m_aJob[ pMember->nJob ].dwJobType == JTYPE_LEGEND_HERO)
		{
			strLevel.Format("%d-H", pMember->nLevel);					//gmpbigsun(20100831) : #10092 히어로 레벨표시
			p2DRender->TextOut( sx + 120, sy, strLevel, dwColor );
		}
		else
//sun: 12, 길드 창의 멤버 텝에 항목별 정렬
		{
...
{ 
	...
	if( CS_VER1 == _GetContentState( CT_NEWUI_19 ) )
	{
		...
		if(pWndButton)
			pWndButton->SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(), "ButtGuildMemberTabMLevel.bmp" ), 0xffff00ff );

		pWndButton = (CWndButton*)GetDlgItem(WIDC_JOB);
		if(pWndButton)
			pWndButton->SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(), "ButtGuildMemberTabJob.bmp" ), 0xffff00ff );

		pWndButton = (CWndButton*)GetDlgItem(WIDC_LEVEL);
		if(pWndButton)
			pWndButton->SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME , g_xFlyffConfig->GetMainLanguage(), "ButtGuildMemberTabLevel.bmp" ), 0xffff00ff );

		pWndButton = (CWndButton*)GetDlgItem(WIDC_NAME);
		if(pWndButton)
			pWndButton->SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME , g_xFlyffConfig->GetMainLanguage(), "ButtGuildMemberTabChar.bmp" ), 0xffff00ff );

		pWndButton = (CWndButton*)GetDlgItem(WIDC_ALIAS);
		if(pWndButton)
			pWndButton->SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME , g_xFlyffConfig->GetMainLanguage(), "ButtGuildMemberTabAlias.bmp" ), 0xffff00ff );
	}

	UpdateData();
	SortbyMemberLevel();	//sun: 12, 길드 창의 멤버 텝에 항목별 정렬
...
{ 
	...
	if( pWndBase == &m_wndMenu )
	{
		...
		case MGI_GUILD_LEAVE:	// 탈퇴
			{
				if(  pGuild->GetQuest( QUEST_INDEX( 1, QUEST_WARMON_LV1 ) ) != NULL && pGuild->GetQuest( QUEST_INDEX( 1, QUEST_WARMON_LV1 ) )->nState == QS_BEGIN )	// 클락워크 퀘스트면 길드 해체 안됨
				{
					QuestProp* pQuestProp = prj.m_aPropQuest.GetAt( QUEST_INDEX( 1, QUEST_WARMON_LV1 ) );
					if( pQuestProp )
					{
...
//sun: 11, 캐릭터 정보 통합
			...
			FLStrcpy( m_aList[m_nMax].szName, _countof( m_aList[m_nMax].szName ), pszPlayer );

			FLStrcpy( m_aList[ m_nMax ].szAlias, _countof( m_aList[ m_nMax ].szAlias ), pMember->m_szAlias );
