...
{ 
	...
	SAFE_DELETE( g_WndMng.m_pWndGuildBankLog );		//sun: 11, 길드뱅크 로그창 추가
} 
...
{ 
	...
	//gmpbigsun( 20100922 ) #17716 : reinterpret_cast 뭔 개수작이냐!!
	//pLResult 는 항상 LPSHORTCUT type 이 아니다.
	//어쩔수 없다.. 원하는 메시지만 처리한다.
	CWndBase* pWndFrame = NULL;
	LPSHORTCUT	lpShortcut = NULL;
	if( WNM_CLICKED == message )
	{
...
void CWndAddItemLog::OnDraw( C2DRender* p2DRender ) 
{
	...
	for(int i=0; iter != m_vLogList.end(); i++, iter++)
	{
		...
		p2DRender->TextOut( 4, pt.y, *iter, m_dwColor );
...
void CWndReceivePenyaLog::OnDraw( C2DRender* p2DRender ) 
{
	...
	for(int i=0; iter != m_vLogList.end(); i++, iter++)
	{
		...
		p2DRender->TextOut( 10, pt.y, *iter, m_dwColor );
