...
{
	vector<PT_T_SHOPCART>::iterator itr; 
...
void  FLWndCart::OnBTOK()
{
	...
	vector<PT_T_SHOPCART> vecTem = CFLWndShop_Cart::WndShop_Cart()->GetVecCart();
	vector<PT_T_SHOPCART> vecInsert;
					...
					else //Slot롯이 1개 이상이면 
					{
						...
						{	
							...
							if(nTotalItemCount > 0)
							{
								...
								vector<PT_T_SHOPCART> vec = CFLWndShop_Cart::WndShop_Cart()->GetVecCart();
