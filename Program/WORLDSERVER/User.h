...
public:
	void	AddGuildHouseTenderMainWnd( map<DWORD, DWORD> & mapGHTenderList );
	void	AddGuildHouseTenderInfoWnd( DWORD dwGHIndex, int nTenderMinPenya, __int64 n64TenderPenya, vector<DWORD>& vecGuildId );
...
//	DWORD	CheckValidMove( const D3DXVECTOR3 & vPos );
...
#ifdef __PROFESSION
	void   			AddProfession();
	void			AddProfessionExp();
	void			AddProfessionLevel();
#endif // __PROFESSION
	/************************************************************************/
#ifdef __M_USER_BOTS
	bool isBot() const;
	...
	void AddTextEx(const TCHAR* szFormat, _Args&& ...args) {
		const auto str = mu::strfmt(szFormat, args...);
		if (str.size()) {
			this->AddText(str.c_str());
		}
	}

};

#define	SEC_SAVEPLAYER	500
