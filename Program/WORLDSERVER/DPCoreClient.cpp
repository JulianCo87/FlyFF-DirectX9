...
{
	if( ConnectToServer( lpszAddr, uPort, FLStreamProtocol::GetInstance(), TRUE ) )
	{
		MyRegister( uKey );
...
void CDPCoreClient::SendPing( void )
{
	if( FALSE == m_bAlive )
	{
		...
		*/
		...
		if( ConnectToServer( g_szCoreAddr, PN_CORESRVR + 0, FLStreamProtocol::GetInstance(), TRUE ) )
		{
			MyRegister( g_uKey );
...
void CDPCoreClient::OnAddBot(CAr& ar, DPID, DPID, OBJID) {
	BYTE byAdd = 0;
	u_long idBot = 0;
	std::string strName;
	ar >> byAdd >> idBot >> strName;

	if (byAdd == 0) {
		user_bot::CBotMng::getInstance()->addBot(idBot, strName);
	}
#ifndef __M_USER_BOTS_DB_BASE
	else { // name is may used by another channel (worldserver)
		   // so set it to "used"
		user_bot::CBotMng::getInstance()->getBotNameMng()->use(strName);
	}
#endif

}
void CDPCoreClient::SendAddBot(const u_long idBot, std::string& strName) {
	BEFORESENDDUAL(ar, PACKETTYPE_ADD_BOT, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << idBot;
	ar.WriteString(strName);
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPCoreClient::SendRemoveBot(const u_long idBot, std::string& strName) {
...
void	CDPCoreClient::OnAddBot(CAr& ar, DPID, DPID, OBJID) {
	...
	ar >> nBotType;
	user_bot::CBotMng::getInstance()->addBot(idBot, strName, nBotType);
}
void	CDPCoreClient::OnRemoveBot(CAr& ar, DPID, DPID, OBJID) {
	u_long idBot = 0;
	ar >> idBot;
#ifndef __M_USER_BOTS_RELOAD_CONFIG
	if (idBot != 0) 
#endif
	{
		user_bot::CBotMng::getInstance()->removeBot(idBot);
	}
}
