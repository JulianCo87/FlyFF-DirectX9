...
void FLWSUser::AddGC1to1TenderGuildView( int nPenya, int nRanking, time_t t, vector<CGuildCombat1to1Mng::__GC1TO1TENDER>& vecTenderGuild )
{
	if( IsDelete() )	return;
	m_Snapshot.cb++;
	m_Snapshot.ar << NULL_ID;
...
void FLWSUser::AddGC1to1MemberLineUpOpenWnd( vector<u_long>& vecMemberId )
{
	if( IsDelete() )	return;
	m_Snapshot.cb++;
	m_Snapshot.ar << NULL_ID;
...
{
	map<DWORD, FLWSUser*>::iterator it;
...
{
	map<DWORD, FLWSUser*>::iterator it = m_users.find( dwSerial );
	if( it == m_users.end() )
		return;
...
FLWSUser* FLWSUserManager::GetUser( DPID dpidCache, DPID dpidUser )
{
	...
	map<DWORD, FLWSUser*>::iterator it = m_users.find( dpidUser );
	if( it != m_users.end() )
		return it->second;
	else
		return NULL;
...
void FLWSUser::AddGCRequestStatus( __int64 nPrizePenya, vector<CGuildCombat::__REQUESTGUILD> vecRequestStatus )
{
...
// 선택 캐릭터 윈도우 띄움
void FLWSUser::AddGCSelectPlayerWindow( vector<CGuildCombat::__JOINPLAYER> &vecSelectPlayer, u_long uidDefender, BOOL bWindow, BOOL bRequestWar )
{
	if( IsDelete() )	return;
	...
	for( i = m_users.begin(); i != m_users.end(); ++i )
	{
		...
		else
		{
...
#endif	// __MAINSERVER
			{
				...
				vector<CEventGeneric::T_CALLTHEROLL_ITEM> vecCallTheRollItem;
...
void FLWSUser::AddReassembleOpenWnd( vector<DWORD> vecItemId )
{
	if( IsDelete() )	return;
...
void FLWSUser::AddSecretRoomContInfo( BYTE nContinent, BYTE nType, vector<__SECRETROOM_TENDER> & vecSecreetRoomTender, int nIndex )
{
	if( IsDelete() )	return;
	m_Snapshot.cb++;
	m_Snapshot.ar << GetId();
...
void FLWSUser::AddSecretRoomLineUpOpenWnd( vector<DWORD>& vecLineUpMember )
{
	if( IsDelete() )	return;
	m_Snapshot.cb++;
	m_Snapshot.ar << GetId();
...
void FLWSUser::AddSecretRoomTenderView( int nTenderPenya, int nRanking, time_t t, vector<__SECRETROOM_TENDER>& vecSRTender )
{
	if( IsDelete() )	return;
	m_Snapshot.cb++;
	m_Snapshot.ar << GetId();
...
{
	for( map<DWORD, FLWSUser*>::iterator it=m_users.begin(); it!=m_users.end(); ++it )
	{
		FLWSUser* pUser = it->second;
...
void FLWSUser::AddHousingVisitableList( vector<DWORD> & vecVisitable )
{
	if( IsDelete() )	return;
	m_Snapshot.cb++;
	m_Snapshot.ar << GetId();
...
void FLWSUser::AddGuildHouseTenderMainWnd( map<DWORD, DWORD> & mapGHTenderList )
{
	if( IsDelete() )	return;
	...
	for( map<DWORD, DWORD>::iterator it = mapGHTenderList.begin(); it != mapGHTenderList.end(); ++it )
	{
		m_Snapshot.ar << it->first << it->second;
	}
}

void FLWSUser::AddGuildHouseTenderInfoWnd( DWORD dwGHIndex, int nTenderMinPenya, __int64 n64TenderPenya, vector<DWORD>& vecGuildId )
{
	if( IsDelete() )	return;
...
#ifdef __M_USER_BOTS
bool FLWSUser::isBot() const {
	return user_bot::CBotMng::getInstance()->isBot(m_idPlayer);
}

long	FLWSUserManager::GetCount() const {
	return user_bot::CBotMng::getInstance()->getCurrentBotCount() + m_lCount;
}
#endif
#endif // __WORLDSERVER

#ifdef __PROFESSION
void FLWSUser::AddProfession()
{
	if( IsDelete() )
		return;
		
	if (m_nProfessionLevel < 1)
	{
		FLERROR_LOG(PROGRAM_NAME, _T("m_nProfessionLevel = %d : %s"), m_nProfessionLevel, GetName());
		return;
	}
#ifdef __SNAPSHOT_SEEK
	m_Snapshot.BeginPacket();
#else //__SNAPSHOT_SEEK
	m_Snapshot.cb++;
#endif //__SNAPSHOT_SEEK
	m_Snapshot.ar << GetId();
	m_Snapshot.ar << SNAPSHOTTYPE_PROFESSION;
	m_Snapshot.ar << m_nProfessionLevel;
	m_Snapshot.ar << prj.m_profession.nProfessionExp[m_nProfessionLevel - 1];

	vector<ProfessionCraft> vCraft = prj.m_profession.vCraft;
	m_Snapshot.ar << static_cast<std::uint32_t>(vCraft.size());

	for( DWORD i = 0; i < vCraft.size(); i++ )
	{
		m_Snapshot.ar << vCraft.at( i ).nCategorie;
		m_Snapshot.ar << vCraft.at( i ).nLevel;
		m_Snapshot.ar << vCraft.at( i ).nExp;
		m_Snapshot.ar << vCraft.at( i ).dwRewardId;
		m_Snapshot.ar << vCraft.at( i ).nRewardNum;
		m_Snapshot.ar << vCraft.at( i ).nPercent;
		m_Snapshot.ar << vCraft.at( i ).nPenya;
		for( int j = 0; j < 3; j++ )
		{
			m_Snapshot.ar << vCraft.at( i ).dwNeededItemId[j];
			m_Snapshot.ar << vCraft.at( i ).nNeededNum[j];
		}
	}		
}

void FLWSUser::AddProfessionLevel()
{
	if( IsDelete() )
		return;
#ifdef __SNAPSHOT_SEEK
...
#endif //__SNAPSHOT_SEEK
	m_Snapshot.ar << GetId();
	m_Snapshot.ar << SNAPSHOTTYPE_PROFESSION_LEVEL;
	m_Snapshot.ar << m_nProfessionLevel;
	m_Snapshot.ar << prj.m_profession.nProfessionExp[m_nProfessionLevel - 1];
}

void FLWSUser::AddProfessionExp()
{
	if( IsDelete() )
		return;
#ifdef __SNAPSHOT_SEEK
...
#endif //__SNAPSHOT_SEEK
	m_Snapshot.ar << GetId();
	m_Snapshot.ar << SNAPSHOTTYPE_PROFESSION_EXP;
	m_Snapshot.ar << m_nProfessionExp;
}
#endif // __PROFESSION
/************************************************************************/
