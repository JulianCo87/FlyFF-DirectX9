...
//	map< DWORD, FLWSUser* >::iterator _it  = pCtrl->m_2pc.begin(); \
//	map< DWORD, FLWSUser* >::iterator _end = pCtrl->m_2pc.end(); \
//	while( _it != _end )	\
//	{
//
...
//		while( pTile != NULL )																						\
//		{																											\
//			for( map<OBJID, OBJID>::const_iterator __it = pTile->arrCtrlSightMap[ CObj::linkPlayer ].begin(); __it != pTile->arrCtrlSightMap[ CObj::linkPlayer ].end(); ++__it )	\
//			{																										\
...
#define FOR_VISIBILITYRANGE( pCtrl )																				\
	...
	{																												\
		...
		while( pTile != NULL )																						\
		{																											\
		for( FLRing<CObj>::iterator /*map< CCtrl*, CCtrl* >::const_iterator*/ __it = pTile->listCtrlSight[ CObj::linkPlayer ].Begin(); __it != pTile->listCtrlSight[ CObj::linkPlayer ].End(); ++__it )	\
			{																										\
