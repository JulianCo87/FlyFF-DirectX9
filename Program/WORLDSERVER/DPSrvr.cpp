...
#ifdef __PROFESSION
	ON_MSG( PACKETTYPE_PROFESSION_CRAFT, OnProfessionCraft );
#endif // __PROFESSION
...
#ifdef __PROFESSION
BOOL CDPSrvr::DoUseItemTarget_UpgradeScroll( FLWSUser* pUser, FLItemElem* pMaterial, FLItemElem* pTarget )
{
	if( !pUser || !pMaterial || !pTarget )
		return FALSE;
	
	if( !pMaterial->GetProp() || !pTarget->GetProp() )
		return FALSE;
	
	return FALSE;
}

BOOL CDPSrvr::DoUseItemTarget_ProfessionChange(FLWSUser* pUser, FLItemElem* pMaterial, FLItemElem* pTarget )
{
	return FALSE;
}
#include <iostream>
void CDPSrvr::OnProfessionCraft( CAr& ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	FLWSUser* pUser = g_xWSUserManager->GetUser( dpidCache, dpidUser );
	if( IsValidObj( pUser ) )
	{
		int nSelect;
		OBJID objId[3];
		ar >> nSelect >> objId[0] >> objId[1] >> objId[2];
		
		if( nSelect < 0 || nSelect >= (int)prj.m_profession.vCraft.size() )
			return;
		
		//if( prj.m_profession.vCraft.at( nSelect ).nLevel > pUser->m_nProfessionLevel )
		//	return;
		
		FLItemElem* pItemElem[3];
		
		for( int i = 0; i < 3; i++ )
		{
			FLItemElem* pItemElemTemp = pUser->m_Inventory.GetAtId( objId[i] );
			if( !IsUsableItem( pItemElemTemp ) )
				return;
				
			if( pItemElemTemp->GetProp()->dwID != prj.m_profession.vCraft.at( nSelect ).dwNeededItemId[i] )
				return;
				
			if( pItemElemTemp->m_nItemNum < prj.m_profession.vCraft.at( nSelect ).nNeededNum[i] )
				return;
			
			pItemElem[i] = pItemElemTemp;
			
			if( i == 2 )
			{	
				if( pUser->GetGold() < prj.m_profession.vCraft.at( nSelect ).nPenya )
				{
					pUser->AddDefinedText( TID_GAME_LACKMONEY, "" );
					return;
				}

				pUser->AddGold( -prj.m_profession.vCraft.at( nSelect ).nPenya );
		
				FLItemElem itemElem;
				itemElem.m_dwItemId = prj.m_profession.vCraft.at( nSelect ).dwRewardId;
				itemElem.m_nItemNum = prj.m_profession.vCraft.at( nSelect ).nRewardNum;
#ifdef __SHOP_RARITY_GENERATION
				const auto prop = itemElem.GetProp();
				if (prop)
				{
					if(prop->dwItemKind1 == IK1_WEAPON)
						itemElem.SetRandomOpt(CRandomOptItemGen::GetInstance()->GenRandomOptItem(prop->dwLimitLevel1, 1.0f, prop, RANK_SUPER, true));
				}
#endif
				if (pUser->m_Inventory.GetEmptyCountByItemId(itemElem.m_dwItemId) <= 0)
				{
				...
				if( xRandom( 100 ) > prj.m_profession.vCraft.at( nSelect ).nPercent )
				{
					for( int j = 0; j < 3; j++ )
						pUser->UpdateItem( (BYTE)pItemElem[j]->m_dwObjId, UI_NUM, pItemElem[j]->m_nItemNum - prj.m_profession.vCraft.at( nSelect ).nNeededNum[j] );	

					//if( pUser->m_nProfessionLevel >= prj.m_profession.nMaxProfessionLevel )
					//	return;	

					CString strFail;
					strFail.Format( "Vous avez echoue lors de votre tentative de creation d'objet." );
					pUser->AddText(strFail);
							
				/*	if( pUser->m_nProfessionExp + prj.m_profession.vCraft.at( nSelect ).nExp / 2 >= prj.m_profession.nProfessionExp[pUser->m_nProfessionLevel - 1] )
					{
						pUser->m_nProfessionExp	= 0;
						pUser->m_nProfessionLevel++;
						pUser->AddProfessionLevel();
						
						CString str;
						str.Format( "Le niveau de votre metier vient d'augmenter." );
						pUser->AddText( str );
					}
					else
					{
						pUser->m_nProfessionExp += prj.m_profession.vCraft.at( nSelect ).nExp / 2;
						pUser->AddProfessionExp();					
						CString str;
						str.Format( "L'experience de votre metier a augmente de %d", prj.m_profession.vCraft.at( nSelect ).nExp / 2);
						pUser->AddText( str );
					}	*/				
				}
				else if( pUser->CreateItem( &itemElem ) )
				{
					for( int j = 0; j < 3; j++ )
						pUser->UpdateItem( (BYTE)pItemElem[j]->m_dwObjId, UI_NUM, pItemElem[j]->m_nItemNum - prj.m_profession.vCraft.at( nSelect ).nNeededNum[j] );
					...
					PT_ITEM_SPEC pItemProp = g_xSpecManager->GetSpecItem(itemElem.m_dwItemId);

					if (pItemProp)
					{
						CString strFail;
						strFail.Format("Vous avez reussi a creer : %s[x%d]", pItemProp->szName, itemElem.m_nItemNum);
						pUser->AddText(strFail);
					}

					
					/*if( pUser->HasBuff( BUFF_ITEM, II_SYS_SYS_SCR_EXPMETIER ) )
					{
						if( ( pUser->m_nProfessionExp + prj.m_profession.vCraft.at( nSelect ).nExp * 2 ) >= prj.m_profession.nProfessionExp[pUser->m_nProfessionLevel - 1] )
						{
							pUser->m_nProfessionExp	= 0;
					...
					{
						if( pUser->m_nProfessionExp + prj.m_profession.vCraft.at( nSelect ).nExp >= prj.m_profession.nProfessionExp[pUser->m_nProfessionLevel - 1] )
						{
							pUser->m_nProfessionExp	= 0;
							pUser->m_nProfessionLevel++;
							pUser->AddProfessionLevel();
						...
						{
							pUser->m_nProfessionExp += prj.m_profession.vCraft.at( nSelect ).nExp;
							pUser->AddProfessionExp();					
							CString str;
							str.Format( "L'experience de votre metier a augmente de %d", prj.m_profession.vCraft.at( nSelect ).nExp);
							pUser->AddText( str );
						}
					}*/
...
#ifdef __L_FWC_FIXES 
		const auto nMatchId = pUser->getfwcInfo().nMatchId;
		if (nMatchId != fwc::INVALID_MATCH_ID) {
			auto pMatch = fwc::CManager::get()->getMatchByID(nMatchId);
			if ((pMatch && pMatch->isFwcPlayer(pUser->m_idPlayer)
				&& pMatch->getnState() >= FWC_STATE_PREPARE)) { // don't save a user in a fwc match.
				return;
			}
		}
#endif

		switch (static_cast<CProject::eDest>(nCurSel))
		{
		default: case CProject::eDest::FLARIS: vPos = D3DXVECTOR3(6968.471F, 100.0F, 3329.339F); dwWorldID = 1; break; //
		case CProject::eDest::FLARIS_COLLECT: vPos = D3DXVECTOR3(6940.4F, 100.0F, 3105.6F); dwWorldID = 1; break; //
		case CProject::eDest::SAINT_CITY: vPos = D3DXVECTOR3(8315.0F, 100.0F, 3724.0F); dwWorldID = 1; break; //
		case CProject::eDest::RHISIS: vPos = D3DXVECTOR3(9112.0F, 66.0F, 4172.0F); dwWorldID = 1; break; //
		case CProject::eDest::DARKON_1: vPos = D3DXVECTOR3(5574.0F, 75.0F, 3904.0F); dwWorldID = 1; break; //
		case CProject::eDest::DARKON_2: vPos = D3DXVECTOR3(3808.0F, 59.0F, 4455.0F); dwWorldID = 1; break; //
		case CProject::eDest::DARKON_3: vPos = D3DXVECTOR3(3222.0F, 10.0F, 3428.0F); dwWorldID = 1; break; //
		case CProject::eDest::HARMONIN: vPos = D3DXVECTOR3(5947.0F, 100.0F, 5406.0F); dwWorldID = 1; break; //
		case CProject::eDest::ISHTAR: vPos = D3DXVECTOR3(7065.0F, 82.0F, 6427.0F); dwWorldID = 1; break; //
		case CProject::eDest::KAILUN: vPos = D3DXVECTOR3(7263.0F, 66.0f, 9125.0F); dwWorldID = 1; break; //
		case CProject::eDest::ARHAKIS: vPos = D3DXVECTOR3(6006.0F, 72.0F, 9253.0F); dwWorldID = 1; break; //
		case CProject::eDest::REALM: vPos = D3DXVECTOR3(49.0F, 100.0F, 152.0F); dwWorldID = 17; break;

		case CProject::eDest::VOLCANE: vPos = D3DXVECTOR3(1396.0F, 86.0F, 569.0F); dwWorldID = 203; break; //

		// case CProject::eDest::DESIREA: vPos = D3DXVECTOR3( 5270.0F, 100.0F, 4723.0F ); dwWorldID = 1; break; //
		case CProject::eDest::DESIREA_M: vPos = D3DXVECTOR3(4910.0F, 100.0F, 3458.0F); dwWorldID = 1; break; //
		case CProject::eDest::AMINUS: vPos = D3DXVECTOR3(6644.0F, 58.0F, 6454.0F); dwWorldID = 1; break; //
		case CProject::eDest::AMINUS_M: vPos = D3DXVECTOR3(6804.0F, 100.0F, 6065.0F); dwWorldID = 1; break; //
		case CProject::eDest::FROZEN: vPos = D3DXVECTOR3(346.8F, 182.3F, 150.46F); dwWorldID = 11; break; //
		case CProject::eDest::PANDORA: vPos = D3DXVECTOR3(719.6F, 182.5F, 106.8F); dwWorldID = 11; break; //
		case CProject::eDest::MAIDEN: vPos = D3DXVECTOR3(728.7F, 182.5F, 450.1F); dwWorldID = 11; break; //
		case CProject::eDest::FERIAL: vPos = D3DXVECTOR3(535.8F, 182.6F, 535.5F); dwWorldID = 11; break; //
		case CProject::eDest::HYLAL: vPos = D3DXVECTOR3(329.1F, 182.5F, 492.4F); dwWorldID = 11; break; //
		}

		if (pUser->IsDie())
			pUser->AddDiagText(prj.GetText(TID_GAME_MSGERRORTELEPORTER1));
		else if (pUser->IsCollecting())
			pUser->AddDiagText(prj.GetText(TID_GAME_MSGERRORTELEPORTER2));
		else if (pUser->IsAttackMode())
			pUser->AddDiagText(prj.GetText(TID_GAME_MSGERRORTELEPORTER3));
		else if (pUser->GetWorld() && pUser->GetWorld()->GetID() == WI_WORLD_GUILDWAR)
			pUser->AddDiagText(prj.GetText(TID_GAME_MSGERRORTELEPORTER4));
		else if (pUser->m_vtInfo.VendorIsVendor() || pUser->m_vtInfo.IsVendorOpen())
			pUser->AddDiagText(prj.GetText(TID_GAME_MSGERRORTELEPORTER5));
		else
