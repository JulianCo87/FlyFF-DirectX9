...
//
	...
	if( !g_DPSrvr.StartServer( g_szCacheBindIP, (u_short)( g_uKey + PN_WORLDSRVR ), FLStreamProtocol::GetInstance(), TRUE ) )
	{
...
// 		g_dpDBClient.SendQueryGuildBank( g_uKey );
	...
	{
	//77	FLTRACE_LOG( PROGRAM_NAME, _T( "GuildCombat.txt not found" ) );
	//	return FALSE;
	}
...
#ifdef __M_USER_BOTS
			user_bot::CBotMng::getInstance()->process();
#endif
