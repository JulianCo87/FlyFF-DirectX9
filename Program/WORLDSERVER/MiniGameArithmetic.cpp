...
BOOL CMiniGameArithmetic::Excute( FLWSUser* pUser, __MINIGAME_PACKET* pMiniGamePacket )
{
	if( CNpcChecker::GetInstance()->IsCloseNpc( MMI_LORD_RAINBOW_ARITHMETIC, pUser->GetWorld(), pUser->GetPos() ) == FALSE ) {
		return FALSE;
	}
...
string CMiniGameArithmetic::MakeQuestion()
{
	...
	if( nOper[2] > 1 )		// 숫자가 3개인 경우다. 
	{
		...
		FLSPrintf( szQuestion, _countof( szQuestion ), "%d %c %d %c %d", nNum[0], GetOperatorCh( nOper[0] ), nNum[1],
														GetOperatorCh( nOper[1] ), nNum[2] );
	}
	else
	{
		nResult = Calculate( nNum[0], nNum[1], nOper[0] );
		FLSPrintf( szQuestion, _countof( szQuestion ), "%d %c %d", nNum[0], GetOperatorCh( nOper[0] ), nNum[1] );
	}
