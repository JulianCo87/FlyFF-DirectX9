...
//	mover.m_nSlot	= nSlot;
	...
	if( mover.m_dwAuthorization > AUTH_ADMINISTRATOR )
	{
...
#ifdef __PROFESSION
	if (LoadProfession(&mover, qry) == false)
	{
		FLERROR_LOG(PROGRAM_NAME, _T("CDbManager::Join. LoadProfession failed. PlayerId:[%07d]")
			, mover.m_idPlayer);
		return;
	}
#endif
