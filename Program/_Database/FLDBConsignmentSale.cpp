...
void FLDBConsignmentSale::_SearchItemList( nsConsignmentSale::FLPacketWDB_SearchAck & o_ack, const nsConsignmentSale::FLPacketWDB_SearchReq & req )
{
	...
	//	const DWORD real_i	= ( req.m_bASC == TRUE ) ? i : dwTotCount - ( i + 1 );
		...
		map< UINT64, FLSaleInfo >::const_iterator it = m_mapSaleInfo.find( u64SRL );
...
void FLDBConsignmentSale::tryGetRegistedCount_AndSendRegistedCount( const DPID dpid, const u_long uPlayerID )
{
	...
	map< u_long, FLRegistCount >::const_iterator it = m_mapUserRegistCount.find( uPlayerID );
...
void FLDBConsignmentSale::_GetRegistedInfoList( nsConsignmentSale::FLPacketWDB_RegistedInfoListAck & o_ack, const u_long uPlayerID )
{
	...
	for( map< UINT64, FLSaleInfo >::const_iterator it = m_mapExpiredSaleInfo.begin(); it != m_mapExpiredSaleInfo.end(); ++it )
	{
		const FLSaleInfo & kSaleInfo	= it->second;
	...
	for( map< UINT64, FLSaleInfo >::const_iterator it = m_mapSaleInfo.begin(); it != m_mapSaleInfo.end(); ++it )
	{
		const FLSaleInfo & kSaleInfo	= it->second;
		if( kSaleInfo.uPlayerID != uPlayerID )
			continue;
...
{
	CMclAutoLock Lock( m_kGlobalCS );
	map< UINT64, FLSaleInfo >::const_iterator it = m_mapSaleInfo.find( u64SaleInfoSRL );
	if( it == m_mapSaleInfo.end() )
		return false;
...
{
	CMclAutoLock Lock( m_kGlobalCS );
	return m_mapUsginID.insert( map< UINT64, UINT64 >::value_type( u64SaleInfoSRL, u64SaleInfoSRL ) ).second;
}
...
void FLDBConsignmentSale::sumUserRegistCount( const u_long uPlayerID, const E_REGIST_MODE eMode, const int nSum )
{
	...
	map< u_long, FLRegistCount >::iterator it = m_mapUserRegistCount.find( uPlayerID );
	if( it == m_mapUserRegistCount.end() )
	{
		pair< map< u_long, FLRegistCount >::iterator, bool> ret = m_mapUserRegistCount.insert( make_pair( uPlayerID, FLRegistCount() ) );
		if( ret.second == false )
			return;

		map< u_long, FLRegistCount >::iterator it_ret = ret.first;
...
{
	if( m_mapSaleInfo.insert( /*map< UINT64, FLSaleInfo >::value_type*/make_pair( kSaleInfo.u64ItemInstanceSRL, kSaleInfo ) ).second == false )
		return false;

	m_mapSorted_ItemName.insert( make_pair( FLStringKey( kSaleInfo.u64ItemInstanceSRL, kSaleInfo.szItemName ), kSaleInfo.u64ItemInstanceSRL ) );
	m_mapSorted_LimitLevel.insert( make_pair( FLLimitLevelKey( kSaleInfo.u64ItemInstanceSRL, kSaleInfo.nLimitLevel, kSaleInfo.szItemName ), kSaleInfo.u64ItemInstanceSRL ) );
	m_mapSorted_Price.insert( make_pair( FLIntKey( kSaleInfo.u64ItemInstanceSRL, kSaleInfo.nPrice ), kSaleInfo.u64ItemInstanceSRL ) );
	m_mapSorted_OnePrice.insert( make_pair( FLOnePriceKey( kSaleInfo.u64ItemInstanceSRL, kSaleInfo.nPrice, kSaleInfo.kItemElem.m_nItemNum ), kSaleInfo.u64ItemInstanceSRL ) );
	m_mapSorted_AbilityOption.insert( make_pair( FLIntKey( kSaleInfo.u64ItemInstanceSRL, kSaleInfo.nAbilityOption ), kSaleInfo.u64ItemInstanceSRL ) );
...
{
	map< UINT64, FLSaleInfo >::iterator it = m_mapSaleInfo.find( u64SaleInfoSRL );
	if( it == m_mapSaleInfo.end() )
		return false;
...
{
	CMclAutoLock Lock( m_kGlobalCS );
	if( m_mapExpiredSaleInfo.insert( make_pair( kSaleInfo.u64ItemInstanceSRL, kSaleInfo ) ).second == false )
	{
		FLERROR_LOG( PROGRAM_NAME, _T("[ DUPLICATE ID (%u64) ]"), kSaleInfo.u64ItemInstanceSRL );
		return false;
	}
...
{
	return m_mapExpiredSaleInfo.insert( make_pair( kSaleInfo.u64ItemInstanceSRL, kSaleInfo ) ).second;
}

bool FLDBConsignmentSale::popExpiredData( const UINT64 u64SaleInfoSRL, FLSaleInfo & o_kSaleInfo )
{
	map< UINT64, FLSaleInfo >::iterator it = m_mapExpiredSaleInfo.find( u64SaleInfoSRL );
