...
#ifdef __PROFESSION
	CDbManager::SaveProfession(&query, m_kMover.m_idPlayer, m_kMover.m_nProfessionLevel, m_kMover.m_nProfessionExp, szQuery, _countof(szQuery));
#endif
...
//	mover.m_dwMute				= query.GetInt( "m_dwFlyTime" );
	...
	if( mover.m_dwAuthorization < AUTH_OBSERVER || mover.m_dwAuthorization > AUTH_ADMINISTRATOR
	...
	mover.m_Stat.SetDiceStr( query.GetInt( "DSm_nStr" ) );
	mover.m_Stat.SetDiceSta( query.GetInt( "DSm_nSta" ) );
	mover.m_Stat.SetDiceDex( query.GetInt( "DSm_nDex" ) );
	mover.m_Stat.SetDiceInt( query.GetInt( "DSm_nInt" ) );
