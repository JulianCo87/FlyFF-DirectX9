...
#ifndef __M_USER_BOTS_DB_BASE
#ifdef __M_USER_BOTS_CHECK_DB
	if (user_bot::CBotMng::getInstance()->isBotName(lpDbOverlappedPlus->AccountInfo.szPlayer) == true) {
...
#else
#ifdef __M_USER_BOTS_CHECK_DB
	if (user_bot::CBotDBMng::getInstance()->isBotName(lpDbOverlappedPlus->AccountInfo.szPlayer) == true) {
...
// 		}
...

		{
			...
			mover.m_Stat.SetDiceStr( qry->GetInt( "DSm_nStr" ) );
			mover.m_Stat.SetDiceSta( qry->GetInt( "DSm_nSta" ) );
			mover.m_Stat.SetDiceDex( qry->GetInt( "DSm_nDex" ) );
			mover.m_Stat.SetDiceInt( qry->GetInt( "DSm_nInt" ) );
...
// 	tCharacter.dwPlayerId = pMover->m_idPlayer;
	...
	vector< FLRemoveItem > vecRemoveItem;
	...
	for( vector< FLRemoveItem >::iterator it = vecRemoveItem.begin(); it != vecRemoveItem.end(); ++it )
	{
...
//	1
	...
	pGuildRank->GetRanking( pQuery, szSql, _countof( szSql ) );
...
void CDbManager::GetPenyaGuildGC( CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
	...
	vector<__GCRESULTVALUEGUILD>::iterator erase_pos = m_GCResultValueGuild.end();
	vector<__GCRESULTVALUEGUILD>::iterator pos = m_GCResultValueGuild.begin();
	vector<__GCRESULTVALUEGUILD>::iterator end = m_GCResultValueGuild.end();
...
void CDbManager::GetPenyaPlayerGC( CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
	...
	vector<__GCRESULTVALUEPLAYER>::iterator erase_pos = m_GCResultValuePlayer.end();
	vector<__GCRESULTVALUEPLAYER>::iterator pos = m_GCResultValuePlayer.begin();
	vector<__GCRESULTVALUEPLAYER>::iterator end = m_GCResultValuePlayer.end();
...
void CDbManager::GuildBankLogView( CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
	...
	switch( byListType )
	{
		case GI_LOG_VIEW_ITEM_ADD:
			FLSPrintf( szQuery, _countof( szQuery ), "RANKING.RANKING_DBF.dbo.usp_guildbank_log_view '%s','%06d','%02d'",
...
bool	CDbManager::LoadEquipInfo( CQuery* pQuery, DWORD dwPlayerId, DWORD dwMaxPartsSize, vector< EQUIP_INFO > & vecEquipInfo )
{
	TCHAR szQuery[64] = {0, };
...
#ifdef __PROFESSION
bool CDbManager::LoadProfession(CMover* pMover, CQuery* qry)
{
	char szQuery[MAX_QUERY_SIZE] = { 0, };
	FLSPrintf(szQuery, _countof(szQuery), "uspProfession @query = 'L1', @idPlayer = '%07d'", pMover->m_idPlayer);
	...
	if (qry->Fetch())
	{
		pMover->m_nProfessionLevel = qry->GetInt("nProfessionLevel");
		pMover->m_nProfessionExp = qry->GetInt("nProfessionExp");
	}
