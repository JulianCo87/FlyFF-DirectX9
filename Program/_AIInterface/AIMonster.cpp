...
DWORD CAIMonster::GetAtkRange( DWORD dwAtkMethod )
{
	...
	if( pItemProp == NULL )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "ItemProp->dwAtk%d이 NULL이다. (%s)" ), dwAtkMethod, GetMover()->GetName() );
	}
#endif
...
//	vecMonsterSkill.assign( it->second.begin(), it->second.end() );
	...
	//g_xWSUserManager->AddCreateSfxObj( pAttacker, pSkillProp->dwSfxObj ); // 스킬 시전 이펙트
