...
READ_TYPE ReadAILine( FILE* fp, char szTokens[3][64] )
{
	...
	int n = sscanf_s( szBuffer, "%s %s %s", szTokens[0], _countof( szTokens[0] ), szTokens[1], _countof( szTokens[1] ), szTokens[2], _countof( szTokens[2] ) );
#else
...
BOOL ReadAIScript()
{
	...
	FILE *fp = FLStdFileOpen( szFullPath, "rt" );
	if( fp == NULL )
	{
// 		char szMsg[80] = { 0, };
// 		FLSPrintf( szMsg, _countof( szMsg ), "%s file not found.", szFile );
// 		AfxMessageBox( szFile );
	...
	{
		char szMsg[80] = { 0, };
		FLSPrintf( szMsg, _countof( szMsg ), "%s file syntax error.", szFullPath );
		AfxMessageBox( szFile );
	}
...
BOOL CAIMonster2::IsIdleMode()
{
	return ( GetState() == AI2_IDLE ) ? TRUE : FALSE;
}
...
//			return;
	...
	case AI2_TRACKING:
		if( m_idTarget == NULL_ID )
		{
			...
			FLASSERT( m_idTarget != NULL_ID );
		}

		if( pMover->GetDestId() == m_idTarget )	// 중복 패킷 전송 막기
			break;

		// 이동할 목표물을 idTarget으로 설정.
		pMover->SetDestObj( m_idTarget, m_fAttackRange, TRUE );
		//g_xWSUserManager->AddMoverSetDestObj( pMover, m_idTarget, m_fAttackRange );
		break;
	case AI2_SEARCH:
		break;
	}
		...
		case CAT_QUAKEDOUBLE:
			{
				if( pTarget )
				{
					g_xWSUserManager->AddWorldShout( pMover->GetName(), prj.GetText(TID_GAME_BOSS_BIGMUSCLE_MSG_04),
		...
		case CAT_QUAKE_ONE:
			{
				...
				{
					TCHAR szChar[128] = { 0 };
					FLSPrintf( szChar, _countof( szChar ), prj.GetText(TID_GAME_BOSS_BIGMUSCLE_MSG_05), pTarget->GetName() );
					g_xWSUserManager->AddWorldShout( pMover->GetName(), szChar, pTarget->GetPos(), pTarget->GetWorld() );
				}
			}
			break;
		}					
