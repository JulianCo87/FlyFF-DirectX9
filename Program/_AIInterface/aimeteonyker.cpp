...
//	m_bHeal		= FALSE;	// 치유했나?
	...
	m_dwHPOld	= 100;
	m_bCritical		= FALSE;
#ifdef METEONYKER_SUBSUMMON
	m_nSummonHPPercent = 100;
#endif // METEONYKER_SUBSUMMON
}
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "moveTodst" ) );
...
//				return TRUE;
		...
		if( pProp && pProp->m_nSummProb )	// 값이 있으면 소환 AI가 있다.
		{
			...
			if ( (m_nSummonHPPercent > nHpPercent) && ( ( m_nSummonHPPercent - nHpPercent ) > 5  ) )
			{
				m_nSummonHPPercent = ((nHpPercent+4)/5)*5;
...
BOOL	CAIMeteonyker::ProcessRetaliation( void )
{
	if( --m_cbRetaliation <= 0 )
	{
		...
		if( objid != NULL_ID )
		{
			...
			// 30초 최고 딜러 공격 신호
			char szText[256]	= { 0,};
			FLSPrintf( szText, _countof( szText ), prj.GetText( TID_GAME_BOSS_METEONYKER_MSG_03 ), pDealer->GetName() );
			g_DPCoreClient.SendCaption( szText, pMover->GetWorld()->GetID(), FALSE, pDealer->m_idparty );	
...
BOOL CAIMeteonyker::ProcessSPAttack( void )
{
	...
	if( nCount == 19 )	// 광역 예고
	{
		...
		CMover * pTarget = ScanTarget( pMover, 60, JOB_ALL );		// 80미터 반경내에서 아무나 고른다.
		...
		if( IsValidObj(pTarget) )
		{
			...
			{
				// 소환 공격 예고
				g_DPCoreClient.SendCaption( prj.GetText( TID_GAME_BOSS_METEONYKER_MSG_01 ), pMover->GetWorld()->GetID(), FALSE, pTarget->m_idparty );	
			}
			else
			{
				// 광역 버프 예고
				g_DPCoreClient.SendCaption( prj.GetText( TID_GAME_BOSS_METEONYKER_MSG_02 ), pMover->GetWorld()->GetID(), FALSE, pTarget->m_idparty );	
			}
		}
	}
	else if( nCount == 0 )
...
void CAIMeteonyker::SubSummonProcess( CMover * /*pTarget*/ )
{
	...
	if( pProp && pProp->m_nSummProb )	// 값이 있으면 소환 AI가 있다.
	{
		...
		//if( pProp->m_nSummNum > (int)( m_vecIdSummon.size() ) )
		{
			if( (int)( xRandom(100) ) < pProp->m_nSummProb )	// 소환 시도.
			{
				...
				int nSummonPlus = 0;
				if ( m_nSummonHPPercent <= 5 )
					nSummonPlus = 2;
				for ( int i = 0; i < pProp->m_nSummNum+nSummonPlus; i++ )
				{
					...
					vLocal.x = 1.5f - xRandomF( 13.0f );		// 보스 주위에 랜덤으로 생성.
					vLocal.y = 0;
					vLocal.z = 1.5f - xRandomF( 13.0f );
					vPos = pMover->GetPos();
					vPos += vLocal;
					pObj->SetPos( vPos );
